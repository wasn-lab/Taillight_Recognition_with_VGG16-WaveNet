...
class Node:
    ...
    def path_prediction_output_callback(self, message):
        ...
        for objectMessage in message.objects:
            box_list.markers.append( self.create_bounding_box_marker( idx, objectMessage, message.header) )
            idx += 1
            text_list.markers.append( self.create_delay_text_marker( idx,  objectMessage, message.header) )
            idx += 1
        self.box_mark_pub.publish(box_list)
        self.delay_txt_mark_pub.publish(text_list)

    def create_bounding_box_marker(self, idx, message, header):
        marker = Marker()
    ...
    def create_delay_text_marker(self, idx, message, header):
        marker = Marker()
        ...
        # marker.text = "%.3fms" % ((self.t_clock - header.stamp).to_sec() * 1000.0)
        ...
        point_2 = a_box.p6
        #
        marker.pose.position.x = (point_1.x + point_2.x)*0.5
        marker.pose.position.y = (point_1.y + point_2.y)*0.5
        marker.pose.position.z = (point_1.z + point_2.z)*0.5 + 2.0
