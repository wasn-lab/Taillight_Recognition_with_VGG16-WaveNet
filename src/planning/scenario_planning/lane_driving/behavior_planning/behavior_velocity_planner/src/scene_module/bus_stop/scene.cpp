 ...
 */
#include <scene_module/bus_stop/scene.h>
...
BusStopModule::BusStopModule(
  const int64_t module_id, const lanelet::ConstLineString3d & bus_stop,
  ...
  autoware_planning_msgs::PathWithLaneId * path, autoware_planning_msgs::StopReason * stop_reason)
{
  ...
  *stop_reason =
    planning_utils::initializeStopReason(autoware_planning_msgs::StopReason::BUS_STOP);
  ...
  if (state_ == State::APPROACH) 
  {
    for (size_t i = 0; i < path->points.size() - 1; ++i) 
    {
      ...
      bg::intersection(bus_stop, line, collision_points);
...
bool BusStopModule::getBackwordPointFromBasePoint(
...
geometry_msgs::Point BusStopModule::getCenterOfStopLine(
  const lanelet::ConstLineString3d & bus_stop)
{
  geometry_msgs::Point center_point;
  center_point.x = (bus_stop[0].x() + bus_stop[1].x()) / 2.0;
  center_point.y = (bus_stop[0].y() + bus_stop[1].y()) / 2.0;
  center_point.z = (bus_stop[0].z() + bus_stop[1].z()) / 2.0;
  return center_point;
}
