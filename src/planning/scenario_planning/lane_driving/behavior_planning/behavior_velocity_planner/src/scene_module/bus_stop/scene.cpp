...
: SceneModuleInterface(module_id), bus_stop_(bus_stop), state_(State::APPROARCH)
{
}

bool BusStopModule::modifyPathVelocity(autoware_planning_msgs::PathWithLaneId * path)
{
  debug_data_ = {};
  debug_data_.base_link2front = planner_data_->base_link2front;

  Eigen::Vector2d stop_point;
  bg::model::linestring<Point> bus_stop = {{bus_stop_[0].x(), bus_stop_[0].y()},
                                            {bus_stop_[1].x(), bus_stop_[1].y()}};
