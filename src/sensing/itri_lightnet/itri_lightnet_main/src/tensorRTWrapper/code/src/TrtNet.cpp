...
trtNet::trtNet(const std::string &engineFile)
    : mTrtContext(nullptr), mTrtEngine(nullptr), mTrtRunTime(nullptr), mTrtRunMode(RUN_MODE::FLOAT16), mTrtInputCount(0), mTrtIterationTime(0)
{
    using namespace std;
    fstream file;
                                                        ...
                                                        IInt8Calibrator *calibrator, IHostMemory *&trtModelStream, const std::vector<std::string> &outputNodesName)
{
    ...
    builder->setMaxWorkspaceSize(2 << 10); // 1G
