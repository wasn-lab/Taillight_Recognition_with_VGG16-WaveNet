...
bool ImageTransfer::put( std::shared_ptr<cv::Mat> &content_in_ptr  ){
    ...
    if (!content_in_ptr.unique() ){ // Not unique
        // Copy element
        ...
        if (!content_in_ptr.unique() ){ // Not not unique (empty or shared)
...
bool ImageTransfer::get( std::shared_ptr<cv::Mat> &content_out_ptr ){
    ...
    if ( is_ptr_shared(_data_ptr) ){ // Not null and not unique
...
RosImagePubSub::RosImagePubSub(ros::NodeHandle &nh_in):
    _nh_ptr(&nh_in),
    _ros_it(nh_in)
{
...
bool RosImagePubSub::add_a_sub(size_t id_in, const std::string &topic_name){
    auto result = _image_subscriber_map.emplace( 
        ...
        _ros_it.subscribe( 
            topic_name, 
            10, 
            boost::bind(&RosImagePubSub::_CompressedImage_CB, this, _1, id_in),
            ros::VoidPtr(),
