...
{
  auto result = _image_publisher_map.emplace(id_in, _ros_it.advertise(topic_name + std::string("/relay"), 1));
  _heartbeat_publisher_map.emplace(id_in,
                                   _nh_ptr.advertise<std_msgs::Empty>(topic_name + std::string("/relay/heartbeat"), 1));
