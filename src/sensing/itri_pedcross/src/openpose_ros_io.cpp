...
OpenPoseROSIO::OpenPoseROSIO(OpenPose &openPose): nh_("/openpose_ros_node"), it_(nh_)
{
    ...
    openpose_human_list_pub_ = nh_.advertise<msgs::OpenPoseHumanList>(output_topic, 10);
    cv_img_ptr_ = nullptr;
    openpose_ = &openPose;
...
void OpenPoseROSIO::processImage(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    std::cout<<"time cost: "<<ros::Time::now() - start<<std::endl;
}
...
void OpenPoseROSIO::publish(const std::shared_ptr<std::vector<std::shared_ptr<op::Datum>>>& datumsPtr)
{
    if (datumsPtr != nullptr && !datumsPtr->empty())
    {
        ...
        msgs::OpenPoseHumanList human_list_msg;
        ...
        std::vector<msgs::OpenPoseHuman> human_list(poseKeypoints.getSize(0));

        for (auto person = 0 ; person < poseKeypoints.getSize(0) ; person++)
        {
            msgs::OpenPoseHuman human;
            ...
            for (auto bodyPart = 0 ; bodyPart < poseKeypoints.getSize(1) ; bodyPart++)
            {
                msgs::PointWithProb body_point_with_prob;
            ...
            if(FLAGS_face)
            {
                ...
                {
                    msgs::PointWithProb face_point_with_prob;
                ...
                msgs::BoundingBox face_bounding_box;
            ...
            if(FLAGS_hand)
            {
                ...
                {
                    msgs::PointWithProb right_hand_point_with_prob;
                    msgs::PointWithProb left_hand_point_with_prob;
