...
public:
  ...
  inline void setThetaSample(float theta_sample)
  {
    theta_sample_ = theta_sample;
  };

  inline void setMaxRadious(float max_radius)
  {
    max_radius_ = max_radius;
  };

  inline void setGridMinValue(float grid_min_value)
  {
    grid_min_value_ = grid_min_value;
  };

  inline void setGridMaxValue(float grid_max_value)
  {
    grid_max_value_ = grid_max_value;
  };

  inline void setGridLengthOfX(float grid_length_x)
  {
    grid_length_x_ = grid_length_x;
  };

  inline void setGridLengthOfY(float grid_length_y)
  {
  ...
  inline void setMaxLidarHeightThres(float maximum_lidar_height_thres)
  {
    maximum_lidar_height_thres_ = maximum_lidar_height_thres;
  };

  inline void setMinLidarHeightThres(float minimum_lidar_height_thres)
  {
    minimum_lidar_height_thres_ = minimum_lidar_height_thres;
  };

  inline void setLayerName(std::string layer_name)
  {
  ...
  void setInputCloud(const PointCloud<PointXYZI>::ConstPtr input);

  void cloudPosPreprocess(const PointCloud<PointXYZI>::ConstPtr& input_cloud,
                          pcl::PointCloud<PointXYZI>::Ptr& output_cloud, float deg_x, float deg_y, float deg_z);
  ...
  assignPoints2GridCell(const grid_map::GridMap& gridmap, const pcl::PointCloud<pcl::PointXYZI>::Ptr& in_sensor_points);
  // grid_map_generator
  void initGridmap();

  grid_map::Matrix generatePointsGridmap(const pcl::PointCloud<pcl::PointXYZI>::Ptr& in_sensor_points);

  inline pcl::PointCloud<pcl::PointXYZI>::Ptr getRingEdgePointCloud()
  {
  ...
  inline pcl::PointCloud<pcl::PointXYZI>::Ptr getGroundPointCloud()
  {
    return ground_pointCloud_;
  };

  inline pcl::PointCloud<pcl::PointXYZI>::Ptr getNonGroundPointCloud()
  {
