...
void
EdgeDetection::LogTotxt(const std::vector<float> contour_distance)
{
...
bool
EdgeDetection::RegisterCallbacks(const ros::NodeHandle& n)
{
...
void
EdgeDetection::setInputCloud (const PointCloud<PointXYZI>::ConstPtr input)
{
        *release_cloud_ = *input;
}

void
EdgeDetection::startThread ()
{
...
grid_map::Matrix
EdgeDetection::generatePointsGridmap(const pcl::PointCloud<pcl::PointXYZI>::Ptr& in_sensor_points)
{
        grid_map::Matrix sensor_points_costmap = makeCostmapFromSensorPoints(
                maximum_lidar_height_thres_, minimum_lidar_height_thres_, grid_min_value_, grid_max_value_, gridmap_,
                layer_name_, in_sensor_points);
...
//    **voxel downsample************************
        ...
        *sampled_cloud_non_terrain_2D = *cloud_non_terrain_2D;
        ...
        getContourV2(sampled_cloud_non_terrain_2D,sampled_cloud_terrain_2D, ring_edge_pointCloud_, sampled_cloud_innerboundary,contour_distance, theta_sample_, max_radius_, false );
