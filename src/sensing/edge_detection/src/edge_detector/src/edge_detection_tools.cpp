        ...
        *cloud_innercontour = *cloud_ground;
        ...
        if (calculate_interior)
        {
                for (size_t i = 0; i < cloud_innercontour->points.size (); ++i)
                {
                        ...
                        {
                                cloud_innercontour->points[i].x = cloud_innercontour->points[i].y = cloud_innercontour->points[i].z = std::numeric_limits<float>::quiet_NaN();;
                        }
...
{
        pcl::FPFHEstimation<pcl::PointXYZ, pcl::Normal, pcl::Histogram<100> >::Ptr fpfh (new pcl::FPFHEstimation<pcl::PointXYZ, pcl::Normal, pcl::Histogram<100> >);
...
//** RANSAC parallel_ine extract ***************************
        if (input_cloud->points.size() > 1) // bigger than 1 RANSAC
        {
                ...
                pcl::SampleConsensusModelParallelLine<PointT>::Ptr model_line(new pcl::SampleConsensusModelParallelLine<PointT> (input_cloud));
