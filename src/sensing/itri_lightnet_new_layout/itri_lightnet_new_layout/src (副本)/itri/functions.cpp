...
bool inconsistentMemory[6];
bool inconsistentState = false;
bool consistentlyInconsistent = false;

int imageWidth  = 608; 	//USER_DEFINED_PARAM
int imageHeight = 384;	//USER_DEFINED_PARAM
int depthFOV = 30;	 	//USER_DEFINED_PARAM
int cnvFrame_th = 2; 	//USER_DEFINED_PARAM
int minDims = 4;   		//USER_DEFINED_PARAM
...
void filterBB (int inpBulb[90][8], int &numInp, int outBulb[90][10], int &numOut, int camSelector)
{
	...
	for (int l = 0; l < numInp; l++)
	{
		if (inpBulb[l][0] > 0)
		{
			...
			{
				float f_30 = 1111;
				if(outBulb[numOut][4] - outBulb[numOut][2] <=0)
					outBulb[numOut][9] = 0;
				else
		...
		if (Bulb[l][8] == -1)
		{
			...
			for (int m = 0; m < numBB; m++)
			{
				if (l != m)
				{
					...
					float distY = 1.75 * (tStart2.y - tStart.y);
...
void finalizeTrafficLight(bool StatA[6], int depthA, bool StatB[6], int depthB, bool finStat[2][6], int finDepth[2])
{
	...
	if (finDepth[1] > 0)
	{
		if (finDepth[0] > 0)
		{
			...
			for (int k = 0; k < 6; k++)
			{
				...
				{
					if (inconsistentMemory[k] != finStat[1][k])
					{
						matchFrame = false;
					}
				}
			...
			if (matchFrame == false)
			{
				...
				if (matchcurLight == false)
				{
					...
					for (int k = 0; k < 6; k++)
					{
						...
						{
							if (inconsistentMemory[k] != StatA[k])
							{
								matchA = false;
							}
							if (inconsistentMemory[k] != StatB[k])
							{
								matchB = false;
							}
						}
					...
					if (matchpreLight == false)
					{
						if (falseCount < 3)
						{
							if(inconsistentMemory)
							{
								for (int k = 0; k < 6; k++)
								{
									finStat[1][k] = inconsistentMemory[k];
								}
							}
							else
							{
	...
	//Logic Consistency Reinforcement Patch
	//Start
	if (finStat[1][2])
	{
		if (finStat[1][3])
	...
	if (finStat[1][0])
	{
		...
		if (finStat[0][2] || finStat[0][3] || finStat[0][4] || finStat[0][5])
		{
			if (!finStat[1][2] && !finStat[1][3] && !finStat[1][4] && !finStat[1][5])
			{
				finStat[1][0] = false;
				finStat[1][1] = true;
			}
	...
	if (!inconsistentState)
	{
		for (int k = 0; k < 6; k++)
		{
			if (finStat[1][k] != finStat[0][k])
			{
				inconsistentCounter = 1;
		...
		if (inconsistentState)
		{
			for (int k = 0; k < 6; k++)
			{
				inconsistentMemory[k] = finStat[1][k];
				finStat[1][k] = finStat[0][k];
			}
		}
	}
	else
	{
		...
		//Pass Current Detection and Remove inconsistent Memory
		if (checkConsistency)
		{
		...
		else
		{
			for (int k = 0; k < 6; k++)
			{
				if (finStat[1][k] == inconsistentMemory[k])
				{
					inconsistentCounter++;
				}
				else
				{
					consistentlyInconsistent = true;
				}
			}
			if (!consistentlyInconsistent)
			{
				if (inconsistentCounter > cnvFrame_th)
				{
					for (int k = 0; k < 6; k++)
					{
						finStat[1][k] = inconsistentMemory[k];
			...
			else
			{
				...
				inconsistentCounter = 1;
				consistentlyInconsistent = false;
			}
		}
	}
