...
void initi_all()
{
    ...
    for (int k = 0; k < 6; k++)
    {
        ...
        preLightStatus30[k] = false;
        finLightStatus60[k] = false;
        preLightStatus60[k] = false;
    }
    for (int l = 0; l < 2; l++)
    {
        for (int k = 0; k < 6; k++)
...
void DoNet(cv_bridge::CvImagePtr cv_ptr_30deg, cv_bridge::CvImagePtr cv_ptr_60deg, struct TL_status *TL_status_info, struct TL_color *TL_color_info)
{
    ...
    //FASTHDR(&dImg, clahe_30deg, &dImg_hdr);
    //FASTHDR(&dImg_60deg, clahe_60deg, &dImg_60deg_hdr);


    // dImg_hdr = cv_ptr_30deg->image;
    // dImg_60deg_hdr = cv_ptr_60deg->image;
#ifdef display_cv_window
    dImg.copyTo(dImg1);
    ...
    for (const auto &item : bbox)
    {
        ...
        // if (normClass == 6)
        // {
        //     trafficLightBody30[numOfBody30][0] = 1;
        //     trafficLightBody30[numOfBody30][1] = item.left;
        //     trafficLightBody30[numOfBody30][2] = item.top;
        //     trafficLightBody30[numOfBody30][3] = item.right;
        //     trafficLightBody30[numOfBody30][4] = item.bot;
        //     trafficLightBody30[numOfBody30][5] = (item.right - item.left) * (item.bot - item.top);
        //     numOfBody30++;
        //    // putText(dImg, 0, Point(_tlBody30[selectTL30][1], _tlBody30[selectTL30][2]), 1, 1, Scalar(0, 120, 255), 2, 8, 0);
        //     //rectangle(dImg, Point(item.left, item.top), Point(item.right,item.bot), Scalar(0, 120, 255), 1, 8, 0);
        // }
        // else 
        if (normClass < 6)
        {
    ...
    // 60 deg camera result
    for (const auto &item : bbox_60deg)
    {
        normClass = item.classId;
        // if (normClass == 6)
        // {
        //     trafficLightBody60[numOfBody60][0] = 1;
        //     trafficLightBody60[numOfBody60][1] = item.left;
        //     trafficLightBody60[numOfBody60][2] = item.top;
        //     trafficLightBody60[numOfBody60][3] = item.right;
        //     trafficLightBody60[numOfBody60][4] = item.bot;
        //     trafficLightBody60[numOfBody60][5] = (item.right - item.left) * (item.bot - item.top);
        //     numOfBody60++;
        //     //rectangle(dImg_60deg, Point(item.left, item.top), Point(item.right,item.bot), Scalar(0, 120, 255), 1, 8, 0);
        ...
        if (normClass < 6)
        {
            ...
            //printf("item.bot = %d\n",item.bot);
            //printf("item.top = %d\n", item.top);
            //rectangle(dImg1_60deg, Point(item.left, item.top), Point(item.right,item.bot), Scalar(0, 120, 255), 1, 8, 0);
            numOfBulb60++;
        }
    }
    ...
    animateTrafficLight(animateFin, finLightStatus[1], predict_pt.x);
