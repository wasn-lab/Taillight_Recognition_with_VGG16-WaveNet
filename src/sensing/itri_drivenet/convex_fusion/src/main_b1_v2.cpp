...
int main(int argc, char** argv)
{
  ...
  while (ros::ok())
  {
    ...
    size_t numberABB = g_object_front_bottom_60.size();

    CLUSTER_INFO* camera_ABB = new CLUSTER_INFO[numberABB];
    CLUSTER_INFO* camera_ABB_bbox = new CLUSTER_INFO[numberABB];

    size_t cnt = 0;
    for (size_t i = 0; i < g_object_front_bottom_60.size(); i++)
    {
      camera_ABB[i + cnt].min.x = g_object_front_bottom_60[i].bPoint.p0.x;
      camera_ABB[i + cnt].min.y = g_object_front_bottom_60[i].bPoint.p0.y;
      camera_ABB[i + cnt].min.z = g_object_front_bottom_60[i].bPoint.p0.z;
      camera_ABB[i + cnt].max.x = g_object_front_bottom_60[i].bPoint.p6.x;
      camera_ABB[i + cnt].max.y = g_object_front_bottom_60[i].bPoint.p6.y;
      camera_ABB[i + cnt].max.z = g_object_front_bottom_60[i].bPoint.p6.z;
      if (g_object_front_bottom_60[i].distance < 0)
      {
    ...
    cnt += g_object_front_bottom_60.size();
...
#pragma omp parallel for
  ...
  convexFusionB1V2.sendErrorCode(0x0006, g_frame_id, g_module_id);
