...
def img_callback(data): 
    ...
    global img_list
    img_list.append(data) #bridge.imgmsg_to_cv2(data, "bgr8")) 
...
def yolo_callback(data):
    global yolo_timestamp
    yolo_timestamp=data.header.stamp
    global keep_yolo_result
    keep_yolo_result.append(yolo_timestamp)
    global yolo_len
    yolo_len += 1
    res_list = []
    ...
    while not rospy.is_shutdown():
        if len(full_image_bbox_info)>0:
            ...
            if info1[2] == "YOLO":
                ...
                data_object_i = 0
                for res in info1[1]:
                    data_copy_temp[data_object_i].camInfo.u = res['box'][0]
                    data_copy_temp[data_object_i].camInfo.v = res['box'][1]
                    data_copy_temp[data_object_i].camInfo.width = res['box'][2]-res['box'][0]
                    data_copy_temp[data_object_i].camInfo.height = res['box'][3]-res['box'][1]
                    xyxy = res['box']
                    cls = res['category']
...
def detect():
    ...
    while not rospy.is_shutdown():
        if not first_yolo:
            if yolo_len > temp_yolo_len:
                if keep_yolo_result[0] in img_timestamp:
                    first_yolo = True
                    yolo_detected = True
                    yolo_index = img_timestamp.index(keep_yolo_result[0])
                    img1 = bridge.imgmsg_to_cv2(img_list[yolo_index], "bgr8")
                    res_list = []
                    ...
                    full_image_bbox_info.append([img1,res_list,"YOLO",keep_yolo_result[0],keep_data_object[0]])
                    img_timestamp = img_timestamp[yolo_index:]
                    img_list = img_list[yolo_index:]
                    frame_counter = 1
                    y_len +=1
                elif len(keep_yolo_result) >=1:
                    keep_yolo_result.pop(0)
                    keep_data_object.pop(0)
                temp_yolo_len = yolo_len    
        else:
            if len(img_timestamp)>yolo_optical_gap:
                if img_timestamp[yolo_optical_gap] not in yolo_every_gap_timestamp:
                    yolo_every_gap_timestamp.append(img_timestamp[yolo_optical_gap])

            #optical
            if (frame_counter % yolo_optical_gap !=0):
                if (frame_counter < len(img_list)):
                    #print('optical',frame_counter)
                    yolo_detected = False
                    img2 = bridge.imgmsg_to_cv2(img_list[frame_counter], "bgr8")
                    ...
                    frame_counter +=1
                    opt_len +=1   
                    #print('img_len',img_len,opt_len,y_len)
            else:
                #yolo
                if yolo_detected == False and len(yolo_every_gap_timestamp)>0 :
                    if (yolo_every_gap_timestamp[0] in keep_yolo_result):
                      if(len(img_list) > yolo_optical_gap):
                        yolo_index = keep_yolo_result.index(yolo_every_gap_timestamp[0])
                        print('nihao',keep_yolo_result[yolo_index]==img_timestamp[yolo_optical_gap])
                        keep_yolo_result = keep_yolo_result[yolo_index:]
                        keep_data_object = keep_data_object[yolo_index:]

                        yolo_detected = True
                        frame_counter = 0
                        img_list = img_list[yolo_optical_gap:]
                        img_timestamp = img_timestamp[yolo_optical_gap:]
                        

                        img1 = bridge.imgmsg_to_cv2(img_list[frame_counter], "bgr8")
                        res_list = []
                        for obj in keep_data_object[0]:
                            scale_bbox_info = np.array([int(obj.camInfo.u),\
                                                        int(obj.camInfo.v),\
                        ...
                        frame_counter += 1                        
                        y_len +=1
                        yolo_every_gap_timestamp.pop(0)

                    elif yolo_every_gap_timestamp[0]<keep_yolo_result[-1]:
                        print('hihihihihi',yolo_every_gap_timestamp[0])
                        yolo_every_gap_timestamp.pop(0)
                        img_list = img_list[yolo_optical_gap:]
                        img_timestamp = img_timestamp[yolo_optical_gap:]
