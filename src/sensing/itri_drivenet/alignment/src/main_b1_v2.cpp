...
int main(int argc, char** argv)
{
  ...
  if (!g_data_sync)
  {
    ...
    for (size_t cam_order = 0; cam_order < g_cam_ids.size(); cam_order++)
    {
      cam_subs[cam_order] = nh.subscribe(g_cam_topic_names[cam_order] + std::string("/relay"), 1, f_callbacks_cam[cam_order]);
  ...
  else
  {
    ...
    for (size_t cam_order = 0; cam_order < g_cam_ids.size(); cam_order++)
    {
      cam_filter_subs[cam_order].subscribe(nh, g_cam_topic_names[cam_order] + std::string("/relay"), 1);
