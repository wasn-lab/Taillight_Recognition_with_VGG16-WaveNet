...
void getSyncLidarCameraData()
{
  ...
  while (ros::ok())
  {
    if (!g_cam_times[0].empty() && !g_lidarall_times.empty() && !g_lidar_ssn_times.empty() && !g_is_data_sync)
    {
  ...
  while (ros::ok())
  {
    ...
    if (!g_data_sync || g_is_data_sync)
    {
      ...
      if (g_data_sync || is_data_ready)
      {
        ...
        getPointCloudInAllBoxFOV(object_arrs, cams_points_ptr, cams_bbox_points_ptr, cam_pixels, objects_2d_bbox_arrs,
