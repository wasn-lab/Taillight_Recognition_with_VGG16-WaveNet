...
void Yolo_app::input_preprocess(std::vector<cv::Mat*> &matSrcs){
    ...
    for(uint i = 0 ; i < batchSize; i++){
        ...
        cudaMemcpyAsync(yoloInput, tmp, inferYolo->getInputH()*inferYolo->getInputW()*3*sizeof(float), cudaMemcpyDeviceToDevice, inferYolo->m_CudaStream);
        cudaFree(tmp);
...
void Yolo_app::input_preprocess(std::vector<cv::Mat*> &matSrcs, std::vector<uint32_t> matId, int input_size, std::vector<int> dist_cols, std::vector<int> dist_rows){
    dsImgs_rows.clear();
    dsImgs_cols.clear();
...
void Yolo_app::get_yolo_result(std::vector<uint32_t>* order, std::vector<std::vector<ITRI_Bbox>* > &vbbx_output){
    ...
    {
        auto binfo = inferYolo->decodeDetections(imageIdx, dsImgs_rows.at(imageIdx),
                                                dsImgs_cols.at(imageIdx));
        auto remaining
            = nmsAllClasses(inferYolo->getNMSThresh(), binfo, inferYolo->getNumClasses());
