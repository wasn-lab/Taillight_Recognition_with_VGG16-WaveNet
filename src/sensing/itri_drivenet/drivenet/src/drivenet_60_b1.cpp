...
void* run_display(void*)
{
  ...
  while (ros::ok() && !g_is_infer_stop)
  {
    ...
    {
      if (g_mat60_0_display.cols * g_mat60_0_display.rows == g_img_size &&
          g_mat60_1_display.cols * g_mat60_1_display.rows == g_img_size &&
          g_mat60_2_display.cols * g_mat60_2_display.rows == g_img_size)
      {
        try
        {
          g_display_mutex.lock();
