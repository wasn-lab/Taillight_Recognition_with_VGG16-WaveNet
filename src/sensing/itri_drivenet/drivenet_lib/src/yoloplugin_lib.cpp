...
static void decodeBatchDetections(const YoloPluginCtx* ctx, std::vector<YoloPluginOutput*>& outputs)
{
  for (uint p = 0; p < ctx->batchSize; ++p)
  {
    ...
    std::vector<BBoxInfo> binfo =
        ctx->inferenceNetwork->decodeDetections(p, ctx->initParams.processingHeight, ctx->initParams.processingWidth);
...
static void dsPreProcessBatchInput(const std::vector<cv::Mat*>& cvmats, cv::Mat& batchBlob, const int& processingHeight,
                                   const int& processingWidth, const int& inputH, const int& inputW)
{
  ...
  batchBlob =
      cv::dnn::blobFromImages(batch_images, 1.0, cv::Size(inputW, inputH), cv::Scalar(0.0, 0.0, 0.0), false, false);
}
...
YoloPluginCtx* YoloPluginCtxInit(YoloPluginInitParams* initParams, size_t batchSize)
{
  ...
  {
    std::cerr << "WARNING: Batchsize set in config file overriden by pipeline. New batchsize is " << ctx->batchSize
              << std::endl;
  ...
  else
  {
    ...
    std::cerr << "Network Type has to be one among the following : yolov3 " << std::endl;
    return nullptr;
  }
...
std::vector<YoloPluginOutput*> YoloPluginProcess(YoloPluginCtx* ctx, std::vector<cv::Mat*>& cvmats)
{
  ...
  {
    preElapsed = ((preEnd.tv_sec - preStart.tv_sec) + (preEnd.tv_usec - preStart.tv_usec) / 1000000.0) * 1000;
    inferElapsed = ((inferEnd.tv_sec - inferStart.tv_sec) + (inferEnd.tv_usec - inferStart.tv_usec) / 1000000.0) * 1000;
    postElapsed = ((postEnd.tv_sec - postStart.tv_sec) + (postEnd.tv_usec - postStart.tv_usec) / 1000000.0) * 1000;
...
void YoloPluginCtxDeinit(YoloPluginCtx* ctx)
{
  if (ctx->inferParams.printPerfInfo)
  {
    ...
    std::cout << std::fixed << std::setprecision(4) << "PreProcess : " << ctx->preTime / ctx->imageCount
              ...
              << " ms Total : " << (ctx->preTime + ctx->postTime + ctx->inferTime) / ctx->imageCount << " ms per Image"
              << std::endl;
  }
