...
void Yolo::createYOLOEngine(const nvinfer1::DataType dataType, Int8EntropyCalibrator* calibrator)
{
  ...
  if (fileExists(m_EnginePath))
  {
    ...
    {
      std::cout << "Engine Compatibility Errors: Using incompatible gpu device, the program will be rebuild engine "
                   "file."
                << std::endl;
...
void Yolo::doInference(void* input, const uint batchSize)
{
  assert(batchSize <= m_BatchSize && "Image batch size exceeds TRT engines batch size");
  NV_CUDA_CHECK(cudaMemcpyAsync(m_DeviceBuffers.at(m_InputBindingIndex), input, batchSize * m_InputSize * sizeof(float),
