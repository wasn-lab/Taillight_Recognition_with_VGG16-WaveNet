...
std::vector<msgs::PointXYZ> openroadnet::calBoundary(std::vector<uint8_t> res, std::vector<std::vector <int> > &dis_free, int type)
{
...
// int openroadnet::read_distance_from_json(const std::string& filename, int arr_x[], int arr_y[], int arr_z[], const int rows, const int cols)
// {
...
//   {
//     if(i % (jdata.size()/10) == 0)std::cout << "[LOADING]: " << std::to_string((int)floor(i/(jdata.size()/10))) << "0 / 100" << std::endl;
...
msgs::FreeSpaceResult openroadnet::run(cv::Mat mat60)
{
  ...
  TF_Tensor *input_tensor = tf_utils::CreateTensor(TF_UINT8, 
                                                    ...
                                                    4, 
                                                    resized.ptr(), 
                                                    resized.size[0] * resized.size[1] * resized.channels() * sizeof(uint8_t));
  ...
  TF_SessionRun(sess,
                ...
                // input_op.data(), input_tensor.data(), input_tensor.size(), // Input tensors, input tensor values, number of inputs.                
  ...
  const auto label = static_cast<int *>(TF_TensorData(output_tensor));    // Return a pointer to the underlying data buffer of TF_Tensor*
