...
class WaveNetClassifier():
  def __init__(self, output_shape, input_shape = None, input_tensor = None, kernel_size = 2, dilation_depth = 9, n_filters = 512, task = 'classification', name='', regression_range = None, load=False, load_dir='./'):
    """
    Parameters:
    ...
    input_shape = tuple(self.input_tensor.shape[1:])
    self.expand_dims = False
  ...
  def residual_block(self, x, i):
    tanh_out = Conv1D(self.n_filters, 
                      ...
                      padding='causal', 
                      name=self.name+'dilated_conv_%d_tanh' % (self.kernel_size ** i), 
                      activation='tanh'
                      )(x)
    sigm_out = Conv1D(self.n_filters, 
                      ...
                      dilation_rate = self.kernel_size**i, 
                      padding='causal', 
                      name=self.name+'dilated_conv_%d_sigm' % (self.kernel_size ** i), 
                      activation='sigmoid'
                      )(x)
    z = Multiply(name=self.name+'gated_activation_%d' % (i))([tanh_out, sigm_out])
    skip = Conv1D(self.n_filters, 1, name=self.name+'skip_%d'%(i))(z)
    res = Add(name=self.name+'residual_block_%d' % (i))([skip, x])
    return res, skip
  ...
  def construct_model(self):  
    ...
    if self.input_tensor is None:
        x = Input(shape=self.input_shape, name=self.name+'original_input')
    else:
...
#     print(x)    
    ...
    if self.expand_dims == True:
        x_reshaped = Reshape(self.input_shape + (1,), name=self.name+'reshaped_input')(x)
    else:
        x_reshaped = x
    skip_connections = []
    out = Conv1D(self.n_filters, 2, dilation_rate=1, padding='causal', name=self.name+'dilated_conv_1')(x_reshaped)
    for i in range(1, self.dilation_depth + 1):
        out, skip = self.residual_block(out,i)
        skip_connections.append(skip)
    out = Add(name=self.name+'skip_connections')(skip_connections)
    out = Activation('relu')(out)
    out = Conv1D(self.n_filters, 4, strides = 1, padding='same', name=self.name+'conv_5ms', activation = 'relu')(out)
    out = AveragePooling1D(4, padding='same', name=self.name+'downsample_to_200Hz')(out)
    if self.time_distributed:
      # target_kernel_size = (int) (self.input_shape[0] / 80 / self.output_shape[0]) # prev_len / x = target_len => x = prev_len / target_len
        target_kernel_size = (int) (20 / 4 / 1)
        out = Conv1D(self.n_filters, target_kernel_size, padding='same', name = self.name+'conv_fit_to_target', activation='relu')(out)
        out = Conv1D(self.output_shape[1], target_kernel_size, padding='same', name=self.name+'conv_final')(out)
        out = AveragePooling1D(target_kernel_size, padding='same', name=self.name+'wave_final')(out)
        out = TimeDistributed(Activation(self.activation))(out)

    else:
        out = Conv1D(self.n_filters, 8, padding='same', activation='relu', name=self.name+'conv_500ms')(out)
        out = Conv1D((int)(self.n_filters/4), 8, padding='same', activation='relu', name=self.name+'conv_500ms_target_shape_4')(out)        
        out = AveragePooling1D(8, padding='same',name = self.name+'downsample_to_2Hz')(out)        
        out = Conv1D(self.output_shape, (int)(self.n_filters/32), padding='same', name=self.name+'final_conv')(out)       
#         out = GlobalAveragePooling1D(name=self.name+'globalavg_pooling')(out)
#         out = Activation(self.activation, name=self.name+'turnlight_output')(out)
        out = Flatten()(out)
        out = Activation(self.activation, name=self.name+'turnlight_output')(out)
#         out = GlobalAveragePooling1D(name=self.name+'turnlight_output')(out)
