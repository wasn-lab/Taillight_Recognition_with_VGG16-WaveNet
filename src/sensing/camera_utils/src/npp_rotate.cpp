...
{
  CHECK((rotation_degree == 90) || (rotation_degree == 180) || (rotation_degree == 270));
  CHECK(src.rows > 0);
  CHECK(src.cols > 0);
  ...
  CHECK(dst_npp8u_ptr);
  CHECK(num_dst_bytes > 0);

  cudaMemcpyAsync(src_npp8u_ptr, src.data, num_src_bytes, cudaMemcpyHostToDevice, cudaStreamPerThread);
  ...
  {
    LOG(ERROR) << "nppiRotate_8u_C3R returns " << result;
    assert(0);
  }
  ...
  cudaMemcpyAsync(dst.data, dst_npp8u_ptr, num_dst_bytes, cudaMemcpyDeviceToHost, cudaStreamPerThread);
