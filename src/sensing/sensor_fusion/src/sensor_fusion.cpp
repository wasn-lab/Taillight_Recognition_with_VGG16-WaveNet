...
struct _lux
{
	int *sky_up; int *sky_dw;
	int *road_ul; int *road_ucl; int *road_ucr; int *road_ur;
	int *road_dl; int *road_dcl; int *road_dcr; int *road_dr;
	int *alux;
	bool high;
};
...
void param_fusion(int** &det, int &ndet, lux_t lums, wea_t &stds, scanlimit_t scan, double scale, image_t &overmap, image_t &undrmap, double anchor_pt[2]);
...
wea_t weh;
	...
	double diff = 0, mean_diff = 0, diff_thr =0; int BAD = 0, GUT = 0; bool trigger = false;
...
width = (height * rw_width) / rw_height;
	...
	limit.x = width / 4; limit.y = height / 4; 
	limit.nRoad = (double)(height * width) / 16; limit.nSky = (double)(height* width) / 4;
	
	lux.high = false;
	lux.sky_up = (int*)malloc(sizeof(int) * fSize); lux.sky_dw = (int*)malloc(sizeof(int) * fSize); 
	lux.road_ul = (int*)malloc(sizeof(int) * fSize); lux.road_ucl = (int*)malloc(sizeof(int) * fSize); 
	lux.road_ucr = (int*)malloc(sizeof(int) * fSize); lux.road_ur = (int*)malloc(sizeof(int) * fSize);
	lux.road_dl = (int*)malloc(sizeof(int) * fSize); lux.road_dcl = (int*)malloc(sizeof(int) * fSize); 
	lux.road_dcr = (int*)malloc(sizeof(int) * fSize); lux.road_dr = (int*)malloc(sizeof(int) * fSize);
	lux.alux = (int*) malloc(sizeof(int) * 10);
	for (int z = 0; z < 10; z++){ lux.alux[z] = 0; }
...
/**************************************************************************/
    ...
    image_transport::Subscriber sub1_0 = it.subscribe("/gmsl_camera/port_a/cam_1/image_raw", 2, imageCallback,ros::VoidPtr(),image_transport::TransportHints("compressed"));
...
/************************************************************************/ 
     ...
     for (int j = 0; j < Lidar_num_cb; j++) 
       ...
       transform_coordinate_main(msgLidarObj.objects[j].cPoint,lidar_coordinate_offset_X, lidar_coordinate_offset_Y, lidar_coordinate_offset_Z);
...
/************************************************************************/
       ...
       transform_coordinate_main(msgRadObj.objects[j].cPoint,radar_coordinate_offset_X, radar_coordinate_offset_Y, radar_coordinate_offset_Z);
...
/************************************************************************/
       ...
       transform_coordinate_main(msgCam60_1_Obj.objects[j].cPoint,cam_coordinate_offset_X, cam_coordinate_offset_Y, cam_coordinate_offset_Z);
...
/************************************************************************/ 
     ...
     for (int j = 0; j < Cam30_1_num_cb ; j++) 
       ...
       transform_coordinate_main(msgCam30_1_Obj.objects[j].cPoint,cam_coordinate_offset_X, cam_coordinate_offset_Y, cam_coordinate_offset_Z);
        // ======>
...
/************************************************************************/ 
     ...
     for (int j = 0; j < Cam120_1_num_cb ; j++) 
       ...
       transform_coordinate_main(msgCam120_1_Obj.objects[j].cPoint,cam_coordinate_offset_X, cam_coordinate_offset_Y, cam_coordinate_offset_Z);
        // ======>
...
printf("put Lidar object to different view(total,60_0,60_1,60_2,30_1,rear) :%d,%ld,%ld,%ld,%ld\n", Lidar_num,msgLidar_60_0_Obj.objects.size(),msgLidar_60_1_Obj.objects.size(),msgLidar_60_2_Obj.objects.size(),msgLidar_rear_Obj.objects.size());

printf("put Lidar object to different view(30_0,30_1,30_2) :%ld,%ld,%ld\n",msgLidar_30_0_Obj.objects.size(),msgLidar_30_1_Obj.objects.size(),msgLidar_30_2_Obj.objects.size());

printf("put Lidar object to different view(120_0,120_1,120_2) :%ld,%ld,%ld\n",msgLidar_120_0_Obj.objects.size(),msgLidar_120_1_Obj.objects.size(),msgLidar_120_2_Obj.objects.size());
/************************************************************************/
    ...
    cam_det[3][i] = Cam60_0_uv[i][3];
      ...
      overlap_analysis(cam_det, Cam60_0_num);  
			...
			for (  j = 0; j < Cam60_0_num; j++)
				printf("1:cam60_0_Detection cam_det[][%d]  %d %d %d %d %d\n", j,cam_det[0][j], cam_det[1][j], cam_det[2][j], cam_det[3][j],cam_det[4][j]);
   ...
   Lidar_num = msgLidar_60_0_Obj.objects.size();
      ...
      overlap_analysis(lid_det, Lidar_num);  
			...
			for (  j = 0; j < Lidar_num; j++)
				printf("0:LidarDetection lid_det[][%d]  %d %d %d %d %d\n", j,lid_det[0][j], lid_det[1][j], lid_det[2][j], lid_det[3][j],lid_det[4][j]);
...
param_fusion(bb_det, total_det, lux, weh, limit, factor, ovImage, uvImage, ref_point);
			...
			for (int j = 0; j < total_det; j++)
				printf("Cam60_0 view bb_det[][%d]  %d %d %d %d %d %d\n", j,bb_det[0][j], bb_det[1][j], bb_det[2][j], bb_det[3][j], bb_det[4][j], bb_det[5][j]);
	  ...
	  for(j=0;j< vDetectedObjectCAM_60_1.size();j++)
	    {
	      ...
	      q6y = vDetectedObjectCAM_60_1[j].bPoint.p6.y; //max
             ...
             height_AB = std::max(p6y, q6y) - std::min(p0y, q0y);
              ...
              if((diff < BB2BB_distance) 
                 || (width_AB < width_A + width_B && height_AB < height_A + height_B))
               {
               check3D[i][j]=1;
    ...
    for (  i = 0; i < msgRadObj.objects.size(); i++)
    {
      ...
      //overlap_analysis(radar_det, radar_num);  
			...
			for (  j = 0; j < radar_num; j++)
				printf("1:RadarDetection radar_det[][%d]  %d %d %d %d %d\n", j,radar_det[0][j], radar_det[1][j], radar_det[2][j], radar_det[3][j],radar_det[4][j]);
...
double ** assoc;
	...
	int overlap_x, overlap_y, overlap, intersect, uni_area; double IOU;
	for (int j = 0; j < total_det; j++){
...
//printf("IOU = %f\n",IOU);
	...
	double bestnIOU; int bestnBB;
	for (int j = 0; j < total_det; j++){
		for (int i = 0; i < total_det; i++){	
			if (assoc[j][i] > 0){
				assoc[i][j] = 0;
				bestnIOU = assoc[j][i]; bestnBB = i;
				for (int k = 0; k < total_det; k++){
					IOU = assoc[i][k];
					if (IOU > 0){
						if (bestnIOU < IOU){
							assoc[bestnBB][k] = 0; assoc[k][bestnBB] = 0;
							bestnIOU = IOU; bestnBB = k;
						}
						else{
							assoc[i][k] = 0; assoc[k][i] = 0;
						}		
					}
					else{
						//do nothing
	...
	int ncount;int mem1_, mem2_; bool toggle = false;
	for (int j = 0; j < total_det; j++){
		ncount = 0; mem1_ = bb_det[4][j]; toggle = false;
		if (mem1_ == 0){
			for (int i = 0; i < total_det; i++){
				mem2_ = bb_det[4][i];
				if (mem2_ == 0){
	...
	for (int j = 0; j < ndet; j++){
		...
		for (int i = 0; i < ndet; i++){
			if (assoc[j][i] > 0.150){
				...
				int startx_b = det[0][i], starty_b = det[1][i];
				int endx_b = startx_b + det[2][i], endy_b = starty_b + det[3][i];
				det[0][j] = (startx_a + startx_b) / 2;             det[0][i] = 0;
				det[1][j] = (starty_a + starty_b) / 2;             det[1][i] = 0;
				det[2][j] = ((endx_a + endx_b) / 2) - det[0][j];   det[2][i] = 0;
				det[3][j] = ((endy_a + endy_b) / 2) - det[1][j];   det[3][i] = 0;
...
//the function of param_fusion is to determine any independent bounding boxes, whether it is just a false positive or an actual true positive
void param_fusion(int** &det, int &ndet, lux_t lums, wea_t &stds, scanlimit_t scan, double scale, image_t &overmap, image_t &undrmap, double anchor_pt[2])
{
	int m, m2, mem_;
	...
	for (int j = 0; j < ndet; j++){		
		...
		if (mem_ == 0){
			...
			if (stds.norm == true){ weaconf[0] = 1.0; weaconf[1] = 1.0; }
			else { weaconf[0] = 0.7; weaconf[1] = 0.9; }
			//Calculate the lux-related confidences		
			if (lums.high == true){ time_w = 0.6; } else { time_w = 0.4; }
			...
			for (int y_scan = ys; y_scan < ye; y_scan++){
				if (y_scan < (scan.y * 2)){
					for (int x_scan = xs; x_scan < xe; x_scan++){
						over += (double)overmap.pixel[x_scan + (y_scan * overmap.width)]; tempC++;
						if (det[4][j] == 0){ undr += (double)undrmap.pixel[x_scan + (y_scan * undrmap.width)]; }
						else { continue; }
					}
				}
				else {
					if (y_scan < (scan.y * 3)){ m = 0; }
					else { m = 4; }
					for (int x_scan = xs; x_scan < xe; x_scan++){
						if (x_scan < (scan.x)){ tempA += lums.alux[m + 2]; }
						else if (x_scan < (scan.x * 2)){ tempA += lums.alux[3 + m]; }
						else if (x_scan < (scan.x * 3)){ tempA += lums.alux[4 + m]; }
						else{ tempA += lums.alux[5 + m]; } tempB++;
			...
			tempC = anchor_pt[0] - xc; tempB = anchor_pt[1] - yc;
			m = det[4][j];
printf("det[4][j] == %d\n",det[4][j]);
			...
			if (m == 0){
printf("m == 0\n");
				luxconf = tempA;  expconf = 1 - ((time_w * (over / temp)) + ((1 - time_w) * (undr / temp)));
				rotconf = 1 - ((acos(tempB / sqrt((tempC * tempC) + (tempB * tempB))) * const_) * 1.5);
				c_luxconf = 0.1; c_expconf = 0.7; // 
				...
				luxconf = 0.8; expconf = 0.8; // 
				rotconf = 1 - ((acos(tempB / sqrt((tempC * tempC) + (tempB * tempB))) * const_) * 4);
				c_luxconf = tempA;  c_expconf = 1 - ((time_w * (over / temp)) + ((1 - time_w) * (undr / temp)));
...
void expmeter(image_t lummap, image_t satmap, image_t &overmap, image_t &undrmap)
{
	...
	for (int j = 0; j < lummap.height; j++)
	{
		for (int i = 0; i < lummap.width; i++)
		{
			lux_elem = (double)lummap.pixel[i + (j * lummap.width)] / 255; ilux_elem = 1 - lux_elem;
			sat_elem = (double)satmap.pixel[i + (j * satmap.width)] / 255; isat_elem = 1 - sat_elem;
