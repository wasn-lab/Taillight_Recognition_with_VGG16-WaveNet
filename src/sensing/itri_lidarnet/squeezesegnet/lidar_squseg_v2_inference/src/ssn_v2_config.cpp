...
void norm_mean(float* mean_ptr, const string& data_set, char ViewType, float phi_center)
{
  ...
  {
    float INPUT_MEAN[4][5] = { { 0.04, -0.99, 0.08, 2.53, 1.12 },
                               { 1.73, 0.08, 0.08, 2.79, 1.95 },
                               { -0.02, 1.84, 0.12, 4.64, 2.08 },
                               { -1.79, 0.19, 0.11, 2.68, 2.00 } };
...
void norm_std(float* std_ptr, const string& data_set, char ViewType, float phi_center)
{
  ...
  {
    float INPUT_STD[4][5] = { { 2.03, 3.86, 0.56, 10.09, 4.38 },
                              { 5.24, 2.56, 0.77, 9.56, 5.81 },
                              { 2.67, 5.05, 0.73, 13.54, 5.69 },
                              { 6.18, 2.81, 0.77, 10.18, 6.79 } };
...
vector<float> phi_center_grid(char ViewType)
{
  ...
  switch (ViewType)
  {
    case 'X':
    {
      phi_center_all = { 0, 90, 180, -90 };
      break;
    }
    case 'T':
    {
      phi_center_all = { 0, 135, -135 };
      break;
    }
    default:
    {
...
TF_inference::TF_inference(string input_data_set, char input_ViewType, float input_phi_center, int input_pub_type)
{
...
int TF_inference::TF_init()
{
  // =============== Tensorflow ==================
  // ======== load graph ========
  string pb_dir = (ros::package::getPath("lidar_squseg_v2_inference") + "/model/SqueezeSegNet/" + data_set + "/" +
                   phi_center_name + ".pb");

  if (!(BFS::exists(pb_dir)))
  {
    pb_dir = (ros::package::getPath("lidar_squseg_v2_inference") + "/model/SqueezeSegNet/" + "hino1" + "/" +
              phi_center_name + ".pb");
  }
  ...
  // ======== get input ops ========
  input_ops = { { TF_GraphOperationByName(graph, "keep_prob"), 0 },
  ...
  // ======== get output ops ========
  out_op = { TF_GraphOperationByName(graph, "interpret_output/pred_cls"), 0 };
...
void TF_inference::TF_run(VPointCloud::Ptr release_Cloud, VPointCloudXYZIL::Ptr result_cloud)
{
  stopWatch.reset();

  VPointCloudXYZID filtered_cloud =
      sph_proj(release_Cloud, phi_center, SPAN_PARA[0], SPAN_PARA[1], theta_UPbound, theta_range);
  ...
  std::vector<TF_Tensor*> input_tensors = {
    tf_utils::CreateTensor(TF_FLOAT, prob_dims.data(), prob_dims.size(), TFprob.data(), TFprob.size() * sizeof(float)),
    tf_utils::CreateTensor(TF_FLOAT, num_dims.data(), num_dims.size(), spR_TFnum.data(),
                           spR_TFnum.size() * sizeof(float)),
    tf_utils::CreateTensor(TF_FLOAT, mask_dims.data(), mask_dims.size(), spR_TFmask.data(),
                           spR_TFmask.size() * sizeof(float))
  };
  ...
  TF_SessionRun(sess, nullptr,  // Run options.
                ...
                input_tensors.size(),        // Input tensors, input tensor values, number of inputs.
  ...
  const auto label = static_cast<long int*>(TF_TensorData(output_tensor));  // Return a pointer to the underlying data
                                                                            // buffer of TF_Tensor*
