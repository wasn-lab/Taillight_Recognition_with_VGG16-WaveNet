...
void callback_LidFrontTop(const msgs::PointCloud::ConstPtr& msg)
{
...
#pragma omp parallel for
  for (size_t i = 0; i < msg->pointCloud.size(); ++i)
  {
    ...
    // cloudPtr_LidFrontTop->points[i].intensity = (int) msg->pointCloud[i].intensity;  //(int)
    // msg->pointCloud[i].intensity
...
void callback_LidFrontLeft(const msgs::PointCloud::ConstPtr& msg)
{
...
#pragma omp parallel for
  for (size_t i = 0; i < msg->pointCloud.size(); ++i)
  {
    ...
    // cloudPtr_LidFrontLeft->points[i].intensity = (int) msg->pointCloud[i].intensity;  //(int)
    // msg->pointCloud[i].intensity
...
void callback_LidFrontRight(const msgs::PointCloud::ConstPtr& msg)
{
...
#pragma omp parallel for
  for (size_t i = 0; i < msg->pointCloud.size(); ++i)
  {
    ...
    // cloudPtr_LidFrontRight->points[i].intensity = (int) msg->pointCloud[i].intensity;  //(int)
    // msg->pointCloud[i].intensity
...
void callback_LidRearLeft(const msgs::PointCloud::ConstPtr& msg)
{
...
#pragma omp parallel for
  for (size_t i = 0; i < msg->pointCloud.size(); ++i)
  {
    ...
    // cloudPtr_LidRearLeft->points[i].intensity = (int) msg->pointCloud[i].intensity;  //(int)
    // msg->pointCloud[i].intensity
...
void callback_LidRearRight(const msgs::PointCloud::ConstPtr& msg)
{
...
#pragma omp parallel for
  for (size_t i = 0; i < msg->pointCloud.size(); ++i)
  {
    ...
    // cloudPtr_LidRearRight->points[i].intensity = (int) msg->pointCloud[i].intensity;  //(int)
    // msg->pointCloud[i].intensity
...
public:
  ...
  void initial(boost::shared_ptr<pcl::visualization::PCLVisualizer> input_viewer, int* input_viewID)
  {
    ...
    // ToCameraMiddles = LidarToCamera (input_viewer, input_viewID, 1250, -4.9 * D2R, -2.1 * D2R, -0.3 * D2R, -60, -0.5,
    // -1340, 1280, 720); UDPclient.initial (GlobalVariable::UI_UDP_IP, GlobalVariable::UI_UDP_Port);
  }

  void setInputCloud(PointCloud<PointXYZI>::Ptr input)
  {
    release_cloud = input;
  }
  ...
  void update()
  {
    ...
    //      cout << "[remove_plane]:" << ptr_cur_cloud->size () << "," << timer_algorithm_running.getTimeSeconds () <<
    //      "s" << endl;
    //
    ...
    //      cout << "[remove ground SCM ]:" << ptr_cur_cloud->size () << "," << timer_algorithm_running.getTimeSeconds
    //      () << "s" << endl;
    //
    //      pcl::PointIndicesPtr indices_ground (new pcl::PointIndices);
    ...
    //      PointCloud<PointXYZ>::Ptr cloud_ground (new PointCloud<PointXYZ>);
    //      extract_Indices (ptr_cur_cloud, indices_ground, *cloud_ground, *ptr_cur_cloud);
    //      cout << "[remove ground SAC ]:" << ptr_cur_cloud->size () << "," << timer_algorithm_running.getTimeSeconds
    //      () << "s" << endl;
    //
    //      vector<PointCloud<PointXYZ>> roi_cloud;
    //      roi_cloud = separate_cloud (ptr_cur_cloud, -GlobalVariable::UI_LATERAL_RANGE,
    //      GlobalVariable::UI_LATERAL_RANGE, 0, 50, -5, -2.2); PointCloud<PointXYZ>::Ptr cloud_roi_inside (new
    //      PointCloud<PointXYZ>); PointCloud<PointXYZ>::Ptr cloud_roi_outside (new PointCloud<PointXYZ>);
    ...
    //      cout << "[down sampling  ]:" << ptr_cur_cloud->size () << "," << timer_algorithm_running.getTimeSeconds ()
    //      << "s" << endl;
    ...
    *ptr_cur_cloud = VoxelGrid_CUDA().compute(ptr_cur_cloud, 0.2);
    // cout << "[voxel_grid     ]:" << ptr_cur_cloud->size () << "," << timer_algorithm_running.getTimeSeconds () << "s"
    // << endl;
    ...
    *ptr_cur_cloud = VoxelFilter_CUDA().compute(cloud_non_ground, 1, 1);
    ...
    // cout << "[remove outlier]:" << ptr_cur_cloud->size () << "," << timer_algorithm_running.getTimeSeconds () << "s"
    // << endl;
    ...
    *cloud_ground = CuboidFilter().pass_through_soild<PointXYZ>(cloud_ground, 0.65, 5, -2, 2, -5, 1);
    ...
    CLUSTER_INFO* cur_cluster =
        S1cluster.getClusters(GlobalVariable::ENABLE_DEBUG_MODE, ptr_cur_cloud, &cur_cluster_num);
...
void detection(int argc, char** argv)
{
  ...
  RosModuleA::RegisterCallBackLidar(callback_LidFrontTop, callback_LidFrontLeft, callback_LidFrontRight,
                                    callback_LidRearLeft, callback_LidRearRight);
  ...
  {
    pcl::visualization::PointCloudColorHandlerGenericField<PointXYZI> handler_OD("intensity");
    viewer = boost::shared_ptr<pcl::visualization::PCLVisualizer>(new pcl::visualization::PCLVisualizer("PCL HDL "
                                                                                                        "Cloud"));
  ...
  while (ros::ok())
  {
    ...
    GlobalVariable::ROS_TIMESTAMP =
        max(max(max(max(lidar_time[0], lidar_time[1]), lidar_time[2]), lidar_time[3]), lidar_time[4]);
