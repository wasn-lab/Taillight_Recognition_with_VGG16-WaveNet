...
S2Track::S2Track (boost::shared_ptr<pcl::visualization::PCLVisualizer> input_viewer,
                  int *input_viewID) :
    viewer (input_viewer),
    viewID (input_viewID)
{
  ...
  for (int i = 0; i < cluster_size; i++)  // i = cur
  {
    ...
    for (size_t j = 0; j < pre_vehicle_table.size (); j++)  // j = pre
    {
      ...
      if (pre_vehicle_table.at (j).found_num > 0)  // if it has pre data
      {
        ...
        float dif_spd = fabs (
            cur_spd - geometry::distance (PointXYZ (pre_vehicle_table.at (j).velocity.x, pre_vehicle_table.at (j).velocity.y, 0), PointXYZ (0, 0, 0)));
        ...
        if (dif_dis > 0.5 && dif_dis < 10 && dif_vol < 3 && dif_num < 100 && dif_spd < 30 && fabs (cos_theta) < 0.5)
        {
          ...
          float dif_pdt = geometry::distance (cluster_info[i].center, pre_vehicle_table.at (j).predict_next_center);  //the difference of cur center and pre predict_next_center
    ...
    if (the_best_index >= 0)
    {
      if (is_debug)
      {
        viewer->addLine (cluster_info[i].center, pre_vehicle_table.at (the_best_index).center, 0.0, 1.0, 0.0, to_string (*viewID));
      ...
      cluster_info[i].velocity.x = (cluster_info[i].center.x - pre_vehicle_table.at (the_best_index).center.x) / frame_time.getTimeSeconds ();
      cluster_info[i].velocity.y = (cluster_info[i].center.y - pre_vehicle_table.at (the_best_index).center.y) / frame_time.getTimeSeconds ();
      cluster_info[i].velocity.z = (cluster_info[i].center.z - pre_vehicle_table.at (the_best_index).center.z) / frame_time.getTimeSeconds ();
      cluster_info[i].predict_next_center.x = cluster_info[i].center.x + pre_vehicle_table.at (the_best_index).velocity.x;
      cluster_info[i].predict_next_center.y = cluster_info[i].center.y + pre_vehicle_table.at (the_best_index).velocity.y;
      cluster_info[i].predict_next_center.z = cluster_info[i].center.z + pre_vehicle_table.at (the_best_index).velocity.z;
    ...
    if (is_debug)
    {
      viewer->addText3D (to_string (cluster_info[i].tracking_id), cluster_info[i].center, 1, 1, 1, 1, to_string (*viewID), 0);
      ++*viewID;
      ...
      //viewer->addText3D (to_string ((int) (cluster_info[i].tracking_id)) + "id", cluster_info[i].max, 0.7, 255, 255, 255, to_string (*viewID));
      ++*viewID;
    }
  }
