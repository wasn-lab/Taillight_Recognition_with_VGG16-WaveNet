...
void S1Cluster::initial(boost::shared_ptr<pcl::visualization::PCLVisualizer> input_viewer, int* input_viewID)
{
  viewer = input_viewer;
  viewID = input_viewID;
  ...
  dbscan.setEpsilon(0.80, 0.80, 0.80, 0.80, 0.80);
  dbscan.setMinpts(3, 3, 3, 3, 3);
}
  ...
  for (size_t i = 0; i < raw_cluster.size(); i++)
  {
    if (cluster_vector.at(i).cluster_tag == 1)
    {
      for (size_t j = 0; j < cluster_vector.size(); j++)
      {
        ...
        {
          if (cluster_vector.at(j).max.x < cluster_vector.at(i).max.x &&
              cluster_vector.at(j).max.y < cluster_vector.at(i).max.y &&
              cluster_vector.at(j).max.z < cluster_vector.at(i).max.z &&
              cluster_vector.at(j).min.x > cluster_vector.at(i).min.x &&
              cluster_vector.at(j).min.y > cluster_vector.at(i).min.y &&
              cluster_vector.at(j).min.z > cluster_vector.at(i).min.z)
          {
...
#pragma omp parallel for
  for (size_t i = 0; i < raw_cluster.size(); i++)
  {
    if (cluster_vector.at(i).cluster_tag == 1)
    {
      ...
      cluster_vector.at(i).angle_from_x_axis =
          pcl::getAngle3D(Eigen::Vector3f(cluster_vector.at(i).center.x, cluster_vector.at(i).center.y, 0),
...
#pragma omp parallel for
  for (size_t i = 0; i < raw_cluster.size(); i++)
  {
    ...
    {
      /*
       float max2plane = pointToPlaneDistance (cluster_vector.at (i).max, plane_coef.values[0], plane_coef.values[1],
       plane_coef.values[2], plane_coef.values[3]); float min2plane = pointToPlaneDistance (cluster_vector.at (i).min,
       plane_coef.values[0], plane_coef.values[1], plane_coef.values[2], plane_coef.values[3]);*/
  ...
  if (debug)
  {
    ...
    for (size_t i = 0; i < raw_cluster.size(); i++)
    {
      if (cluster_vector.at(i).cluster_tag > 0)
      {
        ...
        // viewer->addText3D (to_string ((int)(cluster_vector.at (i).cloud.size()/cluster_vector.at (i).hull_vol)) ,
        // cluster_vector.at (i).max, 0.7, 255, 255, 255, to_string (*viewID));

        // viewer->addText3D (to_string (int (cluster_vector.at (i).GRSD21[10])) + "", cluster_vector.at (i).max, 0.7,
        // 255, 255, 255, to_string (*viewID));

        // see distance
        // viewer->addText3D (to_string ((cluster_vector.at (i).dis_center_origin)) + "m", cluster_vector.at (i).max,
        // 0.7, 255, 255, 255,to_string (*viewID)); viewer->addText3D (to_string ((int) (cluster_vector.at (i).min.x)) +
        // "x," + to_string ((int) (cluster_vector.at (i).min.y)) + "y",cluster_vector.at (i).max, 0.7, 255, 255, 255,
        // to_string (*viewID)); viewer->addText3D (to_string ((int)(cluster_vector.at (i).dz)) + "dz," + to_string
        // ((int) (cluster_vector.at (i).dis_max_min)), cluster_vector.at (i).max, 0.7, 255, 255, 255,to_string
        // (*viewID));

        // see confidence
        // viewer->addText3D (to_string (cluster_vector.at (i).tag_confidence), cluster_vector.at (i).max, 0.5, 255,
        // 255, 255, to_string (*viewID), 0);

        // see size
        // viewer->addText3D (to_string (cluster_vector.at (i).cloud.size()),cluster_vector.at (i).max, 0.5, 255, 255,
        // 255, to_string (*viewID), 0);

        // see distance and cloud size
        /*
         viewer->addText3D ("D" + to_string ((int) (cluster_vector.at (i).dis_center_origin)) + "S" + to_string ((int)
         cluster_vector.at (i).cloud.size ()), cluster_vector.at (i).max, 0.5, 255, 255, 255, to_string (*viewID), 0);
         */

        // see legth, width, high, dis_max_min
        /*
         viewer->addText3D (
         to_string ((int) cluster_vector.at (i).dx) + " " + to_string ((int) cluster_vector.at (i).dy) + " " + to_string
         ((int) cluster_vector.at (i).dz) + " " + to_string (cluster_vector.at (i).dis_max_min), cluster_vector.at
         (i).max  , 0.5, 255, 255, 255, to_string (*viewID), 0);
         */

        // see Polygon Area
        /*
         viewer->addText3D (to_string (pcl::calculatePolygonArea (cluster_vector.at(i).cloud)), cluster_vector.at
         (i).max, 1, 255, 255, 255, to_string (*viewID), 0); viewer->setShapeRenderingProperties
         (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, to_string (*viewID));
         */
         ...
         *ptr_cur_cloud = cluster_vector.at (i).cloud;
         ...
         viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_COLOR, ((double) rand () /
         (RAND_MAX)), ((double) rand () / (RAND_MAX)),
         ((double) rand () / (RAND_MAX)), to_string (*viewID));
         viewer->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, to_string
         (*viewID));*/
