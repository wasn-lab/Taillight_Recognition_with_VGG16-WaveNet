...
void SvmWrapper::initialize(string input_name)
{
  ...
  if (SVMtrain.loadProblem(("train_" + type_name + ".dat").c_str()))
  {
    ...
    SVMparam.gamma = 5;
    SVMparam.C = 10;
    SVMparam.probability =
        1;  // 0 off, 1 on. Using probability parameter will always results in high number of unclassified sample
    ...
    if (SVMtrain.trainClassifier())
    {
      ...
      {
        cout << "[SVM]   Probability support and active --> "
             << "ProbA = " << *SVMmodel.probA << "  "
             << "ProbB = " << *SVMmodel.probB << endl;
      }
      else
      {
...
bool SvmWrapper::calculate(CLUSTER_INFO* single_cluster_info)
{
  if (!is_loaded)
  {
    ...
    if (SVMclassify.classification())
    {
      ...
      if (SVMclassify.hasLabelledTrainingSet())
      {
        ...
        if (SVMclassify.classificationTest())
        {
          ...
          // svm_test_report.correctPredictionsIdx, svm_test_report.totalSamples);
        }
        else
        {
...
void SvmWrapper::labelingTool(CLUSTER_INFO* single_cluster_info,
                              boost::shared_ptr<pcl::visualization::PCLVisualizer> input_viewer, int* input_viewID)
{
  ...
  *cloud_training = single_cluster_info->cloud;
  ...
  input_viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 5,
                                                 to_string(*input_viewID), 0);
...
void SvmWrapper::keyboard_event_occurred(const pcl::visualization::KeyboardEvent& event, void* viewer_void)
{
  if ((event.getKeySym() == "y" || event.getKeySym() == "n" || event.getKeySym() == "m") && event.keyDown() &&
      cloud_training->size() > 0)
  {
