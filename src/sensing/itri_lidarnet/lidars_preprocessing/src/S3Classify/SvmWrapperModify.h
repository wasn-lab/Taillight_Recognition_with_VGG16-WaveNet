...
namespace pcl
{
  ...
  struct SVMData
  {
      ...
      SVMData () :
          label (std::numeric_limits<double>::signaling_NaN ())
      {
      }
  };
  ...
  class PCL_EXPORTS SVM
  {
    protected:
      ...
      void
      adaptInputToLibSVM (std::vector<SVMData> training_set,
                          svm_problem &prob);

      /*      * \brief Convert the libSVM format (svm_problem) into a easier output format.*/
      void
      adaptLibSVMToInput (std::vector<SVMData> &training_set,
                          svm_problem prob);
  ...
  class PCL_EXPORTS SVMTrain : public SVM
  {
    protected:
       ...
       */
      void
      scaleFactors (std::vector<SVMData> training_set,
                    svm_scaling &scaling);
  ...
  class PCL_EXPORTS SVMClassify : public SVM
  {
    protected:
      ...
      void
      scaleProblem (svm_problem &input,
                    svm_scaling scaling);

    public:
      /*      * \brief Constructor.*/
      SVMClassify () :
          model_extern_copied_ (0),
       ...
       */
      bool
      loadClassProblemITRI (string input)
      {
        assert(model_.l != 0);
