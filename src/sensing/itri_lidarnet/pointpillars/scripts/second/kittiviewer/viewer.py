...
class KittiViewer(QMainWindow):
    ...
    def on_loadDetPressed(self):
        ...
        self.info(f"{self.w_det_path.text()}")
        if Path(det_path).is_file():
            print(f"{det_path} can be read.")
            with open(det_path, "rb") as f:
                dt_annos = pickle.load(f)
        else:
            self.info(f"{det_path} cannot be read.")
        ...
        print(f"load {len(dt_annos)} detections.")
