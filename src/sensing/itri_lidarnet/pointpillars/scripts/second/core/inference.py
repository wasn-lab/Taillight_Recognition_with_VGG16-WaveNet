...
class InferenceContext:
    ...
    def get_inference_input_dict_without_kitti(self, time, points):
        assert self.anchor_cache is not None
        assert self.target_assigner is not None
        assert self.voxel_generator is not None
        assert self.config is not None
        assert self.built is True
        rect = np.zeros([3, 4], dtype=np.float32)
        P2 = np.zeros([3, 3], dtype=np.float32)
        Trv2c = np.zeros([3, 4], dtype=np.float32)
        input_cfg = self.config.eval_input_reader
        model_cfg = self.config.model.second
        ...
        input_dict = {
            ...
            'Trv2c': Trv2c,
            'P2': P2,
            'image_shape': np.zeros([2], dtype=np.int32),
            'image_idx': time,
            'image_path': "",
            # 'pointcloud_num_features': num_point_features,
        }
    ...
    @abc.abstractclassmethod
    def _inference_lidar(self, example):
        raise NotImplementedError()

    def inference_lidar(self, example):
        return self._inference_lidar(example)
