...
#ifndef POINTS_PILLAR_ROS_PED_CYC_H
#define POINTS_PILLAR_ROS_PED_CYC_H
...
private:
  ...
  std::unique_ptr<PointPillars_Ped_Cyc> point_pillars_ptr_;
  ...
  */
  void pubDetectedObject(const std::vector<float>& detections, const std::vector<float>& scores, const std::vector<int>& labels, const std_msgs::Header& in_header);

public:
  PointPillarsROS_Ped_Cyc();
