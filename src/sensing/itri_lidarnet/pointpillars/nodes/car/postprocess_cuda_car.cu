                              ...
                              const int NUM_BOX_CORNERS, const int NUM_OUTPUT_BOX_FEATURE)
{
  ...
  if(anchor_mask[tid] == 1 && score > score_threshold && box_pz > -3.0f)
  {
    ...
    box_pz = box_preds[tid*NUM_OUTPUT_BOX_FEATURE + 2] * dev_anchors_dz[tid] + za;
    ...
    box_dz = expf(box_preds[tid*NUM_OUTPUT_BOX_FEATURE + 5]) * dev_anchors_dz[tid];
