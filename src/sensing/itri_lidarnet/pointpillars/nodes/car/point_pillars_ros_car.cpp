...
pcl::StopWatch g_stopWatch_car;

PointPillarsROS_Car::PointPillarsROS_Car()
  : private_nh_("~")
  ...
  , BASELINK_FRAME_("base_link")
{
  ...
  point_pillars_ptr_.reset(new PointPillars_Car(reproduce_result_mode_, score_threshold_, nms_overlap_threshold_,
                                            pfe_onnx_file_, rpn_onnx_file_));
}

void PointPillarsROS_Car::createROSPubSub()
{
  sub_points_ = nh_.subscribe<sensor_msgs::PointCloud2>("/points_raw", 1, &PointPillarsROS_Car::pointsCallback, this);
  pub_objects_ = nh_.advertise<msgs::DetectedObjectArray>("/LidarDetection/Car", 1);
}

geometry_msgs::Pose PointPillarsROS_Car::getTransformedPose(const geometry_msgs::Pose& in_pose, const tf::Transform& tf)
{
...
void PointPillarsROS_Car::pubDetectedObject(const std::vector<float>& detections, const std::vector<float>& scores, const std_msgs::Header& in_header)
{
  ...
  if (num_objects > 0)
  {
    ...
    for (size_t i = 0; i < num_objects; i++)
    {
      ...
      float dimension_z = detections[i * OUTPUT_NUM_BOX_FEATURE_ + 5]; //4
      float dimension_y = detections[i * OUTPUT_NUM_BOX_FEATURE_ + 4]; //3
      float dimension_x = detections[i * OUTPUT_NUM_BOX_FEATURE_ + 3]; //5
...
void PointPillarsROS_Car::analyzeTFInfo(tf::StampedTransform baselink2lidar)
{
...
void PointPillarsROS_Car::pclToArray(const pcl::PointCloud<pcl::PointXYZI>::Ptr& in_pcl_pc_ptr, float* out_points_array,
...
void PointPillarsROS_Car::pointsCallback(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
  ...
  // {
    ...
    //std::cout << "[Top->PPillars_car]: " << diff_time << "ms" << std::endl;
  // }
  g_stopWatch_car.reset();
