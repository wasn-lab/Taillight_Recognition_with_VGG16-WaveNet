...
#include "lidar_point_pillars/ped_cyc/point_pillars_ped_cyc.h"

// clang-format off
PointPillars_Ped_Cyc::PointPillars_Ped_Cyc(const bool reproduce_result_mode, const float score_threshold,
  ...
  , ANCHOR_DX_SIZE_(0.6f) // 1.6f
  , ANCHOR_DY_SIZE_(1.76f) // 3.9f
  , ANCHOR_DZ_SIZE_(1.73f) // 1.56f
  // , PILLAR_Z_SIZE_(4.0f) 
  // , MIN_X_RANGE_(-69.12f)
  // , MIN_Y_RANGE_(-39.68f) 
  // , MIN_Z_RANGE_(-3.0f)
...
PointPillars_Ped_Cyc::~PointPillars_Ped_Cyc()
{
  delete[] anchors_px_;
  delete[] anchors_py_;
  delete[] anchors_pz_;
...
void PointPillars_Ped_Cyc::deviceMemoryMalloc()
{
  // clang-format off
  ...
  GPU_CHECK(cudaMalloc((void**)&dev_filtered_label_, NUM_ANCHOR_ * sizeof(int)));
...
void PointPillars_Ped_Cyc::initAnchors()
{
...
void PointPillars_Ped_Cyc::generateAnchors(float* anchors_px_, float* anchors_py_, float* anchors_pz_, float* anchors_dx_,
                                   float* anchors_dy_, float* anchors_dz_, float* anchors_ro_)
{
  ...
  float x_stride = PILLAR_X_SIZE_ ; // 2020/10/13 from *2.0f to *1.0f
  float y_stride = PILLAR_Y_SIZE_ ;
...
void PointPillars_Ped_Cyc::putAnchorsInDeviceMemory()
{
  // clang-format off
  ...
  // GPU_CHECK(cudaMemcpy(dummy, anchors_px_, NUM_ANCHOR_ * sizeof(float), cudaMemcpyHostToDevice));
  // GPU_CHECK(cudaMemcpy(dummy, anchors_px_, NUM_ANCHOR_ * sizeof(float), cudaMemcpyHostToDevice));
  // GPU_CHECK(cudaMemcpy(anchors_px_, dev_anchors_px_, NUM_ANCHOR_ * sizeof(float), cudaMemcpyDeviceToHost));
...
void PointPillars_Ped_Cyc::convertAnchors2BoxAnchors(float* anchors_px, float* anchors_py, float* anchors_dx, float* anchors_dy,
...
void PointPillars_Ped_Cyc::initTRT()
{
...
void PointPillars_Ped_Cyc::onnxToTRTModel(const std::string& model_file,             // name of the onnx model
...
void PointPillars_Ped_Cyc::preprocessGPU(const float* in_points_array, const int in_num_points)
{
  float* dev_points;
  // clang-format off
...
void PointPillars_Ped_Cyc::preprocess(const float* in_points_array, const int in_num_points)
{
  if (reproduce_result_mode_)
  {
...
void PointPillars_Ped_Cyc::doInference(const float* in_points_array, const int in_num_points, std::vector<float>& out_detections, std::vector<float>& out_scores,
