...
struct LidarScan
{
  ...
  iterator begin()
  {
    return iterator(0, &this->data_);
  }

  static inline Point make_val(float x, float y, float z, float intensity, uint32_t, uint16_t, uint8_t, uint16_t noise,
                               uint32_t range)
  {
    Point p;
  ...
  struct iterator
  {
    ...
    inline Data::RowXpr operator*()
    {
      return data_->row(idx_);
    }

    inline Data::RowXpr operator[](int i)
    {
      return data_->row(idx_ + i);
    }
