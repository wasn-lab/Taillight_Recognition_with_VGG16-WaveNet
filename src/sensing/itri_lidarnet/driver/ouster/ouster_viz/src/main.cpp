...
void print_help()
{
  std::cout << "Usage: viz [options] [hostname] [udp_destination]\n"
            ...
            << "  -m <512x10 | 512x20 | 1024x10 | 1024x20 | 2048x10> : lidar mode, "
               "default 1024x10\n"
            << "  -f <path> : use provided metadata file; do not configure via TCP" << std::endl;
}
...
std::string read_metadata(const std::string& meta_file)
{
  ...
  if (!ifs)
  {
    std::cerr << "Failed to read " << meta_file << "; check that the path is valid" << std::endl;
...
int main(int argc, char** argv)
{
  ...
  if (do_config)
  {
    std::cout << "Configuring sensor: " << argv[optind] << " UDP Destination:" << argv[optind + 1] << std::endl;
  ...
  auto batch_and_update = OS1::batch_to_iter<ouster::LidarScan::iterator>(
      xyz_lut, W, H, ouster::LidarScan::Point::Zero(), &ouster::LidarScan::make_val, [&](uint64_t) {
