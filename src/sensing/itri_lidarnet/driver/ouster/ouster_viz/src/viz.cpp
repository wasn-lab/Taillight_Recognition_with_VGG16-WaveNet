...
const std::vector<std::pair<std::string, vtkSmartPointer<vtkLookupTable>>> palettes = {
  { "Parula", palette_gen(parula) },   { "Viridis", palette_gen(viridis) }, { "Magma", palette_gen(magma) },
  { "Rainbow", palette_gen(rainbow) }, { "Autumn", palette_gen(autumn) },   { "Grey", palette_gen(grey) }
};
...
const std::vector<std::pair<std::string, ColorMode>> color_modes = { { "Z", COLOR_Z },
 ...
 * intensity
 **/
void color_intensity(Eigen::Ref<Eigen::ArrayXd> key_eigen, const VisualizerConfig& config)
{
 ...
 **/
void color_range(Eigen::Ref<Eigen::ArrayXd> range, const VisualizerConfig& config)
{
...
void color_noise(Eigen::Ref<Eigen::ArrayXd> key_eigen, const VisualizerConfig& config)
{
 ...
 **/
void update_color_key(const VisualizerConfig& config, const Points& xyz, Eigen::Ref<Eigen::ArrayXd> intensity,
                      Eigen::Ref<Eigen::ArrayXd> range, std::vector<double>& color_key)
{
 ...
 **/
void update_images(ouster::LidarScan& ls, Eigen::Array<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>& arr,
                   const std::vector<int>& px_offset, const VisualizerConfig& config)
{
  using MapXXd = Eigen::Map<Eigen::ArrayXXd>;
  using MapXXdr = Eigen::Map<Eigen::Array<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>;
  ...
  for (int u = 0; u < ls.H; u++)
  {
    ...
    dst.row(1 * ls.H - u - 1) << r.row(u).tail(ls.W - ofs), r.row(u).head(ofs);
    dst.row(2 * ls.H - u - 1) << i.row(u).tail(ls.W - ofs), i.row(u).head(ofs);
    dst.row(3 * ls.H - u - 1) << n.row(u).tail(ls.W - ofs), n.row(u).head(ofs);
  }
...
public:
  ...
  void OnKeyPress() override
  {
    ...
    if (key == "p")
    {
      ...
      if (vh->config.point_size > 1)
        vh->config.point_size--;
    }
    else if (key == "o")
    {
      std::cout << "Point size increased" << std::endl;
      if (vh->config.point_size < 100)
        vh->config.point_size++;
    }
    else if (key == "m")
    {
      vh->config.color_mode = (vh->config.color_mode + 1) % color_modes.size();
      std::cout << "Cycling point cloud color mode: " << color_modes[vh->config.color_mode].first << std::endl;
    }
    else if (key == "c")
    {
      vh->config.palette = (vh->config.palette + 1) % palettes.size();
      std::cout << "Cycling image color palette: " << palettes[vh->config.palette].first << endl;
    }
    else if (key == "C")
    {
      vh->config.c_palette = (vh->config.c_palette + 1) % palettes.size();
      std::cout << "Cycling point cloud color palette: " << palettes[vh->config.c_palette].first << std::endl;
    }
    else if (key == "v")
    {
      vh->config.cycle_range = !vh->config.cycle_range;
      std::cout << "Cycle range: " << (vh->config.cycle_range ? "On" : "Off") << std::endl;
    }
    else if (key == "n")
    {
      vh->config.image_noise = !vh->config.image_noise;
      std::cout << "Noise image: " << (vh->config.image_noise ? "On" : "Off") << std::endl;
    }
    else if (key == "0")
    {
      vh->config.parallel = !vh->config.parallel;
      std::cout << "Parallel projection: " << (vh->config.parallel ? "On" : "Off") << std::endl;
    }
    else if (key == "d")
    {
      vh->config.fraction_3d += 5;
      if (vh->config.fraction_3d > 100)
        vh->config.fraction_3d = 0;
      std::cout << "Cloud pane: " << vh->config.fraction_3d << "%" << std::endl;
    }
    else if (key == "r")
    {
...
vtkSmartPointer<vtkActor> init_cloud_actor(vtkSmartPointer<vtkPoints>& points, vtkSmartPointer<vtkDoubleArray>& color)
{
...
void fill_viewport(vtkSmartPointer<vtkRenderer> renderer, vtkSmartPointer<vtkImageData> image)
{
...
void run_viz(VizHandle& vh)
{
  ...
  vtkDoubleArray::SafeDownCast(points->GetData())->SetArray(pc_render.data(), n_points * 3, 1);
  ...
  vtkDoubleArray::SafeDownCast(image->GetPointData()->GetScalars())->SetArray(image_data.data(), 3 * n_points, 1);
  ...
  std::function<void()> on_render = [&]() {
    ...
    update_color_key(vh.config, pc_render, vh.lsb.front->intensity(), vh.lsb.front->range(), color_key);
  ...
  style->config_updated = [&]() {
    ...
    cloud_actor->GetMapper()->SetLookupTable(palettes[vh.config.c_palette].second);
...
std::shared_ptr<VizHandle> init_viz(int W, int H)
{
  ...
  vh->lsb.back = std::unique_ptr<ouster::LidarScan>(new ouster::LidarScan(W, H));

  vh->lsb.front = std::unique_ptr<ouster::LidarScan>(new ouster::LidarScan(W, H));
