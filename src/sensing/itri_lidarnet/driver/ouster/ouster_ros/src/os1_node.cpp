...
void populate_metadata_defaults(OS1::sensor_info& info, const std::string& specified_lidar_mode)
{
  ...
  if (v == OS1::invalid_version)
    ...
    ROS_WARN("Firmware < %s not supported; output may not be reliable", to_string(OS1::min_version).c_str());
...
std::string read_metadata(const std::string& meta_file)
{
  ...
  if (!ifs)
    ROS_WARN("Failed to read %s; check that the path is valid", meta_file.c_str());
...
int main(int argc, char** argv)
{
  ...
  auto srv = nh.advertiseService<OS1ConfigSrv::Request, OS1ConfigSrv::Response>(
  ...
  if (replay)
  {
    ...
    ROS_INFO("Sensor sn: %s firmware rev: %s", info.sn.c_str(), info.fw_rev.c_str());
  ...
  else
  {
    ...
    ROS_INFO("Sending data to %s using lidar_mode: %s", udp_dest.c_str(), lidar_mode.c_str());

    auto cli = OS1::init_client(hostname, udp_dest, OS1::lidar_mode_of_string(lidar_mode), lidar_port, imu_port);
