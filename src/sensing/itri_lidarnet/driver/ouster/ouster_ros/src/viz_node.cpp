...
int main(int argc, char** argv)
{
  ...
  auto W = OS1::n_cols_of_lidar_mode(OS1::lidar_mode_of_string(cfg.response.lidar_mode));
  ...
  auto cloud_handler = [&](const CloudOS1& cloud) {
    ...
    std::transform(cloud.begin(), cloud.end(), ls->begin(), [](const PointOS1& p) {
      return ouster::LidarScan::make_val(p.x, p.y, p.z, p.intensity, p.t, p.reflectivity, p.ring, p.noise, p.range);
    });

    viz::update(*vh, ls);
  };

  auto pc_sub = nh.subscribe<CloudOS1, const CloudOS1&>("points", 10, cloud_handler);
