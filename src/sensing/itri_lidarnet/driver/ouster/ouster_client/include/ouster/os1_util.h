 ...
 */
std::vector<double> make_xyz_lut(int W, int H, const std::vector<double>& beam_azimuth_angles,
...
std::function<void(const uint8_t*, iterator_type it)> batch_to_iter(const std::vector<double>& xyz_lut, int W, int H,
  ...
  return [=](const uint8_t* packet_buf, iterator_type it) mutable {
    for (int icol = 0; icol < OS1::columns_per_buffer; icol++)
    {
      ...
      for (uint8_t ipx = 0; ipx < H; ipx++)
      {
        ...
        it[idx + ipx] = c(r * 0.001f * xyz_lut[ind + 0], r * 0.001f * xyz_lut[ind + 1], r * 0.001f * xyz_lut[ind + 2],
                          OS1::px_signal_photons(px_buf), ts - scan_ts, OS1::px_reflectivity(px_buf), ipx,
