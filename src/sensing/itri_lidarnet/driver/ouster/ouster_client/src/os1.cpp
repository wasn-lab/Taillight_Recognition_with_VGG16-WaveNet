...
namespace
{
const std::array<std::pair<lidar_mode, std::string>, 5> lidar_mode_strings = { { { MODE_512x10, "512x10" },
                                                                                 { MODE_512x20, "512x20" },
...
bool do_tcp_cmd(int sock_fd, const std::vector<std::string>& cmd_tokens, std::string& res)
{
...
version version_of_string(const std::string& s)
{
  ...
  if (is && is.eof() && c1 == 'v' && c2 == '.' && c3 == '.' && v.major >= 0 && v.minor >= 0 && v.patch >= 0)
    return v;
  else
...
std::string to_string(lidar_mode mode)
{
  ...
  auto res = std::find_if(lidar_mode_strings.begin(), end,
                          [&](const std::pair<lidar_mode, std::string>& p) { return p.first == mode; });
...
lidar_mode lidar_mode_of_string(const std::string& s)
{
  auto end = lidar_mode_strings.end();
  auto res = std::find_if(lidar_mode_strings.begin(), end,
                          [&](const std::pair<lidar_mode, std::string>& p) { return p.second == s; });
...
std::shared_ptr<client> init_client(const std::string& hostname, const std::string& udp_dest_host, lidar_mode mode,
                                    int lidar_port, int imu_port)
{
  ...
  success &= reader->parse(res.c_str(), res.c_str() + res.size(), &cli->meta, &errors);

  success &= do_tcp_cmd(sock_fd, { "get_beam_intrinsics" }, res);
  success &= reader->parse(res.c_str(), res.c_str() + res.size(), &root, &errors);
