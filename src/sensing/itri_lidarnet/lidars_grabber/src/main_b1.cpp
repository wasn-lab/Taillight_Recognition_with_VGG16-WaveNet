...
void cloud_cb_LidarRearLeft(const boost::shared_ptr<const sensor_msgs::PointCloud2>& input_cloud)
{
  heartBeat[2] = true;
  pcl::fromROSMsg(*input_cloud, *cloudPtr_LidarRearLeft);
  syncLock_callback();
}
void cloud_cb_LidarRearRight(const boost::shared_ptr<const sensor_msgs::PointCloud2>& input_cloud)
{
  heartBeat[3] = true;
  ...
  *cloudPtr_LidAll += *cloudPtr_LidarFrontTop;
  ...
  if (n != 0)
  {
    avg_time = (cloudPtr_LidarFrontLeft->header.stamp + cloudPtr_LidarFrontRight->header.stamp +
                cloudPtr_LidarRearLeft->header.stamp + cloudPtr_LidarRearRight->header.stamp +
...
void syncLock_callback()
{
  ...
  if (GlobalVariable::STITCHING_MODE_NUM == 1)
  {
    ...
    if (heartBeat[2] == true)
    {
      ...
      *cloudPtr_LidarRearLeft = Transform_CUDA().compute<PointXYZI>(
          cloudPtr_LidarRearLeft, GlobalVariable::UI_PARA[12], GlobalVariable::UI_PARA[13], GlobalVariable::UI_PARA[14],
          GlobalVariable::UI_PARA[15], GlobalVariable::UI_PARA[16], GlobalVariable::UI_PARA[17]);
    ...
    if (heartBeat[3] == true)
    {
      ...
      if (GlobalVariable::RearRight_FineTune_Trigger == true)
      {
        ...
        LSA.setInitTransform(GlobalVariable::UI_PARA[18], GlobalVariable::UI_PARA[19], GlobalVariable::UI_PARA[20],
                             GlobalVariable::UI_PARA[21], GlobalVariable::UI_PARA[22], GlobalVariable::UI_PARA[23]);
      ...
      *cloudPtr_LidarRearRight = Transform_CUDA().compute<PointXYZI>(
          cloudPtr_LidarRearRight, GlobalVariable::UI_PARA[18], GlobalVariable::UI_PARA[19],
  ...
  else
  {
    ...
    if (heartBeat[2] == true)
    {
      heartBeat[2] = false;
      *cloudPtr_LidarRearLeft = Transform_CUDA().compute<PointXYZI>(
          cloudPtr_LidarRearLeft, LidarRearLeft_Fine_Param[0], LidarRearLeft_Fine_Param[1], LidarRearLeft_Fine_Param[2],
    ...
    if (heartBeat[3] == true)
    {
      heartBeat[3] = false;
      *cloudPtr_LidarRearRight = Transform_CUDA().compute<PointXYZI>(
          cloudPtr_LidarRearRight, LidarRearRight_Fine_Param[0], LidarRearRight_Fine_Param[1],
          LidarRearRight_Fine_Param[2], LidarRearRight_Fine_Param[3], LidarRearRight_Fine_Param[4],
          LidarRearRight_Fine_Param[5]);
      *cloudPtr_LidarRearRight =
          CuboidFilter().hollow_removal<PointXYZI>(cloudPtr_LidarRearRight, 0, 50, -0.8, 20, -5, 5);
      cloudPtr_LidarRearRight->header.frame_id = "lidar";
      pub_LidarRearRight.publish(*cloudPtr_LidarRearRight);
      checkPubFlag(3);
    }
...
int main(int argc, char** argv)
{
  ...
  else
  {
    ...
    cout << "LidarRearLeft is using "
         ...
         << ", " << LidarRearLeft_Fine_Param[5] << "]" << endl;

    cout << "LidarRearRight is using "
  ...
  ros::Subscriber sub_LidarRearLeft =
      n.subscribe<sensor_msgs::PointCloud2>("/LidarRearLeft/Raw", 1, cloud_cb_LidarRearLeft);
  ros::Subscriber sub_LidarRearRight =
      n.subscribe<sensor_msgs::PointCloud2>("/LidarRearRight/Raw", 1, cloud_cb_LidarRearRight);
