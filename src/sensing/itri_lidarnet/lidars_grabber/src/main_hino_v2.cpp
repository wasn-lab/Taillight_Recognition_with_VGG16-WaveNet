...
void
callback_LidarLeft (const boost::shared_ptr<const sensor_msgs::PointCloud2> &input_cloud)
{
  syncLock.lock ();
  heartBeat[0] = 0;
...
void
callback_LidarRight (const boost::shared_ptr<const sensor_msgs::PointCloud2> &input_cloud)
{
  syncLock.lock ();
  heartBeat[1] = 0;
...
void
callback_LidarFront (const boost::shared_ptr<const sensor_msgs::PointCloud2> &input_cloud)
{
  syncLock.lock ();
  heartBeat[2] = 0;
...
void
callback_LidarTop (const boost::shared_ptr<const sensor_msgs::PointCloud2> &input_cloud)
{
  syncLock.lock ();
  heartBeat[3] = 0;
      ...
      char **argv)
{
  ...
  RosModuleHINO::RegisterCallBackLidarRaw (callback_LidarFront, callback_LidarLeft, callback_LidarRight, callback_LidarTop);
  ...
  while (ros::ok ())
  {
    ...
    if (GlobalVariable::STITCHING_MODE_NUM == 1)
    {
      if (heartBeat[0] == 0)
      {
        ...
        *cloudLeft = Transform_CUDA ().compute<PointXYZI> (cloudLeft, GlobalVariable::UI_PARA[0], GlobalVariable::UI_PARA[1], GlobalVariable::UI_PARA[2],
      ...
      if (heartBeat[1] == 0)
      {
        ...
        *cloudRight = Transform_CUDA ().compute<PointXYZI> (cloudRight, GlobalVariable::UI_PARA[6], GlobalVariable::UI_PARA[7], GlobalVariable::UI_PARA[8],
      ...
      if (heartBeat[2] == 0)
      {
        ...
        *cloudFront = Transform_CUDA ().compute<PointXYZI> (cloudFront, GlobalVariable::UI_PARA[12], GlobalVariable::UI_PARA[13], GlobalVariable::UI_PARA[14],
    ...
    else  //=================================================================== Default Mode
    {
      ...
      {
        heartBeat[0] = 1;
        *cloudLeft = Transform_CUDA ().compute<PointXYZI> (cloudLeft, LidarLeft_Fine_Param[0], LidarLeft_Fine_Param[1], LidarLeft_Fine_Param[2],
      ...
      {
        heartBeat[1] = 1;
        *cloudRight = Transform_CUDA ().compute<PointXYZI> (cloudRight, LidarRight_Fine_Param[0], LidarRight_Fine_Param[1], LidarRight_Fine_Param[2],
      ...
      {
        heartBeat[2] = 1;
        *cloudFront = Transform_CUDA ().compute<PointXYZI> (cloudFront, LidarFront_Fine_Param[0], LidarFront_Fine_Param[1], LidarFront_Fine_Param[2],
    ...
    *cloudLidarAll += *cloudTop;
    ...
    pcl::uint64_t maxPCLtime = max (max (cloudLeft->header.stamp, cloudRight->header.stamp), max (cloudFront->header.stamp, cloudTop->header.stamp));
