...
void lidarAll_Pub(int lidarNum)
{
  ...
  *cloudPtr_LidAll += *cloudPtr_LidTop;
  ...
  if (n != 0)
  {
    avg_time = (cloudPtr_LidLeft->header.stamp + cloudPtr_LidRight->header.stamp + cloudPtr_LidFront->header.stamp +
                cloudPtr_LidTop->header.stamp) /
               n;
  }
  else
  {
...
void syncLock_callback()
{
  ...
  if (GlobalVariable::STITCHING_MODE_NUM == 1)
  {
    if (heartBeat[0] == true)
    {
      ...
      *cloudPtr_LidLeft = Transform_CUDA().compute<PointXYZI>(
          cloudPtr_LidLeft, GlobalVariable::UI_PARA[0], GlobalVariable::UI_PARA[1], GlobalVariable::UI_PARA[2],
          GlobalVariable::UI_PARA[3], GlobalVariable::UI_PARA[4], GlobalVariable::UI_PARA[5]);
    ...
    if (heartBeat[1] == true)
    {
      ...
      *cloudPtr_LidRight = Transform_CUDA().compute<PointXYZI>(
          cloudPtr_LidRight, GlobalVariable::UI_PARA[6], GlobalVariable::UI_PARA[7], GlobalVariable::UI_PARA[8],
          GlobalVariable::UI_PARA[9], GlobalVariable::UI_PARA[10], GlobalVariable::UI_PARA[11]);
    ...
    if (heartBeat[2] == true)
    {
      ...
      *cloudPtr_LidFront = Transform_CUDA().compute<PointXYZI>(
          cloudPtr_LidFront, GlobalVariable::UI_PARA[12], GlobalVariable::UI_PARA[13], GlobalVariable::UI_PARA[14],
          GlobalVariable::UI_PARA[15], GlobalVariable::UI_PARA[16], GlobalVariable::UI_PARA[17]);
  ...
  else  //=================================================================== Default Mode
  {
    ...
    if (heartBeat[0] == true)
    {
      heartBeat[0] = false;
      *cloudPtr_LidLeft = Transform_CUDA().compute<PointXYZI>(
          cloudPtr_LidLeft, LidarLeft_Fine_Param[0], LidarLeft_Fine_Param[1], LidarLeft_Fine_Param[2],
    ...
    if (heartBeat[1] == true)
    {
      heartBeat[1] = false;
      *cloudPtr_LidRight = Transform_CUDA().compute<PointXYZI>(
          cloudPtr_LidRight, LidarRight_Fine_Param[0], LidarRight_Fine_Param[1], LidarRight_Fine_Param[2],
    ...
    if (heartBeat[2] == true)
    {
      heartBeat[2] = false;
      *cloudPtr_LidFront = Transform_CUDA().compute<PointXYZI>(
          cloudPtr_LidFront, LidarFront_Fine_Param[0], LidarFront_Fine_Param[1], LidarFront_Fine_Param[2],
...
void cloud_cb_LidLeft(const boost::shared_ptr<const sensor_msgs::PointCloud2>& input_cloud)
{
  heartBeat[0] = true;
...
void cloud_cb_LidRight(const boost::shared_ptr<const sensor_msgs::PointCloud2>& input_cloud)
{
  heartBeat[1] = true;
...
void cloud_cb_LidFront(const boost::shared_ptr<const sensor_msgs::PointCloud2>& input_cloud)
{
  heartBeat[2] = true;
...
void cloud_cb_LidTop(const boost::shared_ptr<const sensor_msgs::PointCloud2>& input_cloud)
{
  heartBeat[3] = true;
...
int main(int argc, char** argv)
{
  ...
  if (ros::param::has("/LidarLeft_Fine_Param"))
  {
    ...
    cout << "LidarLeft is using  "
         << "[" << LidarLeft_Fine_Param[0] << ", " << LidarLeft_Fine_Param[1] << ", " << LidarLeft_Fine_Param[2] << ", "
         << LidarLeft_Fine_Param[3] << ", " << LidarLeft_Fine_Param[4] << ", " << LidarLeft_Fine_Param[5] << "]"
         << endl;

    cout << "LidarRight is using "
         << "[" << LidarRight_Fine_Param[0] << ", " << LidarRight_Fine_Param[1] << ", " << LidarRight_Fine_Param[2]
         << ", " << LidarRight_Fine_Param[3] << ", " << LidarRight_Fine_Param[4] << ", " << LidarRight_Fine_Param[5]
         << "]" << endl;

    cout << "LidarFront is using "
         << "[" << LidarFront_Fine_Param[0] << ", " << LidarFront_Fine_Param[1] << ", " << LidarFront_Fine_Param[2]
         << ", " << LidarFront_Fine_Param[3] << ", " << LidarFront_Fine_Param[4] << ", " << LidarFront_Fine_Param[5]
         << "]" << endl;
  }
  else
