...
void
callback_LidarAllNonGround (const pcl::PointCloud<pcl::PointXYZI>::ConstPtr& msg)
{
  syncLock.lock ();
  *ptr_cur_cloud = *msg;
  heartBeat[0] = 0;
...
void
callback_Camera30 (const msgs::DetectedObjectArray::ConstPtr&msg)
{
  syncLock.lock ();
...
void
callback_Camera60 (const msgs::DetectedObjectArray::ConstPtr&msg)
{
  syncLock.lock ();
...
void
callback_Camera120 (const msgs::DetectedObjectArray::ConstPtr&msg)
{
  syncLock.lock ();
      ...
      char ** argv)
{
  ...
  RosModuleHINO::initial ("convex_fusion", argc, argv);
  RosModuleHINO::RegisterCallBackLidarAllNonGround (callback_LidarAllNonGround);
  RosModuleHINO::RegisterCallBackCameraDetection (callback_Camera30, callback_Camera60, callback_Camera120);
  ...
  while (ros::ok ())
  {
    ...
    if (NumberABB > 0)
    {
      for (size_t i = 0; i < LidarAllNonGround.size (); i++)
      {
        ...
        {
          if (LidarAllNonGround.points[i].x > CameraABB[j].min.x and LidarAllNonGround.points[i].x < CameraABB[j].max.x
              and LidarAllNonGround.points[i].y < CameraABB[j].min.y and LidarAllNonGround.points[i].y > CameraABB[j].max.y)
          {
            CameraABB[j].cloud.push_back (PointXYZ (LidarAllNonGround.points[i].x, LidarAllNonGround.points[i].y, LidarAllNonGround.points[i].z));
          }
...
#pragma omp parallel for
      for (size_t i = 0; i < NumberABB; i++)
      {
        ...
        bbox2.Compute (CameraABB[i].obb_vertex, CameraABB[i].center, CameraABB[i].min, CameraABB[i].max, CameraABB[i].convex_hull);
      }

      RosModuleHINO::Send_CameraResults (CameraABB, NumberABB, frame_time, "lidar");
    }
