...
getContour(const pcl::PointCloud<PointXYZIL>::Ptr input_cloud, const float theta_sample, const float range_low_bound, const float range_up_bound)
{
  ...
  for (size_t i = 0; i < input_cloud->points.size(); i++)
  {
    ...
    float r_point = input_cloud->points[i].x*input_cloud->points[i].x + input_cloud->points[i].y*input_cloud->points[i].y;  // taking summation of squares (omit square root) as the range for comparison
    ...
    if (r_point > range_low_bound*range_low_bound && r_point < range_up_bound*range_up_bound)
    {
      ...
      float r_contour = contour_cloud.points[indx_theta].x*contour_cloud.points[indx_theta].x + contour_cloud.points[indx_theta].y*contour_cloud.points[indx_theta].y;
