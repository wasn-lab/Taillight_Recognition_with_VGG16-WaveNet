...
void LShape::setInputCloud(const pcl::PointCloud<pcl::PointXYZ> input)
{
  cloud_3d = input;
}

void LShape::Compute(std::vector<pcl::PointXYZ>& out_cube, pcl::PointXYZ& out_centroid, pcl::PointXYZ& out_minPoint,
                     pcl::PointXYZ& out_maxPoint)
{
  ...
  else
  {
    ...
    pcl::PointXYZ fff((point8.points.at(6).x + point8.points.at(0).x) / 2,
                      (point8.points.at(6).y + point8.points.at(0).y) / 2, 0);
...
void LShape::getPointsInPointcloudFrame(cv::Point2f rect_points[], std::vector<cv::Point2f>& pointcloud_frame_points,
...
void LShape::updateCpFromPoints(const std::vector<cv::Point2f>& pointcloud_frame_points, double& PoseX, double& PoseY,
                                double& PoseZ)
{
...
void LShape::toRightAngleBBox(std::vector<cv::Point2f>& pointcloud_frame_points)
{
  cv::Point2f p1 = pointcloud_frame_points[0];
  cv::Point2f p2 = pointcloud_frame_points[1];
  cv::Point2f p3 = pointcloud_frame_points[2];
...
void LShape::updateDimentionAndEstimatedAngle(const std::vector<cv::Point2f>& pointcloud_frame_points, double& PoseOX,
                                              double& PoseOY, double& PoseOZ, double& PoseOW, double& DimX,
                                              double& DimY, double& DimZ, double& Yaw)
{
