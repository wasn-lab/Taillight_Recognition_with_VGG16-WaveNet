...
void PCA::setInputCloud(const pcl::PointCloud<pcl::PointXYZ> input)
{
  cloud_3d = input;
  cloud_2d = input;
...
void PCA::compute(std::vector<pcl::PointXYZ>& out_cube, pcl::PointXYZ& out_centroid, Eigen::Matrix3f& out_covariance,
                  pcl::PointXYZ& out_minPoint, pcl::PointXYZ& out_maxPoint)
{
  ...
  // 2) compute principal directions, get the eigenvectors e0, e1, e2. The reference system will be (e0, e1, e0 X e1)
  // --- note: e0 X e1 = +/- e2
  Eigen::SelfAdjointEigenSolver<Eigen::Matrix3f> eigen_solver(out_covariance, Eigen::ComputeEigenvectors);
  Eigen::Matrix3f eigenVectorsPCA = eigen_solver.eigenvectors();
  // proper orientation in some cases. The numbers come out the same without it, but signs are different and box doesn't
  // get correctly oriented in some cases.
  ...
  // 3) move the points in that RF --- note: the transformation given by the rotation matrix (e0, e1, e0 X e1) & (c0,
  // c1, c2) must be inverted Transform the original cloud to the origin where the principal components correspond to
  // the axes.
  ...
  // 5) given a box centered at the origin with size (max_pt.x - min_pt.x, max_pt.y - min_pt.y, max_pt.z - min_pt.z) the
  // transformation you have to apply is Rotation = (e0, e1, e0 X e1) & Translation = Rotation * center_diag + (c0, c1,
  // c2)
...
void PCA::compute2(std::vector<pcl::PointXYZ>& out_cube, pcl::PointXYZ& out_centroid)
{
