...
void ConvexHull2D::compute()
{
  ...
  std::vector<PointData> indices;
  indices.reserve(m_p.cols());
  ...
  unsigned int currIdx = indicesT[i];

  // skip the first non left turns in the sequence!
  //    std::cout << "lastIdx point: " <<lastIdx << ","<<
  //    m_p.col(lastIdx).transpose() << std::endl;
  ...
  //    std::cout << ","<< m_p.col(currIdx).transpose() << std::endl <<
  //    "===="<<std::endl;
  //    std::cout << "0,5,8: :" << orient2d(m_p.col(0), m_p.col(5), m_p.col(8))
  //    << std::endl;
  //    std::cout << "1,5,8: :" << orient2d(m_p.col(1), m_p.col(5), m_p.col(8))
  //    << std::endl;
  ...
  if (i < nPoints)
  {
    ...
    for (++i; i < nPoints; ++i)
    {
      ...
      if (leftTurn(m_p.col(mPtIdx), m_p.col(currIdx), m_p.col(lastIdx)))
      {
        ...
        // std::cout << "left turn: " <<lPtIdx << ","<< mPtIdx << "," << currIdx
        // << std::endl;
        m_indicesCH.push_back(currIdx);
