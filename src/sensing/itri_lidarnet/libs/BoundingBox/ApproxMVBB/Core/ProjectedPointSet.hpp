...
#include "Common/AssertionDebug.hpp"
#include "Common/TypeDefs.hpp"
#include "Common/TypeDefsPoints.hpp"
...
namespace ApproxMVBB
{
     ...
     * direction m_dirZ
     * We output convexhull indices by adding output vector as parameter
     */
        template<typename Derived>
        OOBB computeMVBB(const Vector3& zDir, const MatrixBase<Derived>& points) //std::vector<unsigned int>& out_chIndices)
        {
            EIGEN_STATIC_ASSERT_MATRIX_SPECIFIC_SIZE(Derived, 3, Eigen::Dynamic);
            ...
            /**************************************************
             * for output of convex hull indices (deprecated) *
             **************************************************/
            //out_chIndices = mar.getConvexHull2D();
            // std::cout << "Points in convexhull : " << out_chIndices.size() << std::endl;
