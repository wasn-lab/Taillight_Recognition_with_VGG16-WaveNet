...
namespace ApproxMVBB
{
    ...
    */
                      ...
                      PREC minBoxExtent       = 1e-12)
    {
        ...
        //        #pragma omp declare reduction(volumeIsSmaller:OOBB: omp_in.volume()<omp_out.volume() ? omp_out=omp_in : omp_out ) initializer(omp_priv(omp_orig))
        //
        //        #pragma omp parallel for schedule(dynamic,1) shared(points) reduction(volumeIsSmaller:oobb) ApproxMVBB_OPENMP_NUMTHREADS
        ...
        for(int x = -int(gridSize); x <= (int)gridSize; ++x)
        {
            for(int y = -int(gridSize); y <= (int)gridSize; ++y)
            {
                for(int z = 0; z <= (int)gridSize; ++z)
                {
                    ...
                    //std::cout << "Points in convexhull : " << tmp.size() << std::endl;
    ...
    */
    ...
    OOBB approximateMVBBDiam(const MatrixBase<Derived>& points,
                             const PREC epsilon,
							 //std::vector<unsigned int>& out_chIndices,
                             ...
                             std::size_t seed            = ApproxMVBB::RandomGenerators::defaultSeed)
    {
        ...
        ProjectedPointSet proj;
        /*************************************************************
         * for passing output convex hull, we use the slower version *
         *************************************************************/
        //OOBB oobb = proj.computeMVBB(dirZ, points);
