...
{
double DiameterEstimator::estimateDiameter(Diameter::TypeSegment* theDiam, double const** theList, const int first,
                                           const int last, const int dim, double epsilon)
{
...
double DiameterEstimator::estimateDiameterInOneList(Diameter::TypeSegment* theDiam, double const** theList,
                                                    const int first, const int last, const int dim, double _epsilon_)
{
  using namespace Diameter;
  ...
  int reduction_mode_in_iterative = _GetReductionModeInIterative();
  int tryToReduceQ = _GetTryToReduceQ();
  int reduction_mode_of_diameter = _GetReductionModeOfDiameter();
  int reduction_mode_of_dbleNorm = _GetReductionModeOfDbleNorm();
  int Q_scan = _GetQscan();
  int tight_bounds = _GetTightBounds();
  ...
  do
  {
    ...
    if (newEstimate > theDiam->squareDiameter)
    {
      ...
      *theDiam = theSeg;
       ...
       */
      newlast = l;
      index = _FarthestPointFromSphere(&theSeg, theList, f, &newlast, dim, reduction_mode_in_iterative);
      if (reduction_mode_in_iterative == 1)
      {
  ...
  */
      ...
      bound = 4.0 * _ScalarProduct(theList[index], theDiam->extremity1, theList[index], theDiam->extremity2, dim) +
  ...
  */
      if (1.0 + 4.0 * _ScalarProduct(theList[index], theDiam->extremity1, theList[index], theDiam->extremity2, dim) /
                    theDiam->squareDiameter <
...
*/
  if (reduction_mode_in_iterative > 0 && reduction_mode_of_diameter == 1)
    reduction_mode_of_diameter = 0;

  newlast = l;
  index = _LastPointOutsideSphereWithDiameter(theDiam, theDiam->squareDiameter, theList, f, &newlast, dim,
                                              reduction_mode_of_diameter);
  if (reduction_mode_of_diameter == 1 || reduction_mode_of_diameter == 2)
  {
    if (verboseWhenReducing)
...
*/
  ...
  index1 = _LastPointOutsideSphereWithDiameter(theDiam, upperSquareDiameter, theList, f, &index2, dim, 0);
...
*/
  ...
  if (tryToReduceQ && theDoubleNormals.n > 1)
  {
    for (k = 0; k < theDoubleNormals.n; k++)
      theDoubleNormals.seg[k].reduction_mode = reduction_mode_of_dbleNorm;
  ...
  */
      i = _LastPointOutsideSphereWithDiameter(&theDoubleNormals.seg[n], upperSquareDiameter, theList, f, &index, dim,
                                              0);
      if (i >= index)
        continue;
  ...
  */
      if (tight_bounds)
      {
        for (k = i + 1; k <= index; k++)
        {
          bound = 4.0 * _ScalarProduct(theList[k], theDoubleNormals.seg[n].extremity1, theList[k],
                                       theDoubleNormals.seg[n].extremity2, dim) +
  ...
  */
      ...
      if (tight_bounds)
      {
        j = _LastPointOutsideSphereAndBoundWithDiameter(&theDoubleNormals.seg[n], upperSquareDiameter, theList,
                                                        index + 1, &newlast, dim,
                                                        theDoubleNormals.seg[n].reduction_mode, &bound);
        if (upperBound < bound)
          upperBound = bound;
      }
      else
      {
        j = _LastPointOutsideSphereWithDiameter(&theDoubleNormals.seg[n], upperSquareDiameter, theList, index + 1,
                                                &newlast, dim, theDoubleNormals.seg[n].reduction_mode);
      }
  ...
  */
      ...
      newEstimate =
          _QuadraticDiameterInTwoLists(&theSeg, nullptr, nullptr, theList, i + 1, index, theList, index + 1, j, dim);
