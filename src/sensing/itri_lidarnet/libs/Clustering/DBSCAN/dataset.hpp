...
public:
  ...
  void gen_data(size_t rows, size_t cols)
  {
    _data.clear();
    _labels.resize(rows);
  ...
  void load_pcl(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr input)
  {
    _data.clear();
  ...
  void load_pcl(const pcl::PointCloud<pcl::PointXYZIL>::ConstPtr input)
  {
    _data.clear();
    _data.resize(input->size());
    _labels.clear();
