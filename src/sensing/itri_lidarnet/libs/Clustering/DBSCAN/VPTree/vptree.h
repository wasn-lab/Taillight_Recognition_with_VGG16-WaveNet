...
private:
  ...
  struct HeapItem
  {
    HeapItem(size_t idx, double dist) : idx(idx), dist(dist)
    {
    }
    size_t idx;
    double dist;
...
public:
  ...
  VPTREE() : m_root_idx(FIRTS_NODE_IDX), m_next_idx(FIRTS_NODE_IDX), m_similar_dist(MIN_SIMILAR_DIST)
  {
  }

  VPTREE(double similar_dist) : m_root_idx(FIRTS_NODE_IDX), m_next_idx(FIRTS_NODE_IDX), m_similar_dist(similar_dist)
  {
  }
...
private:
  ...
  struct DistanceComparator
  {
    ...
    DistanceComparator(size_t i, const Dataset::DataContainer& d) : item_idx(i), items(d)
    {
    }
  ...
  void search_by_k(uint32_t node_idx, const T& target, TNeighborsList& nlist, size_t k, const Dataset::DataContainer& d,
                   TPQueue& heap, double& t, bool exclude_exact) const
  {
    if (node_idx == 0)
      return;
  ...
  void search_by_dist(uint32_t node_idx, const T& target, TNeighborsList& nlist, double t,
