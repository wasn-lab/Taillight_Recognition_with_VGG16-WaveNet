...
public:
  ...
  DBSCAN_VP(const Dataset::Ptr dset) : m_dset(dset), cluster_id(0)
  {
  }
...
#pragma omp ordered
  ...
  int32_t get_totoal_cluster_number()
  {
    return cluster_id;
  }
...
private:
  void find_neighbors(const Dataset::DataContainer& d, double eps, uint32_t pid, TVpTree::TNeighborsList& neighbors)
  {
    neighbors.clear();
