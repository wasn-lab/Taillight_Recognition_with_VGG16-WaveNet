...
PointCloud<PointXYZIR> NoiseFilter::runRingOutlierRemoval(PointCloud<PointXYZIR>::Ptr input, int ring_num,
                                                          float threshold)
{
  ...
  for (const auto& ring_pointcloud : pcl_input_ring_array)
  {
    ...
    for (auto iter = std::begin(ring_pointcloud.points); iter != std::end(ring_pointcloud.points) - 1; ++iter)
    {
      ...
      {
        float diff_point_1 = std::sqrt(std::pow(p_forward.x - p.x, 2.0) + std::pow(p_forward.y - p.y, 2.0) +
                                       std::pow(p_forward.z - p.z, 2.0));

        float diff_point_2 = std::sqrt(std::pow(p_forward.x - p_forward_forward.x, 2.0) +
