...
int
VoxelFilter_CUDA::getNumberOfAvailableThreads ()
{
  cudaDeviceProp prop;
...
bool
VoxelFilter_CUDA::removeNoiseNaive (pcl::PointCloud<pcl::PointXYZ> &point_cloud,
                                    float resolution,
                                    int number_of_points_in_bucket_threshold)
{
  ...
  err = cudaRemoveNoiseNaive (threads, d_markers, d_point_cloud, d_hashTable, d_buckets, rgd_params, point_cloud.points.size (),
                              number_of_points_in_bucket_threshold);
  if (err != ::cudaSuccess)
  {
    return false;
  }
...
PointCloud<PointXYZ>
VoxelFilter_CUDA::compute (PointCloud<PointXYZ>::Ptr input,
                           float resolution,
                           int number_of_points_in_bucket_threshold)
{
