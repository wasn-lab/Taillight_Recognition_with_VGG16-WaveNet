...
public:
  ...
  template <typename PointT>
  bool run(typename pcl::PointCloud<PointT>& point_cloud, Eigen::Affine3f matrix);

  template <typename PointT>
  PointCloud<PointT> compute(typename PointCloud<PointT>::Ptr input, float tx, float ty, float tz, float rx, float ry,
                             float rz);

  template <typename PointT>
  PointCloud<PointT> compute(typename PointCloud<PointT>::Ptr input, Eigen::Affine3f mr);
