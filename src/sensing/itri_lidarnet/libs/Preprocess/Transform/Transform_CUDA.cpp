...
bool Transform_CUDA::removePointsInsideSphere(pcl::PointCloud<pcl::PointXYZ>& point_cloud)
{
  ...
  err = cudaRemovePointsInsideSphere(maxThreadsNumber, d_point_cloud, d_markers, point_cloud.points.size(),
                                     sphere_radius);
  if (err != ::cudaSuccess)
  {
    return false;
...
bool Transform_CUDA::run(typename pcl::PointCloud<PointT>& point_cloud, Eigen::Affine3f matrix)
{
...
template bool Transform_CUDA::run(pcl::PointCloud<PointXYZ>& point_cloud, Eigen::Affine3f matrix);

template bool Transform_CUDA::run(pcl::PointCloud<PointXYZI>& point_cloud, Eigen::Affine3f matrix);

template <typename PointT>
PointCloud<PointT> Transform_CUDA::compute(typename PointCloud<PointT>::Ptr input, float tx, float ty, float tz,
...
template PointCloud<PointXYZ> Transform_CUDA::compute(typename PointCloud<PointXYZ>::Ptr input, float tx, float ty,
                                                      float tz, float rx, float ry, float rz);

template PointCloud<PointXYZI> Transform_CUDA::compute(typename PointCloud<PointXYZI>::Ptr input, float tx, float ty,
                                                       float tz, float rx, float ry, float rz);

template <typename PointT>
PointCloud<PointT> Transform_CUDA::compute(typename PointCloud<PointT>::Ptr input, Eigen::Affine3f mr)
{
  PointCloud<PointT> out_cloud;
  out_cloud = *input;
...
template PointCloud<PointXYZ> Transform_CUDA::compute(typename PointCloud<PointXYZ>::Ptr input, Eigen::Affine3f mr);

template PointCloud<PointXYZI> Transform_CUDA::compute(typename PointCloud<PointXYZI>::Ptr input, Eigen::Affine3f mr);
