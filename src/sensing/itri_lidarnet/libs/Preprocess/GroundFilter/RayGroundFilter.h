...
{
public:
  RayGroundFilter(float sensor_height, float general_max_slope, float local_max_slope, float radial_divider_angle,
                  float concentric_divider_distance, float min_height_threshold, float clipping_height,
                  float min_point_distance, float reclass_distance_threshold);

  template <typename PointT>
  PointIndices compute(typename PointCloud<PointT>::Ptr input);
...
private:
   ...
   */
  void ConvertXYZIToRTZColor(const PointCloud<PointXYZ>::Ptr in_cloud, PointCloudXYZIRTColor& out_organized_points,
  ...
  void ClassifyPointCloud(std::vector<PointCloudXYZIRTColor>& in_radial_ordered_clouds,
                          PointIndices& out_ground_indices, PointIndices& out_no_ground_indices);
   ...
   */
  void ClipCloud(const PointCloud<PointXYZ>::Ptr in_cloud_ptr, double in_clip_height,
   ...
   */
  void RemovePointsUpTo(const PointCloud<PointXYZ>::Ptr in_cloud_ptr, double in_min_distance,
