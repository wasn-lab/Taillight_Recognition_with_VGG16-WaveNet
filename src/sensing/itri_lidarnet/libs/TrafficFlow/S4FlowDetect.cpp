...
S4FlowDetect::S4FlowDetect() : viewer(NULL), viewID(NULL)
{
}

S4FlowDetect::S4FlowDetect(boost::shared_ptr<pcl::visualization::PCLVisualizer> input_viewer, int* input_viewID)
...
void S4FlowDetect::add_gateway_rectangle(pcl::PointXYZ rectangle_min, pcl::PointXYZ rectangle_max)
{
  pt_min = rectangle_min;
  pt_max = rectangle_max;
}

void S4FlowDetect::update(bool is_debug, CLUSTER_INFO* cluster_info, int cluster_number, int* counter)
{
  ...
  for (int i = 0; i < cluster_number; i++)
  {
    ...
    {
      pcl::PointXYZ pt(cluster_info[i].cloud.points[j].x, cluster_info[i].cloud.points[j].y,
                       cluster_info[i].cloud.points[j].z);
      result_point_into |= !(pt.x < pt_min.x || pt.y < pt_min.y || pt.z < pt_min.z || pt.x > pt_max.x ||
                             pt.y > pt_max.y || pt.z > pt_max.z);
    ...
    if (result_point_into && pcl::geometry::distance(pcl::PointXYZ(0, 0, 0), cluster_info[i].velocity) > 1.5)
    {
      ...
      {
        if (pcl::geometry::distance(cluster_info[i].track_last_center, it->center) < 0.1 &&
            cluster_info[i].track_last_center.x < 99999)
        {
...
void S4FlowDetect::show_info(int counter[], double system_running_time_second, double frames_running_time_second)
{
  ...
  viewer->addText("Flow (quantities/time):" + std::to_string(flow), 0, interval_y, 20, 1, 1, 1, std::to_string(*viewID),
                  0);
  ++*viewID;

  double Headway = 1 / flow;
  interval_y = interval_y - 25;
  viewer->addText("Headway (time/quantities):" + std::to_string(Headway), 0, interval_y, 20, 1, 1, 1,
                  std::to_string(*viewID), 0);
  ++*viewID;
...
/*
 ...
 result_point_into |= ! (pt.x < pt_min.x || pt.y < pt_min.y || pt.z < pt_min.z || pt.x > pt_max.x || pt.y > pt_max.y ||
 pt.z > pt_max.z); if(result_point_into)break;
 }
 ...
 cos_theta = (cluster_info[i].velocity.x * pre_detect_table.at (k).velocity.x + cluster_info[i].velocity.y *
 pre_detect_table.at (k).velocity.y
