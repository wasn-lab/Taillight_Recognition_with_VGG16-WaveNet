...
bool BoundingBoxModel::estimate(CLUSTER_INFO & cluster_info)
{
  ...
  centroid.x = centroid.x / (double)cluster_info.cloud.size();
  centroid.y = centroid.y / (double)cluster_info.cloud.size();
  centroid.z = centroid.z / (double)cluster_info.cloud.size();
  ...
  cluster_info.obb_center.y = (intersection_y_1 + intersection_y_2) / 2.0;
  cluster_info.obb_center.z = centroid.z;
  constexpr double ep = 0.001;
  cluster_info.obb_dx = std::fabs(e_x.dot(diagonal_vec));
  cluster_info.obb_dy = std::fabs(e_y.dot(diagonal_vec));
  cluster_info.obb_dz = std::max((max_z - min_z), ep);
