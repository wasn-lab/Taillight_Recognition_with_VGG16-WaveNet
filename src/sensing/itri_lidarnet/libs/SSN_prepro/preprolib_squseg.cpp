                           ...
                           const float imageWidth, const float theta_UPbound, const float theta_range)
{
  ...
  for (size_t i = 0; i < cloud_il->points.size(); i++)
  {
    ...
    if (phi >= phi_Nbound && phi <= phi_Pbound && theta <= theta_UPbound && theta >= theta_LOWbound)
    {
      ...
      {
        if ((filtered_cloud.points.at(theta_d * imageWidth + phi_d).d) == 0 ||
            d < (filtered_cloud.points.at(theta_d * imageWidth + phi_d).d))
        {
                          ...
                          const float imageWidth, const float theta_UPbound, const float theta_range)
{
  ...
  // cout << "filled_percentage (val over all) = " <<  (float) fill_count/ (float) cloud_i->points.size() * 100.0 << "%"
  // << endl; cout << "filled_percentage (val over filtered_cloud) = " <<  (float) fill_count/ (float) (imageHeight *
  // imageWidth) * 100.0 << "%" << endl;
...
void SSNspan_config(float* OUT_ptr, const char ViewType, const float phi_center)
{
  switch (ViewType)
  {
    ...
    case 'T':
    {
      const float SPAN_PARA[3][2] = { { 90, 512 }, { 180, 1024 }, { 90, 512 } };  // {span, imagewidth}
...
float proj_center(string data_set, int index)
{
  ...
  {
    float CENTER[2] = { -2.5, -0.6 };  // {x,z}, temporaly used for b1 test to let hino model as default, wil be reset
                                       // to {-3.0,-0.6}
    return CENTER[index];
  }
}
...
float SSNtheta_config(string data_set, int index)
{
  ...
  {
    float THETA_PARA[2] = { 18.0, 41.0 };  // {UPbound,range} temporally used para of hino model for demo, will be reset
                                           // to {16.0, 35.0}
    return THETA_PARA[index];
  }
}