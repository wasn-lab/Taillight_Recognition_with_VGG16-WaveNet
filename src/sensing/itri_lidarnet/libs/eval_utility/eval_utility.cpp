...
void pointcloudEval::IOUcal_pointwise(int class_index)
{
  ...
  for (size_t i = 0; i < GT_extract.points.size(); i++)
  {
    ...
    {
      if ((GT_extract.points.at(i).x == PD_extract.points.at(j).x) &&
          (GT_extract.points.at(i).y == PD_extract.points.at(j).y) &&
          (GT_extract.points.at(i).z == PD_extract.points.at(j).z))
      {
        intersect_cnt++;
        break;
      }
