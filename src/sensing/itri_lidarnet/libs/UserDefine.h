...
POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZIDL,
                                  (float, x, x)(float, y, y)(float, z, z)(float, intensity,
                                                                          intensity)(float, d, d)(int, label, label))

POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZLO,
                                  (float, x, x)(float, y, y)(float, z, z)(int, label, label)(int, object, object))

POINT_CLOUD_REGISTER_POINT_STRUCT(PointXYZIL, (float, x, x)(float, y, y)(float, z, z)(float, intensity,
                                                                                      intensity)(int, label, label))
...
struct CLUSTER_INFO
{
  ...
  // Classification
  int cluster_tag;  // tag =0 (not key object) tag =1 (unknown object) tag =2 (pedestrian) tag =3 (motorcycle)  tag =4
                    // (car)  tag =5 (bus)
  int confidence;
  float hull_vol;
  float GRSD21[21];
  ...
  // Output
  pcl::PointXY to_2d_PointWL[2];  // to_2d_PointWL[0] --> 2d image coordinate x,y     to_2d_PointWL[1] --> x = width y =
                                  // height
