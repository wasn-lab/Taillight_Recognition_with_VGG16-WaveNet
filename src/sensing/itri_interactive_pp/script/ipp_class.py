...
class buffer_data():
    def __init__(self):
        ...
        self.output_data_column = pd.MultiIndex.from_product(
            [['frame', 'node'], ['id']])
        self.output_data_column = self.output_data_column.append(
            self.data_columns_vehicle.copy())
    ...
    def add_temp_obstacle(self, present_id):
        ...
        for node_id in mask_id:
            ...
            node_df = node_df[node_df['frame_id'] ==
                              self.get_attribute("current_frame") - 1].copy()
            node_df = node_df.replace(
                'frame_id', self.get_attribute("current_frame"))
    ...
    def refresh_buffer(self):
        ...
        if len(self.frame_length) > 11:
            if self.frame_length[0] > 0:
                self.buffer_frame = self.buffer_frame.drop(
                    range(0, self.frame_length[0]))
    ...
    def update_predict_frame(self, data):
        ...
        if self.buffer_predicted_frame is not None:
            self.buffer_predicted_frame = pd.concat(
