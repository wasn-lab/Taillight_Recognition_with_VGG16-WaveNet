...
class Node(object):
    ...
    def scene_ts_to_node_ts(self, scene_ts):
        ...
        # print self.first_timestep, scene_ts[0]
        assert isinstance(ts,np.ndarray)
        assert isinstance(paddingl,int)
        assert isinstance(paddingu,int)
    ...
    def get(self, tr_scene, state, padding=np.nan):
        """
         -> np.ndarray
...
class MultiNode(Node):
    ...
    @staticmethod
    def find_non_overlapping_nodes(nodes_list, min_timesteps=1):
        """
         -> list
    ...
    def get_node_at_timesteps(self, scene_ts):
        #  -> Node
    ...
    def get_all(self, tr_scene, state, padding=np.nan):
        #  -> np.ndarray
