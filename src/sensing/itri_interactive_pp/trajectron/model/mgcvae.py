...
#         print(self.hyperparams['use_map_encoding'])
        if self.hyperparams['use_map_encoding']:
            if self.node_type in self.hyperparams['map_encoder']:
                ...
                ### edit by onebone
#                 print('')
#                 print('model if absent')
#                 print(model_if_absent)
    ...
    def encode_node_future(self, mode, node_present, node_future):
        """
    ...
    def encode_robot_future(self, mode, robot_present, robot_future):
        """
    ...
    def q_z_xy(self, mode, x, y_e):
        ...
        if self.debug:
            self.assertion(self.latent.dist_from_h(to_latent(h), mode))
    ...
    def project_to_GMM_params(self, tensor):
        ...
        if self.debug:
            self.assertion(log_pis)
            self.assertion(mus)
            self.assertion(log_sigmas)
            self.assertion(corrs)
