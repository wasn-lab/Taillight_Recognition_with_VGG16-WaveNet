                                    ...
                                    float resolution_X, float resolution_Y, float resolution_Z, gridParameters &out_rgd_params)
{
        ...
        try
        {
                thrust::device_ptr<pcl::PointXYZI> t_cloud(d_point_cloud);
                err = cudaGetLastError();
                if(err != ::cudaSuccess) return err;

                thrust::pair<thrust::device_ptr<pcl::PointXYZI>,thrust::device_ptr<pcl::PointXYZI> >
                ...
                pcl::PointXYZI minX,maxX,minZ,maxZ,minY,maxY;

                err = cudaMemcpy(&minX,minmaxX.first.get(),sizeof(pcl::PointXYZI),cudaMemcpyDeviceToHost);
                if(err != ::cudaSuccess) return err;
                err = cudaMemcpy(&maxX,minmaxX.second.get(),sizeof(pcl::PointXYZI),cudaMemcpyDeviceToHost);
                if(err != ::cudaSuccess) return err;
                err = cudaMemcpy(&minZ,minmaxZ.first.get(),sizeof(pcl::PointXYZI),cudaMemcpyDeviceToHost);
                if(err != ::cudaSuccess) return err;
                err = cudaMemcpy(&maxZ,minmaxZ.second.get(),sizeof(pcl::PointXYZI),cudaMemcpyDeviceToHost);
                if(err != ::cudaSuccess) return err;
                err = cudaMemcpy(&minY,minmaxY.first.get(),sizeof(pcl::PointXYZI),cudaMemcpyDeviceToHost);
                if(err != ::cudaSuccess) return err;
                err = cudaMemcpy(&maxY,minmaxY.second.get(),sizeof(pcl::PointXYZI),cudaMemcpyDeviceToHost);
                if(err != ::cudaSuccess) return err;
...
cudaError_t cudaCalculateGrid(int threads, pcl::PointXYZI* d_point_cloud, bucket *d_buckets,
                              hashElement *d_hashTable, int number_of_points, gridParameters rgd_params)
{
        ...
        try
        {
                // std::cout << "thrust::system_error1111"<< thrust::system_error <<std::endl;
                ...
                // std::cout << "thrust::system_error"<< thrust::system_error <<std::endl;
