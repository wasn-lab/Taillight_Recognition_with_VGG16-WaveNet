...
void chatterCallback1(const msgs::DetectedObjectArray::ConstPtr& msg)
{
  ...
  for (int i = 0; i < size; i++)
  {
    ...
    // cout << "Relative speed: (" << msg->objects[i].track.relative_velocity.x << "," <<
    // msg->objects[i].track.relative_velocity.y << ")" << endl;
    ...
    ABS_BB[i + 100] =
        cos(Heading) * rtObj.rtU.BoundingBox[i + 100] - sin(Heading) * rtObj.rtU.BoundingBox[i + 500] + SLAM_x;
    ABS_BB[i + 500] =
        sin(Heading) * rtObj.rtU.BoundingBox[i + 100] + cos(Heading) * rtObj.rtU.BoundingBox[i + 500] + SLAM_y;
    ABS_BB[i + 200] =
        cos(Heading) * rtObj.rtU.BoundingBox[i + 200] - sin(Heading) * rtObj.rtU.BoundingBox[i + 600] + SLAM_x;
    ABS_BB[i + 600] =
        sin(Heading) * rtObj.rtU.BoundingBox[i + 200] + cos(Heading) * rtObj.rtU.BoundingBox[i + 600] + SLAM_y;
    ABS_BB[i + 300] =
        cos(Heading) * rtObj.rtU.BoundingBox[i + 300] - sin(Heading) * rtObj.rtU.BoundingBox[i + 700] + SLAM_x;
    ABS_BB[i + 700] =
        sin(Heading) * rtObj.rtU.BoundingBox[i + 300] + cos(Heading) * rtObj.rtU.BoundingBox[i + 700] + SLAM_y;
    for (int i = 0; i < 800; i++)
    {
    ...
    /*if (msg->objects[i].track.is_ready_prediction==1)
    {
      ...
      cout << "PP.P1: (" << msg->objects[i].track.forecasts[4].position.x << "," <<
    ...
    endl;
      ...
      nbytes = write(s, &frame, sizeof(struct can_frame));
      cout << "PP.P3: (" << msg->objects[i].track.forecasts[14].position.x << "," <<
    msg->objects[i].track.forecasts[14].position.y << ")" << endl; cout << "PP.P4: (" <<
    msg->objects[i].track.forecasts[19].position.x << "," << msg->objects[i].track.forecasts[19].position.y << ")" <<
    endl;
    }*/
  }
}
