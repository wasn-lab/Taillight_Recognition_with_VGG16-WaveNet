...
void untitledModelClass::step()
{
  ...
  for (i = 0; i < 11; i++)
  {
    ...
    for (idx = 0; idx < 6; idx++)
    {
      matrix[((int32_T)(Accumulation / 0.1 + 1.0) + 11 * idx) - 1] = rt_powd_snf(Accumulation, (real_T)idx);
    }
  }
  ...
  for (i = 0; i < (int32_T)rtU.Input3; i++)
  {
    ...
    for (b_j = 0; b_j < 4; b_j++)
    {
      ...
      for (c_i = 0; c_i < 10; c_i++)
      {
        ...
        Accumulation += std::sqrt(Min_Range_one * Min_Range_one + Distance * Distance);
        Distance = (1.0 + (real_T)i) * 2.0;
        Min_Range_one = X_points_one[c_i] - rtU.Input2[((int32_T)(Distance - 1.0) + 100 * b_j) - 1];
        Distance = Y_points_one[c_i] - rtU.Input2[((int32_T)Distance + 100 * b_j) - 1];
    ...
    for (b_j = 0; b_j < 4; b_j++)
    {
      ...
      for (c_i = 0; c_i < 10; c_i++)
      {
        ...
        Distance = X_points_two[c_i] - rtU.Input2[((int32_T)((1.0 + (real_T)i) * 2.0 - 1.0) + 100 * b_j) - 1];
        b_a = Y_points_two[c_i] - rtU.Input2[((int32_T)((1.0 + (real_T)i) * 2.0) + 100 * b_j) - 1];
