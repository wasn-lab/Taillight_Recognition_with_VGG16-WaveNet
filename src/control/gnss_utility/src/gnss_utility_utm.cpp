...
namespace gnss_utility_utm
{

// DegToRad
// Converts degrees to radians.
FLOAT gnss_utm::DegToRad(FLOAT deg) {
  return (deg / 180.0 * pi);
}
...
FLOAT gnss_utm::RadToDeg(FLOAT rad) {
  return (rad / pi * 180.0);
}
...
FLOAT gnss_utm::ArcLengthOfMeridian (FLOAT phi) {
...
FLOAT gnss_utm::UTMCentralMeridian(int zone) {
  FLOAT cmeridian;
...
FLOAT gnss_utm::FootpointLatitude(FLOAT y) {
...
void gnss_utm::MapLatLonToXY (FLOAT phi, FLOAT lambda, FLOAT lambda0, FLOAT &x, FLOAT &y) {
...
void gnss_utm::MapXYToLatLon (FLOAT x, FLOAT y, FLOAT lambda0, FLOAT& phi, FLOAT& lambda)
{
...
int gnss_utm::LatLonToUTMXY (FLOAT lat, FLOAT lon, int zone, FLOAT& x, FLOAT& y) {
  if ( (zone < 1) || (zone > 60) )
...
// Returns:
// The function does not return a value.
void gnss_utm::UTMXYToLatLon (FLOAT x, FLOAT y, int zone, bool southhemi, FLOAT& lat, FLOAT& lon) {
  FLOAT cmeridian;
