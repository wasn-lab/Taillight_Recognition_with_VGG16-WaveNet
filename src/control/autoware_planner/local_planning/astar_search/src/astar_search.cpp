  ...
  for (int i = 0; i < height; i++)
  {
    for (int j = 0; j < width; j++)
    {
      ...
      if (use_potential_heuristic_)
      {
        ...
        // std::cout << "potential_weight_ : " << cost * potential_weight_ << std::endl;
      }
    }
  }
}
...
bool AstarSearch::setGoalNode(const geometry_msgs::Pose& goal_pose)
{
  ...
  {
    ROS_WARN("Goal is out of range or detect collision...");
    return false;
  }
...
bool AstarSearch::detectCollision(const SimpleNode& sn)
{
  ...
  for (double x = rear; x < front; x += resolution)
  {
    for (double y = left; y > right; y -= resolution)
    {
      // 2D point rotation
