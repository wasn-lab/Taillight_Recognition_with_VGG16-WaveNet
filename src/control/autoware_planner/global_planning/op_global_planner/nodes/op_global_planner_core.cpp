...
GlobalPlanner::GlobalPlanner()
{
  ...
  PlannerHNS::ROSHelpers::GetTransformFromTF("map", "map", transform);
  ...
  sub_current_pose = nh.subscribe("/rear_current_pose", 10, &GlobalPlanner::callbackGetCurrentPose, this);
  ...
  if(bVelSource == 0)
    ...
    sub_can_info = nh.subscribe("/can_info", 10, &GlobalPlanner::callbackGetCANInfo, this);
  	...
  	sub_veh_info = nh.subscribe("/veh_info", 10, &GlobalPlanner::callbackGetVehInfo, this);


  if(m_params.bEnableDynamicMapUpdate)
    sub_road_status_occupancy = nh.subscribe<>("/occupancy_wayarea", 1, &GlobalPlanner::callbackGetRoadStatusOccupancyGrid, this);
...
void GlobalPlanner::callbackGetVehInfo(const msgs::VehInfoConstPtr &msg)
{
  m_VehicleState.speed = msg->ego_speed;
  m_CurrentPose.v = m_VehicleState.speed;
