...
// C/C++ source code generated on : Thu Oct 24 16:36:16 2019
//
...
void MM_DPP_1ModelClass::merge(int32_T idx[118], real_T x[118], int32_T offset,
  int32_T np, int32_T nq, int32_T iwork[118], real_T xwork[118])
{
  int32_T n;
  int32_T q;
  int32_T iout;
...
// Function for MATLAB Function: '<S3>/SLAM_UKF_MM'
void MM_DPP_1ModelClass::sort(real_T x[118], int32_T idx[118])
{
  int32_T nNaNs;
  real_T xwork[118];
  int32_T iwork[118];
  ...
  x4[3] = 0.0;
  idx4[3] = 0;
  memset(&idx[0], 0, 118U * sizeof(int32_T));
  memset(&xwork[0], 0, 118U * sizeof(real_T));
  nNaNs = 0;
  ib = 0;
  for (m = 0; m < 118; m++) {
    if (rtIsNaN(x[m])) {
  ...
  if ((nNaNs & 1U) != 0U) {
    x[(m - nNaNs) + 118] = xwork[(m - nNaNs) + 118];
  }

  if (118 - nNaNs > 1) {
    memset(&iwork[0], 0, 118U * sizeof(int32_T));
    ib = (118 - nNaNs) >> 2;
    m = 4;
    while (ib > 1) {
      if ((ib & 1U) != 0U) {
    ...
    if (118 - nNaNs > m) {
      merge(idx, x, 0, m, 118 - (nNaNs + m), iwork, xwork);
    }
  }
}
...
void MM_DPP_1ModelClass::MM(real_T heading, const real_T X_pos[2], const real_T
  oi_xy[236], const real_T dist_op[118], const real_T Map_data[2714], real_T
  ...
  *seg_heading)
{
  ...
  real_T oi_xy_0[2];
  boolean_T exitg1;
  memcpy(&dist_ini[0], &dist_op[0], 118U * sizeof(real_T));
  sort(dist_ini, iidx);
  for (b_ii = 0; b_ii < 118; b_ii++) {
    b_index[b_ii] = iidx[b_ii];
    b_index[b_ii + 118] = 0;
    SEG_GPS_HEAD[b_ii] = Map_data[b_ii];
    SEG_GPS_HEAD[118 + b_ii] = Map_data[826 + b_ii];
  }

  for (idx = 0; idx < 118; idx++) {
    op_distance_n = Map_data[b_index[idx] + 117] - oi_xy[b_index[idx] - 1];
    C = Map_data[b_index[idx] + 235] - oi_xy[b_index[idx] + 117];
    c_a = Map_data[b_index[idx] + 353] - oi_xy[b_index[idx] - 1];
    d_a = Map_data[b_index[idx] + 471] - oi_xy[b_index[idx] + 117];
    if (std::sqrt(op_distance_n * op_distance_n + C * C) <= Map_data[b_index[idx]
        + 943]) {
      b_index[118 + idx] = (std::sqrt(c_a * c_a + d_a * d_a) <=
                            Map_data[b_index[idx] + 943]);
    } else {
      b_index[118 + idx] = 0;
    }

    x[idx] = (b_index[118 + idx] == 1);
  }

  idx = 0;
  b_ii = 1;
  exitg1 = false;
  while ((!exitg1) && (b_ii < 119)) {
    if (x[b_ii - 1]) {
      idx++;
  ...
  if (1 > idx) {
    *note = 1.0;
    rel_dist_xy(X_pos, &Map_data[118], dist_ini);
    rel_dist_xy(X_pos, &Map_data[354], dist_end);
    ...
    if (idx + 1 == 0) {
      ...
      for (b_ii = idx + 1; b_ii + 1 < 119; b_ii++) {
    ...
    *op_distance = dist_op[idx];
    oi_near[0] = oi_xy[idx];
    oi_near[1] = oi_xy[idx + 118];
    if ((idx + 1 > 1) && (idx + 1 < 118)) {
      oi_xy_0[0] = oi_xy[idx];
      oi_xy_0[1] = oi_xy[idx + 118];
      op_distance_n = rel_dist_xy_c(X_pos, oi_xy_0);
      oi_xy_0[0] = Map_data[idx + 118];
      oi_xy_0[1] = Map_data[idx + 236];
      if (op_distance_n < rel_dist_xy_c(X_pos, oi_xy_0)) {
        oi_xy_0[0] = Map_data[idx + 354];
        oi_xy_0[1] = Map_data[idx + 472];
        if (op_distance_n < rel_dist_xy_c(X_pos, oi_xy_0)) {
          *note = 0.0;
        }
      }
    ...
    *head_err = std::abs(heading - SEG_GPS_HEAD[idx + 119]);
    ...
    *op_distance = dist_op[idx];
    oi_near[0] = oi_xy[idx];
    oi_near[1] = oi_xy[b_index[ii_data[0] - 1] + 117];
    for (b_ii = 0; b_ii < 118; b_ii++) {
    ...
    *head_err = std::abs(heading - SEG_GPS_HEAD[idx + 119]);
    ...
    if (c_a < dist_op[b_index[ii_data[0] - 1] - 1]) {
      ...
      *head_err = std::abs(heading - SEG_GPS_HEAD[b_ii + 119]);
      if (*head_err <= 0.78539816339744828) {
        *seg_direction = 1.0;
      } else if ((*head_err >= 0.78539816339744828) && (*head_err <= 90.0)) {
        *seg_direction = 0.0;
  ...
  for (b_ii = 0; b_ii < 118; b_ii++) {
    dist_ini[b_ii] = Map_data[826 + b_ii] * 3.1415926535897931 / 180.0;
  ...
  op_distance_n = oi_near[1] - Map_data[590 + idx] * oi_near[0];
  if (Map_data[590 + idx] < 0.0) {
    C = (-Map_data[590 + idx] * X_pos[0] - op_distance_n) + X_pos[1];
  ...
  num_lane_direction[0] = Map_data[590 + idx];
...
{
  real_T c_x_data[118];
  real_T xwork_data[118];
  int32_T iwork_data[118];
  int32_T n;
  real_T x4[4];
  int8_T idx4[4];
  int32_T ib;
...
{
  int32_T dim;
  real_T vwork_data[118];
  int32_T vstride;
  int32_T iidx_data[118];
  int32_T b;
  int32_T c_k;
  int32_T vwork_size;
  int32_T tmp;
...
void MM_DPP_1ModelClass::rel_dist_xy_d(const real_T ref_xy[2], const real_T
  ...
  *dist_size)
{
  real_T pt_xy_data_0[118];
  real_T tmp_data[118];
  real_T tmp_data_0[118];
  int32_T loop_ub;
  int32_T i;
...
{
  real_T xy_ini_data[236];
  real_T xy_end_data[236];
  real_T seg_id_data[118];
  real_T ind_temp_data[118];
  real_T op_distance_n;
  real_T C;
  int32_T b_index_data[236];
  real_T SEG_GPS_HEAD_data[236];
  real_T dist_ini_data[118];
  real_T dist_end_data[118];
  boolean_T x_data[118];
  int32_T ii_data[118];
  int32_T nx;
  int32_T idx;
  int32_T g_idx;
...
void MM_DPP_1ModelClass::step()
{
  ...
  real_T head_err;
  real_T seg_heading;
  real_T oi_xy_data[236];
  real_T dist_op_data[118];
  real_T forward_length;
  real_T xy_ends_POS_data[472];
  real_T total_length;
  real_T Forward_Static_Path_id_0_data[118];
  int16_T cb_data[118];
  real_T Forward_Static_Path_data[474];
  real_T count;
  real_T count_1;
  real_T target_k;
  real_T Length_1;
  real_T ang_1;
  real_T Forward_Static_Path_data_0[474];
  real_T End_x;
  real_T y;
  real_T OBXY_m[8];
  real_T c;
  ...
  int32_T u_data[118];
  int32_T v_data[118];
  real_T end_heading_0_data[118];
  int32_T t_data_0[118];
  int32_T u_data_0[118];
  int32_T v_data_0[118];
  static const real_T a[11] = { 0.0, 0.1, 0.2, 0.30000000000000004, 0.4, 0.5,
    0.6, 0.7, 0.8, 0.9, 1.0 };

  real_T table[236];
  real_T shortest_distance[118];
  int8_T settled[118];
  int8_T pidx_data[118];
  int8_T zz_data[118];
  real_T tmp_path_data[118];
  int8_T nidx_data[118];
  int8_T c_data[118];
  int32_T ii_data_0[118];
  int32_T idx;
  int32_T b_idx;
  int32_T n;
  boolean_T x_data[118];
  boolean_T b_x[118];
  int8_T f_ii_data[118];
  real_T rtb_Add;
  real_T rtb_Gain1;
  real_T rtb_Gain_p;
  real_T rtb_X[5];
  ...
  real_T rtb_Forward_Static_Path_id_l[118];
  real_T rtb_Forward_Static_Path_x_h[118];
  real_T rtb_Forward_Static_Path_y_p[118];
  real_T rtb_Forward_Static_Path_id_i[118];
  real_T rtb_XP_i[78];
  real_T rtb_YP_g[78];
  real_T rtb_XP[78];
  real_T rtb_YP[78];
  ...
  rtb_Add = std::abs(1.0 - rtDW.Memory1_PreviousInput) + std::abs(118.0 -
  ...
  if (rtb_Add > 0.0) {
    ...
    memset(&table[0], 0, 236U * sizeof(real_T));
    for (i = 0; i < 118; i++) {
      shortest_distance[i] = (rtInf);
      settled[i] = 0;
    }

    memset(&rtDW.path[0], 0, 13924U * sizeof(real_T));
    idx = 0;
    i = 1;
    exitg1 = false;
    while ((!exitg1) && (i < 119)) {
    ...
    table[ii_data_0[0] + 117] = 0.0;
    settled[ii_data_0[0] - 1] = 1;
    rtDW.path[ii_data_0[0] - 1] = 1.0;
    b_idx = 0;
    idx = 1;
    exitg1 = false;
    while ((!exitg1) && (idx < 119)) {
      if (rtConstP.Constant3_Value[idx - 1] == 118.0) {
        b_idx++;
      ...
      if (settled[b_ix] == 0) {
        ...
        for (ix = 0; ix < 118; ix++) {
          b_x_0 = (rtConstP.Constant3_Value[pidx_data[0] - 1] ==
                   rtConstP.Constant5_Value[118 + ix]);
          if (b_x_0) {
            b_idx++;
          }
        ...
        for (i = 0; i < ix; i++) {
          for (iy = 0; iy < 118; iy++) {
            b_x[iy] = (rtConstP.Constant5_Value[c_data[i] + 235] ==
          ...
          if (!(settled[ii_data_0[0] - 1] != 0)) {
            rtb_X_state_0[0] = rtConstP.Constant3_Value[n] -
              rtConstP.Constant3_Value[ii_data_0[0] + 117];
            rtb_X_state_0[1] = rtConstP.Constant3_Value[pidx_data[0] + 235] -
              rtConstP.Constant3_Value[ii_data_0[0] + 235];
            ...
            if ((table[ii_data_0[0] - 1] == 0.0) || (table[ii_data_0[0] - 1] >
              ...
              table[ii_data_0[0] + 117] = table[pidx_data[0] - 1] + ajj;
              for (iy = 0; iy < 118; iy++) {
                b_x[iy] = (rtDW.path[(118 * iy + pidx_data[0]) - 1] != 0.0);
              }

              idx = 0;
              b_idx = 1;
              exitg1 = false;
              while ((!exitg1) && (b_idx < 119)) {
                if (b_x[b_idx - 1]) {
                  idx++;
              ...
              for (b_idx = 0; b_idx < c_ix; b_idx++) {
                tmp_path_data[b_idx] = rtDW.path[((f_ii_data[b_idx] - 1) * 118 +
                  pidx_data[0]) - 1];
              }

              idx = ii_data_0[0] - 1;
              for (iy = 0; iy < c_ix; iy++) {
                rtDW.path[idx + 118 * iy] = tmp_path_data[iy];
              }

              rtDW.path[idx + 118 * c_ix] = rtConstP.Constant5_Value[c_data[i] +
                235];
            } else {
              table[ii_data_0[0] + 117] = table[ii_data_0[0] - 1];
            }
          }
        }
        ...
        if (idx <= 2) {
          if (idx == 1) {
            delta_offset = table[ii_data_0[0] + 117];
          } else if (table[ii_data_0[0] + 117] > table[ii_data_0[1] + 117]) {
            delta_offset = table[ii_data_0[1] + 117];
          } else if (rtIsNaN(table[ii_data_0[0] + 117])) {
            if (!rtIsNaN(table[ii_data_0[1] + 117])) {
              delta_offset = table[ii_data_0[1] + 117];
            } else {
              delta_offset = table[ii_data_0[0] + 117];
            }
          ...
          if (!rtIsNaN(table[ii_data_0[0] + 117])) {
            b_idx = 1;
          } else {
            b_idx = 0;
            i = 2;
            exitg1 = false;
            while ((!exitg1) && (i <= idx)) {
              if (!rtIsNaN(table[ii_data_0[i - 1] + 117])) {
                b_idx = i;
                exitg1 = true;
              } else {
                i++;
          ...
          if (b_idx == 0) {
            ...
            delta_offset = table[ii_data_0[b_idx - 1] + 117];
            while (b_idx + 1 <= idx) {
              if (delta_offset > table[ii_data_0[b_idx] + 117]) {
                delta_offset = table[ii_data_0[b_idx] + 117];
              }
        ...
        for (iy = 0; iy < idx; iy++) {
          x_data[iy] = (table[ii_data_0[iy] + 117] == delta_offset);
        }
        ...
        if (idx == 0) {
          ...
          shortest_distance[i] = table[nidx_data[ii_data_0[0] - 1] + 117];
          settled[i] = 1;
        }
      } else {
        exitg2 = 1;
    ...
    for (iy = 0; iy < 118; iy++) {
      b_x[iy] = (rtDW.path[(118 * iy + zz_data[0]) - 1] != 0.0);
    }

    i = 0;
    idx = 0;
    exitg1 = false;
    while ((!exitg1) && (idx + 1 < 119)) {
      if (b_x[idx]) {
        i++;
        if (i >= 118) {
          exitg1 = true;
    ...
    for (b_ix = 0; b_ix < c_ix; b_ix++) {
      rtDW.path_2[b_ix] = rtDW.path[(118 * b_ix + zz_data[0]) - 1];
    }
  ...
  rtDW.SFunction_DIMS4_h[0] = 118;
  rtDW.SFunction_DIMS4_h[1] = 23;
  memcpy(&rtDW.Static_Path_0[0], &rtConstP.pooled2[0], 2714U * sizeof(real_T));
  ...
  for (ix = 0; ix < 118; ix++) {
    if (rtConstP.pooled2[590 + ix] == (rtInf)) {
      table[ix] = rtConstP.pooled2[118 + ix];
      table[118 + ix] = rtb_X[1];
    } else if (rtConstP.pooled2[590 + ix] == 0.0) {
      table[ix] = rtb_X[0];
      table[118 + ix] = rtConstP.pooled2[236 + ix];
    } else {
      rtb_Gain1 = -1.0 / rtConstP.pooled2[590 + ix];
      b_j = rtb_X[1] - rtb_Gain1 * rtb_X[0];
      forward_length = rtConstP.pooled2[590 + ix] - rtb_Gain1;
      table[ix] = (b_j - rtConstP.pooled2[708 + ix]) / forward_length;
      table[118 + ix] = (rtConstP.pooled2[590 + ix] * b_j - rtConstP.pooled2[708
                         + ix] * rtb_Gain1) / forward_length;
    }

    delta_offset = table[ix] - rtb_X[0];
    offset_6 = table[118 + ix] - rtb_X[1];
  ...
  rtDW.Memory_PreviousInput = 118.0;
