...
#include "mm_tp/MM_TP_object_msg.h"
...
int Path_flag, Freespace_mode;
float look_ahead_time_straight, look_ahead_time_turn;
int ID_1, ID_2, ID_3, ID_4;
float safe_range, takeover_mag, forward_length_2;
float w_l, w_k, w_k_1, w_obs, w_c, w_lane, w_fs;
...
void publishNavPath(double XP_final[], double YP_final[], double XP_final_1[], double YP_final_1[])
{
	nav_msgs::Path Dpath;
...
void rt_OneStep(void)
{
	...
	publishNavPath(rtObj.rtY.XP_final,rtObj.rtY.YP_final,rtObj.rtY.XP_final_1,rtObj.rtY.YP_final_1);
...
void mmtppubtopicCallback(const mm_tp::MM_TP_input_msg::ConstPtr& mmtpinputmsg)
{
  ...
  // rtObj.rtU.Look_ahead_time = mmtpinputmsg->Look_ahead_time;
	...
	for (int i =0 ; i<size ; i++)
	{
		...
		rtObj.rtU.BB_all_XY[2*i] = (mmtpinputmsg->BB_all_XY[i].object_p0x);
		rtObj.rtU.BB_all_XY[2*i+1] = (mmtpinputmsg->BB_all_XY[i].object_p0y);
		rtObj.rtU.BB_all_XY[2*i+100] = (mmtpinputmsg->BB_all_XY[i].object_p3x);
		rtObj.rtU.BB_all_XY[2*i+101] = (mmtpinputmsg->BB_all_XY[i].object_p3y);
		rtObj.rtU.BB_all_XY[2*i+200] = (mmtpinputmsg->BB_all_XY[i].object_p4x);
		rtObj.rtU.BB_all_XY[2*i+201] = (mmtpinputmsg->BB_all_XY[i].object_p4y);
		rtObj.rtU.BB_all_XY[2*i+300] = (mmtpinputmsg->BB_all_XY[i].object_p7x);
		rtObj.rtU.BB_all_XY[2*i+301] = (mmtpinputmsg->BB_all_XY[i].object_p7y);
		std::cout << "p0: (" << mmtpinputmsg->BB_all_XY[i].object_p0x << "," << mmtpinputmsg->BB_all_XY[i].object_p0y << ")" << std::endl;
		std::cout << "p3: (" << mmtpinputmsg->BB_all_XY[i].object_p3x << "," << mmtpinputmsg->BB_all_XY[i].object_p3y << ")" << std::endl;
		std::cout << "p4: (" << mmtpinputmsg->BB_all_XY[i].object_p4x << "," << mmtpinputmsg->BB_all_XY[i].object_p4y << ")" << std::endl;
		std::cout << "p7: (" << mmtpinputmsg->BB_all_XY[i].object_p7x << "," << mmtpinputmsg->BB_all_XY[i].object_p7y << ")" << std::endl;
		...
		point.x = mmtpinputmsg->BB_all_XY[i].object_p0x;
		point.y = mmtpinputmsg->BB_all_XY[i].object_p0y;
		polygon.polygon.points.push_back(point);
		point.x = mmtpinputmsg->BB_all_XY[i].object_p3x;
		point.y = mmtpinputmsg->BB_all_XY[i].object_p3y;
		polygon.polygon.points.push_back(point);
		point.x = mmtpinputmsg->BB_all_XY[i].object_p4x;
		point.y = mmtpinputmsg->BB_all_XY[i].object_p4y;
		polygon.polygon.points.push_back(point);
		point.x = mmtpinputmsg->BB_all_XY[i].object_p7x;
		point.y = mmtpinputmsg->BB_all_XY[i].object_p7y;
		polygon.polygon.points.push_back(point);										
	}
...
void FS_data_callback(const nav_msgs::OccupancyGrid& FSmsg)
{
	...
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
		{
			...
			{
				rtObj.rtU.Freespace[fs_index] = 1;
			}
			else
			{
				rtObj.rtU.Freespace[fs_index] = 0;
			}
		}
	}
}
...
int main(int argc, char **argv)
{
	...
	// ros::Subscriber FS_sub = nh.subscribe("costmap", 1, FS_data_callback);

	// publisher
	mmtp_pub = nh.advertise<mm_tp::MM_TP_msg>("mm_tp_topic", 1);
	dynamicpath_pub = nh.advertise<msgs::DynamicPath>("dynamic_path_para", 1);
	NavPath_Pub = nh.advertise<nav_msgs::Path>("Path", 1);
	polygon_pub = nh.advertise<geometry_msgs::PolygonStamped>("Polygon", 1);
