...
// C/C++ source code generated on : Fri Sep 27 17:42:07 2019
//
...
// Function for MATLAB Function: '<S2>/DynamicPathPlanning1'
void MM_DPP_1ModelClass::freespacedetectCollision_d(const real_T Freespace[37500],
  const real_T XP[6], const real_T YP[6], const real_T Vehicle_state[3], real_T
  forward_length, real_T safe_range, const real_T Veh_size[2], const real_T
  Veh_CG[2], real_T *U_c, real_T *safety_level_all, real_T *forward_length_free)
{
...
void MM_DPP_1ModelClass::step()
{
  ...
  if (rtb_Add > 0.0) {
    ...
    do {
      ...
      if (settled[b_ix] == 0) {
        ...
        for (b_idx = 0; b_idx < ix; b_idx++) {
          for (i = 0; i < 188; i++) {
            b_x[i] = (rtConstP.Constant5_Value[c_data[b_idx] + 375] ==
                      rtConstP.Constant3_Value[i]);
          }
          ...
          if (!(settled[ii_data_0[0] - 1] != 0)) {
            ...
            if ((table[ii_data_0[0] - 1] == 0.0) || (table[ii_data_0[0] - 1] >
              ...
              for (i = 0; i < 188; i++) {
                b_x[i] = (rtDW.path[(188 * i + pidx_data[0]) - 1] != 0.0);
              }
              ...
              idx = ii_data_0[0] - 1;
              for (i = 0; i < c_ix; i++) {
                rtDW.path[idx + 188 * i] = tmp_path_data[i];
              }

              rtDW.path[idx + 188 * c_ix] =
        ...
        for (i = 0; i < idx; i++) {
          nidx_data[i] = (uint8_T)ii_data_0[i];
        }
        ...
        for (i = 0; i < idx; i++) {
          x_data[i] = (table[ii_data_0[i] + 187] == offset_2);
        }
    ...
    for (i = 0; i < 188; i++) {
      b_x[i] = (rtDW.path[(188 * i + zz_data[0]) - 1] != 0.0);
    }
      ...
      if (c_ix < 5) {
        if (c_ix - 1 != 0) {
          ...
          for (iy = n + 6; iy <= ix; iy += 5) {
            ...
            end_heading = 0.0;
            i = (iy + c_ix) - 2;
            for (loop_ub = iy; loop_ub <= i; loop_ub++) {
        ...
        ajj = 1.0 / ajj;
        ix = ((4 - c_ix) * 5 + jj) + 6;
        for (b_idx = jj + 5; b_idx + 1 <= ix; b_idx += 5) {
  ...
  for (b_ix = 0; b_ix < loop_ub; b_ix++) {
    for (i = 0; i < b_idx; i++) {
      p_sqrt_data_0[i + b_idx * b_ix] = p_sqrt_data[5 * b_ix + i];
    }
  }
  ...
  for (b_ix = 0; b_ix < loop_ub; b_ix++) {
    for (i = 0; i < b_idx; i++) {
      ix = b_idx * b_ix;
      p_sqrt_data[i + ix] = p_sqrt_data_0[ix + i];
    }
  }
  ...
  if (idx != 0) {
    for (ix = 0; ix < 5; ix++) {
      temp_dia[ix] = std::abs(rtDW.UnitDelay33_DSTATE[5 * ix + ix]);
    }
    ...
    for (idx = 0; idx < b_idx; idx++) {
      for (c_ix = idx + 1; c_ix < b_idx; c_ix++) {
        p0[c_ix + 5 * idx] = 0.0;
      }
    }
  ...
  for (b_ix = 0; b_ix < b_idx; b_ix++) {
    for (i = 0; i < loop_ub; i++) {
      p_sqrt_data_0[i + loop_ub * b_ix] = p_sqrt_data[b_idx * i + b_ix] *
        2.23606797749979;
    }
  }
  ...
  for (ix = 0; ix < 5; ix++) {
    ...
    b_ix = ix + 2;
    for (i = 0; i < 5; i++) {
      rtb_X_AUG[i + 5 * (b_ix - 1)] = rtb_UnitDelay34[i] + temp_dia[i];
    }
  }
  ...
  for (b_ix = 0; b_ix < 5; b_ix++) {
    for (i = 0; i < 5; i++) {
      p_sqrt_data[b_ix + 5 * i] = rtb_UnitDelay34[b_ix] * temp_dia[i];
    }
  }

  for (b_ix = 0; b_ix < 5; b_ix++) {
    for (i = 0; i < 5; i++) {
      p0[i + 5 * b_ix] = p_sqrt_data[5 * b_ix + i] * 2.0;
    }
  }
  ...
  for (ix = 0; ix < 10; ix++) {
    ...
    for (b_ix = 0; b_ix < 5; b_ix++) {
      for (i = 0; i < 5; i++) {
        b_idx = 5 * b_ix + i;
        p0[i + 5 * b_ix] = p_sqrt_data[b_idx] * 0.1 + p0[b_idx];
      }
    }
  }
  ...
  if (d_idx > 0) {
    ...
    for (b_ix = 0; b_ix < 5; b_ix++) {
      for (i = 0; i < 5; i++) {
        ...
        rtb_K[b_idx] = 0.0;
        for (iy = 0; iy < 5; iy++) {
          rtb_K[b_idx] = p0[5 * iy + b_ix] * p_sqrt_data_0[5 * i + iy] + rtb_K[5
            * i + b_ix];
        }
      }
    ...
    for (b_ix = 0; b_ix < 5; b_ix++) {
      offset_2 = 0.0;
      for (i = 0; i < 5; i++) {
        offset_2 += rtb_K[5 * i + b_ix] * rtb_UnitDelay34[i];
      }
    ...
    for (b_ix = 0; b_ix < 5; b_ix++) {
      for (i = 0; i < 5; i++) {
        b_idx = 5 * b_ix + i;
        p_sqrt_data[i + 5 * b_ix] = (real_T)I[b_idx] - rtb_K[b_idx];
      }
    }
    ...
    for (b_ix = 0; b_ix < 5; b_ix++) {
      for (i = 0; i < 5; i++) {
        ...
        rtb_K[b_idx] = 0.0;
        for (iy = 0; iy < 5; iy++) {
          rtb_K[b_idx] = p_sqrt_data[5 * iy + i] * p0[5 * b_ix + iy] + rtb_K[5 *
            b_ix + i];
        }
      }
    }
  ...
  for (i = 0; i < loop_ub; i++) {
    x_data[i] = (rtDW.Static_Path_0[i] == rtb_Gain1);
  }
  ...
  for (i = 0; i < ix; i++) {
    x_data[i] = (dist_op_data[i] == rtb_Gain1);
  }

  d_idx = 1;
  b_x_0 = x_data[0];
  for (b_idx = 2; b_idx <= ix; b_idx++) {
  ...
  while ((!exitg1) && (ix <= rtDW.SFunction_DIMS4_h[0] - 1)) {
    if (total_length > forward_length) {
      ...
      if (iy <= rtDW.SFunction_DIMS4_h[0]) {
        total_length += rtDW.Static_Path_0[i + (rtDW.SFunction_DIMS4_h[0] << 3)];
        jj = iy;
        n = 1;
        ix++;
      } else if (c_ix == 1) {
  ...
  if ((n == 1) || (n == 0)) {
    ...
    iy = c_ix - jj;
    for (i = 0; i < iy; i++) {
      rtDW.Static_Path_ends_POS_data[i] = rtDW.xy_ends_POS_data[jj + i];
    }

    for (i = 0; i < iy; i++) {
      rtDW.Static_Path_ends_POS_data[i + iy] = rtDW.xy_ends_POS_data[(jj + i) +
        xy_ends_POS_size_idx_0];
    }

    for (i = 0; i < iy; i++) {
      rtDW.Static_Path_ends_POS_data[i + (iy << 1)] = rtDW.xy_ends_POS_data[(jj
        + i) + (xy_ends_POS_size_idx_0 << 1)];
    }

    for (i = 0; i < iy; i++) {
      rtDW.Static_Path_ends_POS_data[i + iy * 3] = rtDW.xy_ends_POS_data[(jj + i)
    ...
    loop_ub = n - ix;
    for (i = 0; i <= loop_ub; i++) {
      Forward_Static_Path_id_0_data[i] = dist_op_data[(ix + i) - 1];
    }
    ...
    iy = n + loop_ub;
    for (i = 0; i < n; i++) {
      rtDW.Static_Path_ends_POS_data[i] = rtDW.xy_ends_POS_data[c_ix + i];
    }

    for (i = 0; i < n; i++) {
      rtDW.Static_Path_ends_POS_data[i + iy] = rtDW.xy_ends_POS_data[(c_ix + i)
        + xy_ends_POS_size_idx_0];
    }

    for (i = 0; i < n; i++) {
      rtDW.Static_Path_ends_POS_data[i + (iy << 1)] = rtDW.xy_ends_POS_data
        [(c_ix + i) + (xy_ends_POS_size_idx_0 << 1)];
    }

    for (i = 0; i < n; i++) {
      rtDW.Static_Path_ends_POS_data[i + iy * 3] = rtDW.xy_ends_POS_data[(c_ix +
        i) + xy_ends_POS_size_idx_0 * 3];
    }

    for (i = 0; i < loop_ub; i++) {
      rtDW.Static_Path_ends_POS_data[(i + ix) - c_ix] = rtDW.xy_ends_POS_data[i];
    }

    for (i = 0; i < loop_ub; i++) {
      rtDW.Static_Path_ends_POS_data[((i + ix) - c_ix) + iy] =
        rtDW.xy_ends_POS_data[i + xy_ends_POS_size_idx_0];
    }

    for (i = 0; i < loop_ub; i++) {
      rtDW.Static_Path_ends_POS_data[((i + ix) - c_ix) + (iy << 1)] =
        rtDW.xy_ends_POS_data[(xy_ends_POS_size_idx_0 << 1) + i];
    }

    for (i = 0; i < loop_ub; i++) {
      rtDW.Static_Path_ends_POS_data[((i + ix) - c_ix) + iy * 3] =
    ...
    ix = ((rtDW.SFunction_DIMS4_h[0] - d_idx) + b_idx) + 1;
    if (1 > ix) {
      tmp_0 = 0;
    } else {
      tmp_0 = (int16_T)ix;
    ...
    jj = c_ix - n;
    for (i = 0; i < jj; i++) {
      table[i] = dist_op_data[n + i];
    }

    for (i = 0; i <= loop_ub; i++) {
      table[(i + c_ix) - n] = dist_op_data[i];
    }

    for (i = 0; i < ix; i++) {
      Forward_Static_Path_id_0_data[cb_data[i]] = table[i];
    }
    ...
    loop_ub = ix - c_ix;
    for (i = 0; i <= loop_ub; i++) {
      Forward_Static_Path_id_0_data[i] = dist_op_data[(c_ix + i) - 1];
    }
  ...
  for (i = 0; i <= loop_ub; i++) {
    rtDW.Forward_Static_Path_data[i + b_idx] = rtDW.Static_Path_ends_POS_data[i
      + iy];
  }
  ...
  loop_ub = b_idx - 1;
  for (i = 0; i <= loop_ub; i++) {
    rtb_Forward_Static_Path_y_p[i] = rtDW.Forward_Static_Path_data[i + b_idx];
  }
  ...
  if (Length_1 > 0.1) {
    ...
    for (c_ix = 0; c_ix < (int32_T)Length_1; c_ix++) {
      total_length = ((1.0 + (real_T)c_ix) - 1.0) * 0.1;
      rtDW.Path_RES_0_1[c_ix] = total_length * std::cos(ang_1) +
        rtb_Forward_Static_Path_x_h[0];
      rtDW.Path_RES_0_1[20000 + c_ix] = total_length * std::sin(ang_1) +
        rtb_Forward_Static_Path_y_p[0];
      total_length = 1.0 + (real_T)c_ix;
    }
  } else {
  ...
  i = jj;
  for (i = 0; i < jj; i++) {
    rtDW.rtb_X_data[i] = rtb_X[0] - rtDW.Path_RES_1_data[i];
  }

  power_ec(rtDW.rtb_X_data, &jj, rtDW.tmp_data, &n);
  for (i = 0; i < jj; i++) {
    rtDW.rtb_X_data[i] = rtb_X[1] - rtDW.Path_RES_1_data[i + jj];
  }
  ...
  if (n <= 2) {
    ...
    if (d_idx + 1 == 0) {
      ...
      for (ix = d_idx + 1; ix < n; ix++) {
        if (offset_2 > rtDW.ob_distance_data[ix]) {
          offset_2 = rtDW.ob_distance_data[ix];
          d_idx = ix;
        }
      }
    }
  ...
  if (rtDW.SFunction_DIMS2_h - 2 >= 1) {
    for (ix = 1; ix - 1 <= rtDW.SFunction_DIMS2_h - 3; ix++) {
      if (iy == 0) {
        ...
        total_length = rt_atan2d_snf(rtb_Forward_Static_Path_y_p[ix + 1] -
        ...
        if (ang_1 >= 0.1) {
          ...
          for (c_ix = 0; c_ix < (int32_T)ang_1; c_ix++) {
            ...
            b_idx = (int32_T)((1.0 + (real_T)c_ix) + count);
            rtDW.Path_RES_0_data[b_idx - 1] = end_heading * std::cos
              (total_length) + rtb_Forward_Static_Path_x_h[ix];
            rtDW.Path_RES_0_data[b_idx + 19999] = end_heading * std::sin
              (total_length) + rtb_Forward_Static_Path_y_p[ix];
          }
  ...
  if ((xy_ends_POS_size_idx_0 == 0) || (Path_RES_0_size_idx_1 == 0)) {
    if (total_length <= jj) {
      ...
      table_tmp = rtDW.Path_RES_1_data[d_idx - 1];
      head_err = rtDW.Path_RES_1_data[(d_idx + jj) - 1];
    ...
    for (i = 0; i < loop_ub; i++) {
      rtDW.Path_RES_0_1[i] = rtDW.Path_RES_1_data[d_idx + i];
    }

    for (i = 0; i < loop_ub; i++) {
      rtDW.Path_RES_0_1[i + loop_ub] = rtDW.Path_RES_1_data[(d_idx + i) +
    ...
    for (i = 0; i <= loop_ub; i++) {
      for (iy = 0; iy < n; iy++) {
        rtDW.Path_RES_0_data_k[iy + n * i] =
          rtDW.Path_RES_0_data[xy_ends_POS_size_idx_0 * i + iy];
      }
    }
    ...
    for (i = 0; i < c_ix; i++) {
      for (iy = 0; iy < ix; iy++) {
        rtDW.Path_RES_data[iy + b_idx * i] = rtDW.Path_RES_0_1[ix * i + iy];
      }
    }

    for (i = 0; i < c_ix; i++) {
      for (iy = 0; iy < jj; iy++) {
        rtDW.Path_RES_data[(iy + ix) + b_idx * i] = rtDW.Path_RES_0_data_k[jj *
          i + iy];
      }
    }
    ...
    if (target_k - Length_1 <= count) {
      ...
      head_err = rtDW.Path_RES_data[((int32_T)target_k + b_idx) - 1];
      rtb_Gain_p = target_k * 0.1;
    } else {
      target_k = count + Length_1;
      d_idx = (int32_T)target_k;
      table_tmp = rtDW.Path_RES_data[d_idx - 1];
      head_err = rtDW.Path_RES_data[(d_idx + b_idx) - 1];
      rtb_Gain_p = target_k * 0.1;
    }
  }
  ...
  loop_ub = rtDW.SFunction_DIMS4_h[0];
  for (i = 0; i < loop_ub; i++) {
    x_data[i] = (rtb_Forward_Static_Path_id_l[rtDW.SFunction_DIMS4_f - 1] ==
                 dist_op_data[i]);
  }
  ...
  for (i = 0; i <= loop_ub; i++) {
    rtDW.Forward_Static_Path_data_m[i + d_idx] =
      rtDW.Static_Path_ends_POS_data[i + iy];
  }
  ...
  rtDW.Forward_Static_Path_data_m[iy + d_idx] =
    rtDW.Static_Path_ends_POS_data[iy * 3 + i];
  rtDW.SFunction_DIMS2_a = d_idx;
  loop_ub = d_idx - 1;
  if (0 <= loop_ub) {
  ...
  rtDW.SFunction_DIMS3_a = d_idx;
  loop_ub = d_idx - 1;
  for (i = 0; i <= loop_ub; i++) {
    rtb_Forward_Static_Path_y_p[i] = rtDW.Forward_Static_Path_data_m[i + d_idx];
  }

  rtDW.SFunction_DIMS4_l = b_idx;
  if (0 <= b_idx - 1) {
  ...
  for (i = 0; i < jj; i++) {
    rtDW.tmp_data_c[i] = table_tmp - rtDW.Path_RES_1_data[i];
  }

  power_jb(rtDW.tmp_data_c, &jj, rtDW.tmp_data, &n);
  for (i = 0; i < jj; i++) {
    rtDW.rtb_X_data[i] = head_err - rtDW.Path_RES_1_data[i + jj];
  }
  ...
  if ((xy_ends_POS_size_idx_0 == 0) || (Path_RES_0_size_idx_1 == 0)) {
    if (total_length <= jj) {
      ...
      n = ix - d_idx;
      i = n + d_idx;
      total_length = rtDW.Path_RES_1_data[i - 1];
      ang_1 = rtDW.Path_RES_1_data[(i + jj) - 1];
      if (n == 0) {
        n = 0;
      } else {
        if (!(n > 2)) {
    ...
    if (150.0 - Length_1 <= count) {
      ...
      target_k = count + Length_1;
      i = (int32_T)target_k;
      total_length = rtDW.Path_RES_data[i - 1];
      ang_1 = rtDW.Path_RES_data[(i + b_idx) - 1];
      count = target_k * 0.1;
    }
  }
  ...
  for (ix = 0; ix < rtDW.SFunction_DIMS4_f; ix++) {
    loop_ub = rtDW.SFunction_DIMS4_h[0];
    for (i = 0; i < loop_ub; i++) {
      x_data[i] = (rtb_Forward_Static_Path_id_l[ix] == rtDW.Static_Path_0[i]);
    }

    d_idx = 0;
    b_x_0 = x_data[0];
    for (c_ix = 1; c_ix < rtDW.SFunction_DIMS4_h[0]; c_ix++) {
      if ((int32_T)b_x_0 < (int32_T)x_data[c_ix]) {
        b_x_0 = x_data[c_ix];
        d_idx = c_ix;
      }
    }
    ...
    for (i = 0; i < loop_ub; i++) {
      rtDW.Forward_Static_Path_0_data[ix + rtDW.SFunction_DIMS4_f * i] =
        rtDW.Static_Path_0[rtDW.SFunction_DIMS4_h[0] * i + d_idx];
    }
  }
  ...
  while ((!exitg1) && (n <= b_ix)) {
    offset_2 = (1.0 + (real_T)n) * 2.0;
    for (i = 0; i < 4; i++) {
      OBXY_m[i << 1] = rtb_V_boundingbox[((int32_T)(offset_2 + -1.0) + 100 * i)
        - 1];
      OBXY_m[1 + (i << 1)] = rtb_V_boundingbox[(100 * i + (int32_T)offset_2) - 1];
    }
    ...
    while ((!exitg3) && (c_ix <= rtDW.SFunction_DIMS4_f - 1)) {
      ...
      if (offset_2 <= Length_1) {
        if (!b_x_0) {
          ...
          exitg4 = false;
          while ((!exitg4) && (ix < 5)) {
            if (!rtIsNaN(rtb_num_lane_direction_f[ix - 1])) {
              b_idx = ix;
              exitg4 = true;
            } else {
              ix++;
  ...
  for (i = 0; i < ix; i++) {
    dist_op_data[i] = rtDW.Static_Path_0[(rtDW.SFunction_DIMS4_h[0] * 7 +
  ...
  if ((rtU.Freespace_mode == 0.0) || (rtU.Freespace_mode == 2.0)) {
    ...
    for (n = 0; n <= b_ix; n++) {
      ...
      OBXY_EL[(int32_T)(offset_2 + -1.0) + 99] = ((rtb_V_boundingbox[(int32_T)
        ...
        rtb_V_boundingbox[b_idx + 199]) * 0.3;
    ...
    for (i = 0; i < 13; i++) {
      for (iy = 0; iy < 10; iy++) {
        ...
        X_diff[iy + 11 * i] = x_0;
        X_diff_0[iy + 10 * i] = x_0;
      }

      idx = 10 + 11 * i;
      X_diff[idx] = X_diff_0[10 * i + 9];
      for (iy = 0; iy < 10; iy++) {
        b_idx = 11 * i + iy;
    ...
    for (i = 0; i < 143; i++) {
      x_0 = X_diff[i] / XY_difflen[i];
      offset_2 = Y_diff[i] / XY_difflen[i];
      offset_3 = 1.1 * -offset_2 + X_2[i];
      Path_vehFLX[i] = offset_3 + 1.4000000000000001 * x_0;
      yy_idx_0 = 1.1 * x_0 + Y[i];
      Path_vehFLY[i] = yy_idx_0 + 1.4000000000000001 * offset_2;
      offset_5 = X_2[i] - 1.1 * -offset_2;
      Path_vehFRX[i] = offset_5 + 1.4000000000000001 * x_0;
      offset_6 = Y[i] - 1.1 * x_0;
      Path_vehFRY[i] = offset_6 + 1.4000000000000001 * offset_2;
      Path_vehRLX[i] = offset_3 - 5.6000000000000005 * x_0;
      Path_vehRLY[i] = yy_idx_0 - 5.6000000000000005 * offset_2;
      Path_vehRRX[i] = offset_5 - 5.6000000000000005 * x_0;
      Path_vehRRY[i] = offset_6 - 5.6000000000000005 * offset_2;
      X_diff[i] = x_0;
      XY_difflen[i] = -offset_2;
      Y_diff[i] = offset_2;
    }
      ...
      if (!(Path_col[(ix << 2) + 3] == 1.0)) {
        ...
        while ((!exitg1) && (c_ix < 11)) {
          ...
          while ((!exitg3) && (b_idx <= b_ix)) {
            ...
            yy_idx_0 = OBXY_EL[d_idx + 199] - OBXY_EL[(int32_T)((1.0 + (real_T)
              b_idx) * 2.0) + 99];
            ...
            for (i = 0; i < 4; i++) {
              for (iy = 0; iy < 4; iy++) {
                proj_veh[i + (iy << 2)] = 0.0;
                proj_veh[i + (iy << 2)] += OBXY_m[iy << 1] * rtb_Oi_near_o[i];
                proj_veh[i + (iy << 2)] += OBXY_m[(iy << 1) + 1] *
                  rtb_Oi_near_o[i + 4];
              }

              OBXY_EL_0[i << 1] = OBXY_EL[((int32_T)(offset_2 + -1.0) + 100 * i)
                - 1];
              OBXY_EL_0[1 + (i << 1)] = OBXY_EL[(100 * i + (int32_T)offset_2) -
                1];
            }
            ...
            for (idx = 0; idx < 4; idx++) {
              for (i = 0; i < 4; i++) {
                proj_ob[idx + (i << 2)] = 0.0;
                proj_ob[idx + (i << 2)] += OBXY_EL_0[i << 1] *
                  rtb_Oi_near_o_0[idx];
                proj_ob[idx + (i << 2)] += OBXY_EL_0[(i << 1) + 1] *
            ...
            if (Path_col[ix << 2] == 1.0) {
              Path_col[2 + (ix << 2)] = 1.0 + (real_T)b_idx;
              exitg3 = true;
            } else {
              b_idx++;
            }
  ...
  if (x_0 != 0.0) {
    ...
    for (idx = 0; idx < 13; idx++) {
      ...
      for (i = 0; i < 13; i++) {
        Cc_0[i] = -rtb_U_c[i] / offset_2;
      }

      exp_n(Cc_0);
      for (i = 0; i < 13; i++) {
        rtb_U_c[i] = Path_col[i << 2] * (Cc_0[i] / offset_3);
      }

      Cobs[idx] = sum_a(rtb_U_c);
  ...
  if (x_0 != 0.0) {
    ...
    for (ix = 0; ix < 13; ix++) {
      ...
      exp_n(Cc_0);
      for (i = 0; i < 13; i++) {
        rtb_U_c[i] = Path_col[(i << 2) + 3] * (Cc_0[i] / offset_3);
      }

      Clane[ix] = sum_a(rtb_U_c);
  ...
  for (i = 0; i < 11; i++) {
    x[i] = rtDW.UnitDelay5_DSTATE[i] - rtb_TmpSignalConversionAtSFun_e[0];
  }

  power_d(x, K1_0);
  for (i = 0; i < 11; i++) {
    x[i] = rtDW.UnitDelay5_DSTATE[11 + i] - rtb_TmpSignalConversionAtSFun_e[1];
  }

  power_d(x, X2);
  for (i = 0; i < 11; i++) {
    b_Path_dis_data[i] = K1_0[i] + X2[i];
  }
  ...
  if (d_idx == 0) {
    ...
    for (c_ix = d_idx; c_ix + 1 < 12; c_ix++) {
      if (offset_2 > b_Path_dis_data[c_ix]) {
        offset_2 = b_Path_dis_data[c_ix];
        d_idx = c_ix + 1;
      }
    }
  }
  ...
  for (i = 0; i <= loop_ub + 11; i++) {
    LastPath_overlap_data[i] = rtDW.UnitDelay5_DSTATE[(d_idx + i) - 1];
  }

  loop_ub = -d_idx;
  for (i = 0; i <= loop_ub + 11; i++) {
    LastPath_overlap_data[i + jj] = rtDW.UnitDelay5_DSTATE[(d_idx + i) + 10];
  }

  for (b_idx = 0; b_idx < 13; b_idx++) {
    for (i = 0; i < 11; i++) {
      b_Path_dis_data[i] = X_2[11 * b_idx + i] - rtDW.UnitDelay5_DSTATE[10];
    }

    power_d(b_Path_dis_data, x);
    for (i = 0; i < 11; i++) {
      b_Path_dis_data[i] = Y[11 * b_idx + i] - rtDW.UnitDelay5_DSTATE[21];
    }

    power_d(b_Path_dis_data, K1_0);
    for (i = 0; i < 11; i++) {
      b_Path_dis_data[i] = x[i] + K1_0[i];
    }
    ...
    for (i = 0; i <= idx; i++) {
      Path_overlap_data[i + Path_overlap_size[0]] = Y[11 * b_idx + i];
    }
    ...
    if (12 - d_idx >= Path_overlap_size[0]) {
      ...
      for (iy = 0; iy < idx; iy++) {
        LastPath_overlap_data_0[iy] = LastPath_overlap_data[i + iy] -
          Path_overlap_data[iy];
      }

      for (iy = 0; iy < idx; iy++) {
        LastPath_overlap_data_0[iy + idx] = LastPath_overlap_data[(i + iy) + jj]
      ...
      for (i = 0; i < loop_ub; i++) {
        K_11[i] = b_Path_dis_data[oi_xy_size[0] * i];
      }
    ...
    for (i = 0; i <= idx; i++) {
      p_data[i] = (int8_T)((int8_T)((int8_T)(n * (int8_T)i) + ix) - 1);
    }
    ...
    for (i = 0; i < oi_xy_size[1]; i++) {
      offset_2 += b_Path_dis_data[i] * Path_dis_data[i];
    }
    ...
    Cc_0[b_idx] = rtb_J_out_k[b_idx] / 11.0 * offset_2 / 3.0 /
      (rtb_J_out_k[b_idx] * (real_T)iy / 11.0);
  }
  ...
  for (idx = 0; idx < 13; idx++) {
    ...
    for (d_idx = 0; d_idx < 10; d_idx++) {
      x_0 = rtb_U_c[idx];
      i = (11 * idx + d_idx) + 1;
      if ((!rtIsNaN(XY_difflen[i])) && (rtIsNaN(rtb_U_c[idx]) || (rtb_U_c[idx] <
            XY_difflen[i]))) {
        x_0 = XY_difflen[i];
      }
  ...
  for (idx = 0; idx < 13; idx++) {
    ...
    for (d_idx = 0; d_idx < 10; d_idx++) {
      ...
      if ((!rtIsNaN(XY_difflen[(11 * idx + d_idx) + 1])) && (rtIsNaN
           (rtb_J_fsc[idx]) || (rtb_J_fsc[idx] < XY_difflen[(11 * idx + d_idx) +
            1]))) {
        offset_2 = XY_difflen[(11 * idx + d_idx) + 1];
      }
  ...
  if (d_idx == 0) {
    ...
    x_0 = rtb_J_fsc[d_idx - 1];
    while (d_idx + 1 < 14) {
      if (x_0 < rtb_J_fsc[d_idx]) {
        x_0 = rtb_J_fsc[d_idx];
      }
  ...
  for (idx = 0; idx < 13; idx++) {
    ...
    for (d_idx = 0; d_idx < 10; d_idx++) {
      offset_2 = rtb_J_fsc[idx];
      i = (11 * idx + d_idx) + 1;
      if ((!rtIsNaN(XY_difflen[i])) && (rtIsNaN(rtb_J_fsc[idx]) ||
           (rtb_J_fsc[idx] < XY_difflen[i]))) {
        offset_2 = XY_difflen[i];
      }
  ...
  for (d_idx = 0; d_idx < 13; d_idx++) {
    ...
    for (n = 0; n < 10; n++) {
      offset_2 = Cobs_0[d_idx];
      i = (11 * d_idx + n) + 1;
      if ((!rtIsNaN(XY_difflen[i])) && (rtIsNaN(Cobs_0[d_idx]) || (Cobs_0[d_idx]
            < XY_difflen[i]))) {
        offset_2 = XY_difflen[i];
      }

      Cobs_0[d_idx] = offset_2;
    }
  ...
  for (i = 0; i < 13; i++) {
    rtb_J_out_k[i] = (((((rtb_U_c[i] * 10.0 / x_0 * rtU.W_1[1] + rtb_J_out_k[i] /
                          rtb_Gain_p * rtU.W_1[0]) + rtb_J_fsc[i] * 10.0 /
                         offset_2 * rtU.W_1[2]) + offset[i] / offset_3 *
                        end_heading) + rtU.W_1[3] * Cobs[i]) + rtU.W_1[4] *
                      Cc_0[i]) + rtU.W_1[5] * Clane[i];
  }
  ...
  loop_ub = rtDW.SFunction_DIMS4_h[0];
  for (i = 0; i < loop_ub; i++) {
    x_data[i] = (rtb_Forward_Static_Path_id_i[rtDW.SFunction_DIMS4_l - 1] ==
                 rtDW.Static_Path_0[i]);
  }
  ...
  for (i = 0; i < ix; i++) {
    Forward_Static_Path_id_0_data[i] = rtDW.Static_Path_0
      [(rtDW.SFunction_DIMS4_h[0] * 7 + t_data_0[i]) - 1] * 3.1415926535897931;
  }

  for (i = 0; i < ix; i++) {
    end_heading_0_data[i] = Forward_Static_Path_id_0_data[i] / 180.0;
  }
  ...
  G2splines_e(c, y_endpoint1, dist_op_data[0], rtDW.Static_Path_0[(u_data[0] +
    rtDW.SFunction_DIMS4_h[0] * 13) - 1], total_length + offset_6 * std::cos
              ...
              - 1], count, x, b_Path_dis_data, XP1, YP1, K1_0, K_11, &Cobs[0]);
  G2splines_e(x_endpoint2, y_endpoint2, dist_op_data[0], rtDW.Static_Path_0
              [(u_data[0] + rtDW.SFunction_DIMS4_h[0] * 13) - 1], total_length +
    ...
    rtDW.SFunction_DIMS4_h[0] * 13) - 1], count, X2, Y2, XP2, YP2, K1_0, K_11,
              &Cobs[1]);
  G2splines_e(x_endpoint3, y_endpoint3, dist_op_data[0], rtDW.Static_Path_0
              ...
              end_heading_0_data[0], rtDW.Static_Path_0[(u_data_0[0] +
    rtDW.SFunction_DIMS4_h[0] * 13) - 1], count, X3, Y3, XP3, YP3, K1_0, K_11,
              &Cobs[2]);
  G2splines_e(x_endpoint4, y_endpoint4, dist_op_data[0], rtDW.Static_Path_0
              ...
              end_heading_0_data[0], rtDW.Static_Path_0[(u_data_0[0] +
    rtDW.SFunction_DIMS4_h[0] * 13) - 1], count, X4, Y4, XP4, YP4, K1_0, K_11,
              &Cobs[3]);
  G2splines_e(x_endpoint5, y_endpoint5, dist_op_data[0], rtDW.Static_Path_0
              ...
              end_heading_0_data[0], rtDW.Static_Path_0[(u_data_0[0] +
    rtDW.SFunction_DIMS4_h[0] * 13) - 1], count, X5, Y5, XP5, YP5, K1_0, K_11,
              &Cobs[4]);
  G2splines_e(x_endpoint6, y_endpoint6, dist_op_data[0], rtDW.Static_Path_0
              ...
              rtDW.Static_Path_0[(u_data_0[0] + rtDW.SFunction_DIMS4_h[0] * 13)
              - 1], count, X6, Y6, XP6, YP6, K1_0, K_11, &Cobs[5]);
  G2splines_e(table_tmp, head_err, dist_op_data[0], rtDW.Static_Path_0[(u_data[0]
    + rtDW.SFunction_DIMS4_h[0] * 13) - 1], total_length, ang_1,
              end_heading_0_data[0], rtDW.Static_Path_0[(u_data_0[0] +
    rtDW.SFunction_DIMS4_h[0] * 13) - 1], count, X7, Y7, XP7, YP7, K1_0, K_11,
              &Cobs[6]);
  G2splines_e(x_endpoint8, y_endpoint8, dist_op_data[0], rtDW.Static_Path_0
              ...
              std::sin(end_heading - 1.5707963267948966), end_heading_0_data[0],
              rtDW.Static_Path_0[(u_data_0[0] + rtDW.SFunction_DIMS4_h[0] * 13)
              - 1], count, X8, Y8, XP8, YP8, K1_0, K_11, &Cobs[7]);
  G2splines_e(x_endpoint9, y_endpoint9, dist_op_data[0], rtDW.Static_Path_0
              [(u_data[0] + rtDW.SFunction_DIMS4_h[0] * 13) - 1], total_length +
              offset_2 * std::cos(end_heading - 1.5707963267948966), ang_1 +
              offset_2 * std::sin(end_heading - 1.5707963267948966),
              end_heading_0_data[0], rtDW.Static_Path_0[(u_data_0[0] +
    rtDW.SFunction_DIMS4_h[0] * 13) - 1], count, X9, Y9, XP9, YP9, K1_0, K_11,
              &Cobs[8]);
  G2splines_e(x_endpoint10, y_endpoint10, dist_op_data[0], rtDW.Static_Path_0
              ...
              offset_3 * std::sin(end_heading - 1.5707963267948966),
              end_heading_0_data[0], rtDW.Static_Path_0[(u_data_0[0] +
    rtDW.SFunction_DIMS4_h[0] * 13) - 1], count, X10, Y10, XP10, YP10, K1_0,
              K_11, &Cobs[9]);
  G2splines_e(x_endpoint11, y_endpoint11, dist_op_data[0], rtDW.Static_Path_0
              ...
              yy_idx_0 * std::sin(end_heading - 1.5707963267948966),
              end_heading_0_data[0], rtDW.Static_Path_0[(u_data_0[0] +
    rtDW.SFunction_DIMS4_h[0] * 13) - 1], count, X11, Y11, XP11, YP11, K1_0,
              K_11, &Cobs[10]);
  G2splines_e(x_endpoint12, y_endpoint12, dist_op_data[0], rtDW.Static_Path_0
              ...
              offset_5 * std::sin(end_heading - 1.5707963267948966),
              end_heading_0_data[0], rtDW.Static_Path_0[(u_data_0[0] +
    rtDW.SFunction_DIMS4_h[0] * 13) - 1], count, X12, Y12, XP12, YP12, K1_0,
              K_11, &Cobs[11]);
  G2splines_e(x_endpoint13, xy_end_point[25], dist_op_data[0],
    ...
    + rtDW.SFunction_DIMS4_h[0] * 13) - 1], count, K1_0, K_11, XP13, YP13, K13,
              K_113, &Cobs[12]);
  for (i = 0; i < 11; i++) {
    X_2[i] = x[i];
    X_2[i + 11] = X2[i];
        ...
        while ((!exitg1) && (c_ix < 11)) {
          ...
          while ((!exitg3) && (b_idx <= b_ix)) {
            ...
            for (d_idx = 0; d_idx < 4; d_idx++) {
              for (i = 0; i < 4; i++) {
                proj_ob[d_idx + (i << 2)] = 0.0;
                proj_ob[d_idx + (i << 2)] += OBXY_EL_0[i << 1] *
                  rtb_Oi_near_o_0[d_idx];
                proj_ob[d_idx + (i << 2)] += OBXY_EL_0[(i << 1) + 1] *
  ...
  for (b_ix = 0; b_ix < 13; b_ix++) {
    offset[b_ix] = Path_col[b_ix << 2];
    rtb_U_c[b_ix] = Path_col[b_ix << 2];
  }
  ...
  if (x_0 != 0.0) {
    ...
    for (idx = 0; idx < 13; idx++) {
      ...
      power_dw3x(Cc_0, rtb_U_c);
      for (b_ix = 0; b_ix < 13; b_ix++) {
        Cobs_0[b_ix] = -rtb_U_c[b_ix] / offset_3;
      }

      exp_n(Cobs_0);
      for (b_ix = 0; b_ix < 13; b_ix++) {
        rtb_U_c[b_ix] = Path_col[b_ix << 2] * (Cobs_0[b_ix] / total_length);
      }

      offset[idx] = sum_a(rtb_U_c);
      if ((1 + idx == 1) && (Path_col[0] == 1.0)) {
  ...
  for (b_ix = 0; b_ix < 13; b_ix++) {
    ...
    rtb_U_c[b_ix] = Path_col[(b_ix << 2) + 3];
  }
  ...
  if (x_0 != 0.0) {
    ...
    for (ix = 0; ix < 13; ix++) {
      ...
      exp_n(Cobs_0);
      for (b_ix = 0; b_ix < 13; b_ix++) {
        rtb_U_c[b_ix] = Path_col[(b_ix << 2) + 3] * (Cobs_0[b_ix] / offset_2);
      }
  ...
  for (b_ix = 0; b_ix <= loop_ub + 11; b_ix++) {
    LastPath_overlap_data[b_ix] = rtDW.UnitDelay6_DSTATE[(d_idx + b_ix) - 1];
  }

  loop_ub = -d_idx;
  for (b_ix = 0; b_ix <= loop_ub + 11; b_ix++) {
    LastPath_overlap_data[b_ix + jj] = rtDW.UnitDelay6_DSTATE[(d_idx + b_ix) +
      10];
  }

  for (b_idx = 0; b_idx < 13; b_idx++) {
    for (b_ix = 0; b_ix < 11; b_ix++) {
      b_Path_dis_data[b_ix] = X_2[11 * b_idx + b_ix] - rtDW.UnitDelay6_DSTATE[10];
    }

    power_d(b_Path_dis_data, x);
    for (b_ix = 0; b_ix < 11; b_ix++) {
      b_Path_dis_data[b_ix] = Y[11 * b_idx + b_ix] - rtDW.UnitDelay6_DSTATE[21];
    }
    ...
    for (b_ix = 0; b_ix <= idx; b_ix++) {
      Path_overlap_data[b_ix + Path_overlap_size[0]] = Y[11 * b_idx + b_ix];
    }
    ...
    if (12 - d_idx >= Path_overlap_size[0]) {
      ...
      for (i = 0; i < idx; i++) {
        LastPath_overlap_data_0[i] = LastPath_overlap_data[b_ix + i] -
          Path_overlap_data[i];
      }

      for (i = 0; i < idx; i++) {
        LastPath_overlap_data_0[i + idx] = LastPath_overlap_data[(b_ix + i) + jj]
          - Path_overlap_data[i + Path_overlap_size[0]];
      }
    ...
    Cobs_0[b_idx] = Cobs[b_idx] / 11.0 * offset_2 / 3.0 / (Cobs[b_idx] * (real_T)
      iy / 11.0);
  }
  ...
  if ((rtU.Freespace_mode == 1.0) || (rtU.Freespace_mode == 2.0)) {
    for (idx = 0; idx < 13; idx++) {
      xy_end_point_0[0] = xy_end_point[idx << 1];
      xy_end_point_0[1] = xy_end_point[(idx << 1) + 1];
      xy_end_point_0[2] = dist_op_data[0];
      freespacedetectCollision_d(rtU.Freespace, &rtb_XP[6 * idx], &rtb_YP[6 *
        idx], xy_end_point_0, count, rtU.safe_range, rtConstP.pooled9,
        rtConstP.pooled8, &rtb_U_c[idx], &Cobs[idx], &Cc_0[idx]);
    }
  }
  ...
  for (idx = 0; idx < 13; idx++) {
    if (rtb_U_c_l[idx] == 1.0) {
      ...
      offset_2 = Cobs_0[idx] * rtU.w_fs + Cobs[idx];
    } else {
      memcpy(&rtb_U_c[0], &rtb_forward_length_free_o[0], 13U * sizeof(real_T));
      offset_2 = Clane[idx] * rtU.w_fs + rtb_safety_level_all_p[idx];
    }

    rtb_J_fsc[idx] = offset_2;
    rtb_J_out_k[idx] = (rtb_J_out_k[idx] + offset[idx]) + offset_2;
  }
  ...
  if (!rtIsNaN(rtb_J_out_k[0])) {
    ...
    exitg1 = false;
    while ((!exitg1) && (b_idx < 14)) {
      if (!rtIsNaN(rtb_J_out_k[b_idx - 1])) {
        idx = b_idx;
        exitg1 = true;
      } else {
        b_idx++;
  ...
  if (idx == 0) {
    ...
    for (b_idx = idx; b_idx + 1 < 14; b_idx++) {
      if (offset_2 > rtb_J_out_k[b_idx]) {
        offset_2 = rtb_J_out_k[b_idx];
        idx = b_idx + 1;
      }
    }
  }
  ...
  for (ix = 0; ix < 11; ix++) {
    ...
    rtDW.UnitDelay5_DSTATE[ix] = ((((rtb_XP_i[b_ix] * a[ix] + rtb_XP_i[6 * b_idx])
      + rtb_XP_i[d_idx] * ang_1) + rtb_XP_i[i] * rt_powd_snf(a[ix], 3.0)) +
      rtb_XP_i[c_ix] * rt_powd_snf(a[ix], 4.0)) + rtb_XP_i[n] * rt_powd_snf(a[ix],
      5.0);
    rtDW.UnitDelay5_DSTATE[ix + 11] = ((((rtb_YP_g[b_ix] * a[ix] + rtb_YP_g[6 *
      b_idx]) + rtb_YP_g[d_idx] * ang_1) + rtb_YP_g[i] * rt_powd_snf(a[ix], 3.0))
      + rtb_YP_g[c_ix] * rt_powd_snf(a[ix], 4.0)) + rtb_YP_g[n] * rt_powd_snf
      (a[ix], 5.0);
    x[ix] = ang_1;
    b_Path_dis_data[ix] = rt_powd_snf(a[ix], 3.0);
    K1_0[ix] = rt_powd_snf(a[ix], 4.0);
    K_11[ix] = rt_powd_snf(a[ix], 5.0);
    X2[ix] = ang_1;
    Y2[ix] = rt_powd_snf(a[ix], 3.0);
    K2[ix] = rt_powd_snf(a[ix], 4.0);
    K_12[ix] = rt_powd_snf(a[ix], 5.0);
  }
  ...
  for (b_ix = 0; b_ix < 6; b_ix++) {
    ...
    rtY.XP_final[b_ix] = rtb_XP_i[d_idx];

    // Outport: '<Root>/YP_final' incorporates:
    //   MATLAB Function: '<S2>/Fianl_Path_Decision'

    rtY.YP_final[b_ix] = rtb_YP_g[d_idx];

    // Outport: '<Root>/XP_final_1' incorporates:
    //   MATLAB Function: '<S2>/Fianl_Path_Decision'

    rtY.XP_final_1[b_ix] = rtb_XP[d_idx];

    // Outport: '<Root>/YP_final_1' incorporates:
    //   MATLAB Function: '<S2>/Fianl_Path_Decision'

    rtY.YP_final_1[b_ix] = rtb_YP[d_idx];
  }
  ...
  if (forward_length <= rtb_Gain_p) {
    ...
    rtb_Gain_p = ((((rtb_XP_i[6 * b_idx + 1] * ajj + rtb_XP_i[6 * b_idx]) +
                    rtb_XP_i[6 * b_idx + 2] * (ajj * ajj)) + rtb_XP_i[6 * b_idx
                   + 3] * rt_powd_snf(ajj, 3.0)) + rtb_XP_i[6 * b_idx + 4] *
                  rt_powd_snf(ajj, 4.0)) + rtb_XP_i[6 * b_idx + 5] * rt_powd_snf
      (ajj, 5.0);
    ajj = ((((rtb_YP_g[6 * b_idx + 1] * ajj + rtb_YP_g[6 * b_idx]) + rtb_YP_g[6 *
             b_idx + 2] * (ajj * ajj)) + rtb_YP_g[6 * b_idx + 3] * rt_powd_snf
            (ajj, 3.0)) + rtb_YP_g[6 * b_idx + 4] * rt_powd_snf(ajj, 4.0)) +
      rtb_YP_g[6 * b_idx + 5] * rt_powd_snf(ajj, 5.0);
  } else if ((forward_length > rtb_Gain_p) && (forward_length <= rtb_Gain_p +
              count)) {
    ajj = (forward_length - rtb_Gain_p) / count;
    count = ajj * ajj;
    rtb_Gain_p = ((((rtb_XP[6 * b_idx + 1] * ajj + rtb_XP[6 * b_idx]) + rtb_XP[6
                    * b_idx + 2] * count) + rtb_XP[6 * b_idx + 3] * rt_powd_snf
                   (ajj, 3.0)) + rtb_XP[6 * b_idx + 4] * rt_powd_snf(ajj, 4.0))
      + rtb_XP[6 * b_idx + 5] * rt_powd_snf(ajj, 5.0);
    ajj = ((((rtb_YP[6 * b_idx + 1] * ajj + rtb_YP[6 * b_idx]) + rtb_YP[6 *
             b_idx + 2] * count) + rtb_YP[6 * b_idx + 3] * rt_powd_snf(ajj, 3.0))
           + rtb_YP[6 * b_idx + 4] * rt_powd_snf(ajj, 4.0)) + rtb_YP[6 * b_idx +
      5] * rt_powd_snf(ajj, 5.0);
  } else {
    rtb_Gain_p = ((((rtb_XP[6 * b_idx + 1] + rtb_XP[6 * b_idx]) + rtb_XP[6 *
                    b_idx + 2]) + rtb_XP[6 * b_idx + 3]) + rtb_XP[6 * b_idx + 4])
      + rtb_XP[6 * b_idx + 5];
    ajj = ((((rtb_YP[6 * b_idx + 1] + rtb_YP[6 * b_idx]) + rtb_YP[6 * b_idx + 2])
            + rtb_YP[6 * b_idx + 3]) + rtb_YP[6 * b_idx + 4]) + rtb_YP[6 * b_idx
      + 5];
  }

  rtb_Gain_p -= rtb_X[0];
  ajj -= rtb_X[1];
  count = std::sin(-rtb_TmpSignalConversionAtSFun_e[2]);
  ang_1 = std::cos(-rtb_TmpSignalConversionAtSFun_e[2]);

  // Outport: '<Root>/Vehicle_Target_x' incorporates:
  //   MATLAB Function: '<S2>/Target_Point_Decision'

  rtY.Vehicle_Target_x = ang_1 * rtb_Gain_p + -count * ajj;

  // Outport: '<Root>/Vehicle_Target_y' incorporates:
  //   MATLAB Function: '<S2>/Target_Point_Decision'

  rtY.Vehicle_Target_y = count * rtb_Gain_p + ang_1 * ajj;
  ...
  rtY.forward_length_free = rtb_U_c[b_idx];
  ...
  for (b_ix = 0; b_ix < 11; b_ix++) {
    ...
    rtDW.UnitDelay6_DSTATE[b_ix] = ((((rtb_XP[6 * b_idx + 1] * a[b_ix] + rtb_XP
      [6 * b_idx]) + rtb_XP[6 * b_idx + 2] * x[b_ix]) + rtb_XP[6 * b_idx + 3] *
      b_Path_dis_data[b_ix]) + rtb_XP[6 * b_idx + 4] * K1_0[b_ix]) + rtb_XP[6 *
      b_idx + 5] * K_11[b_ix];
    rtDW.UnitDelay6_DSTATE[b_ix + 11] = ((((rtb_YP[6 * b_idx + 1] * a[b_ix] +
      rtb_YP[6 * b_idx]) + rtb_YP[6 * b_idx + 2] * X2[b_ix]) + rtb_YP[6 * b_idx
      + 3] * Y2[b_ix]) + rtb_YP[6 * b_idx + 4] * K2[b_ix]) + rtb_YP[6 * b_idx +
      5] * K_12[b_ix];
  }
